...
> ## ★★★ 먼저 읽을 것 — 모든 수정에 적용하는 3원칙
> **① 단일 진입점** 한 가지 일은 한 곳에서만 · **② 동적 설계** 값은 런타임 파생(하드코딩 금지) ·
> **③ 모든 글에 적용** 네이버·티스토리 / 경제·테마 4조합 전부
>
> 전문·자가점검 체크리스트·위반 사고 이력: [`CLAUDE.md`](CLAUDE.md) 최상단.
> 도구를 가리지 않는 강제: git pre-commit 훅 (`git config core.hooksPath .githooks`).
