...
def _execute_j00_approval(key: str):
    ...
    if j01_cmd and _sched:
        ...
        # ★ FIX[7] (전수감사): build_j01_command 이 None(예: blog.post.revise 자유문장 경로)이면
        #   실제 처리가 없다 — 종전 '데몬이 직접 처리합니다'는 거짓 성공. 정직한 안내로 교체.
        #   (발행글 수정은 해당 글 인라인 승인 버튼 → 이벤트 버스 경로로만 동작)
        _send_tg(f"⚠️ `{intent}` — 이 작업은 자유문장 승인으로 실행되지 않습니다.\n"
                 f"발행글 수정은 해당 글의 인라인 승인 버튼으로만 가능합니다.")
