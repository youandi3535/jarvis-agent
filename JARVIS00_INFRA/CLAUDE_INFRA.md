...
| ★ `__main__` 진입점 ensure_preflight 의무 (ERRORS [154] 박제) | 외부 영향 가능한 모든 `if __name__ == "__main__"` 블록은 *반드시* `from JARVIS00_INFRA.preflight import ensure_preflight; ensure_preflight()` 선행 호출. 새 진입점 추가 시 자동 검증: `grep -rn '__name__.*__main__' --include='*.py' . \| xargs grep -L 'ensure_preflight' 2>/dev/null` — **단, 이 문장은 실측과 어긋나 있다** — 2026-07-29 실측 결과 누락 파일이 **13개**였다(`jarvis_daemon.py`·`api_server.py`·`JARVIS02_WRITER/scheduler.py` 등 외부 영향 진입점 포함). 종전엔 `log_monitor·agent_registration_check` 둘만이라 적혀 있었고 그중 `log_monitor.py` 는 2026-07-29 폐기돼 이제 존재하지도 않는다. 기대값을 문장에 박아 둔 탓에 드리프트가 조용히 자랐다 — **실측 대조 후 정리 필요**(전수 감사 잔여 항목). |
