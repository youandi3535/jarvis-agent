...
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| **10개** | **81,900+ LOC**<br/><sub>+ 대시보드 4,947</sub> | **201개** | **42개** | **28개** | **58종**<br/><sub>18 카테고리</sub> | **230건**<br/><sub>경제 89 · 테마 141</sub> |
...
flowchart LR
    D["📝 완성 대본"] --> V{"검증"}
    V --> F["사실성<br/><sub>수치 ↔ 수집 실데이터 대조</sub>"]
...
**검증 레그 6개** — 결정론 3 + LLM 1회 통합 호출 + 종합 점수.

| # | 검수 차원 | 판정 방식 | 실패 시 |
|:-:|----------|----------|--------|
| 1 | **사실성** | 본문의 모든 수치를 수집 실데이터와 대조 (grounding) | **차단** (fail-closed) |
| 2 | **종목 재무 실측** | PER·ROE·영업이익률 ±10% · 주가 ±5% 밴드 | 차단 |
| 3 | **본문 ↔ 차트 교차대조** | 같은 지표가 본문과 차트에서 다르면 ±3% 이내여야 | 차단 |
| 4 | **이미지 사실성** | 차트 수치 ↔ 수집 실데이터 · 출처 없으면 렌더 금지 | 실데이터 대체 / 숫자 없는 카드 |
| 5 | **매력도·유익성** | LLM 심사관 5축 채점 (매력·유익·제목·독창·구성) | → **6번의 A축(20점)으로 합류** |
| 6 | **종합 품질** ★ | 100점 루브릭 (항목 38~40개) | **70점 미달 → 재작성** |

> **품질 veto는 6번 하나뿐입니다.** 5축 개별 임계(70/70/60/60/65) veto는 2026-07-19 폐지됐습니다 —
> LLM 채점의 ±5점 노이즈가 괜찮은 글까지 재작성시켰기 때문입니다. 지금은 매력도 점수가
...
- **재시도는 최대 2회** (`harness.DEFAULT_MAX_ATTEMPTS`) — 같은 실패가 반복되면 즉시 중단하고 사람에게 보고합니다.
- **킬스위치 6개** — 코드 수정 없이 레그별 즉시 비활성화:
  `PREPUBLISH_FACT_GATE` · `PREPUBLISH_ENGAGEMENT_GATE` · `PREPUBLISH_IMAGE_GATE` ·
  `PREPUBLISH_CROSSCHECK_GATE` · `PREPUBLISH_SCORE_GATE` · `GATE_FAIL_CLOSED`
- 단일 진입점 `JARVIS02_WRITER/prepublish_gate.py` — 경제·테마 두 경로가 **같은 게이트**를 공유.
  사실성·매력도는 **LLM 1회 통합 호출**로 묶어 발행 지연을 줄였습니다.
...
| | 오류 학습 | 글 품질 학습 |
|---|---|---|
| 엔진 | `bandit.py` (Discounted Contextual Linear UCB) | `quality_learner.py` (UCB + 보상 EMA) |
| arm / 대상 | fixer 전략 | 작성 지침(insight) |
| 보상 신호 | **원 오류 재현 검증** 통과 여부 | **100점 루브릭 총점 ÷ 100** |
| 귀속 규칙 | **결과를 바꾼 결정만 기록** (귀속 가능성) | 사용 기록 ↔ 분석 결과 매칭 |
| 망각 | **γ=0.995 지수 감쇠** (기억창 ≈ 200관측) | weight EMA (α=0.3) |
...
> 2026-07-25 감사에서 밴딧이 **3,062회 동안 학습을 멈춘 채** 돌던 것이 발견됐습니다. 8개 arm 중
> 7개가 `n=3062 / rsum=-3062.0`(평균 정확히 −1.000)로 소수점까지 같았습니다. 원인은
> **"아무 fixer도 매칭 안 됨"을 "모든 fixer가 실패함"으로 기록**한 것 — 순서를 어떻게 정했든
> 결과가 같았던, 즉 **영향이 0이었던 결정**에 벌점을 주어 잡음이 신호를 127:1로 덮었습니다.
> 이후 *귀속 가능한 관측만 기록* + *감쇠* + *퇴화 감지*로 재설계했습니다
> ([ERRORS 498](JARVIS07_GUARDIAN/ERRORS.md)).
> **코드가 도는 것과 학습이 되는 것은 다른 문제**라는 것이 이 사고의 교훈입니다.
...
| 날짜 | 변경 | 왜 중요한가 |
|:----:|------|------------|
| 07-25 | **밴딧 학습 정지 복구** — 귀속 가능한 관측만 기록 + γ 감쇠 + 퇴화 감지 | 3,062회 동안 죽어 있던 강화학습이 실제로 arm을 구분하기 시작 |
| 07-25 | **학습 원장 원자 저장** — `os.replace` + `flock` 단일 헬퍼 | 테마 subprocess 전환으로 실재화된 **학습 자산 전멸** 위험 차단 |
| 07-25 | **오류 판별 재료를 로그 문구 → 구조화 신호로** | 코드버그가 정규식에 걸려 폐기되던 오탐 제거 (8/8 재현) |
| 07-25 | **자동수정 "성공" 판정에 원 오류 재현 검증** | 파싱만 통과한 *증상 은폐* 패치에 양의 보상을 주던 구조 제거 |
...
| 영역 | 사용 기술 |
|------|----------|
| **LLM** | Claude **Sonnet 5** 단일 모델 — `shared/llm.py` MODELS 단독 소유(alias 11개 전부 동일 모델).<br/>호출은 **Claude Code SDK subprocess**(Max 구독) → 외부 API 비용 0 |
