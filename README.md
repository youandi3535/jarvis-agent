...
|:-:|:-:|:-:|:-:|:-:|:-:|
| **10개** | **73,100+ LOC**<br><sub>(+ Next.js 대시보드 3,700 LOC)</sub> | **195개** | **29개** | **46종** | **3,300+건 · 해결률 89%**<br><sub>(2026-07-17 기준)</sub> |
...
| 기능 | 설명 |
|------|------|
| 📝 **블로그 자동 발행** | 경제 브리핑(매일 07:00) + 테마주 분석(매일 21:00) — 네이버·티스토리 동시 발행 |
...
| 영역 | 변경 | 근거 |
|------|------|------|
| **★ 전수 감사 — 세밀 로직·죽은코드·병목 (2026-07-17)** | 10개 모듈 전체를 5차원(연결성·죽은코드·재시도/병목·정확성·단일진입점)으로 병렬 감사 → 22건 확정·조치. **근본수정 8종**: EventListener 이중 부착(job_runs·학습지문 2배 오염)·경제 대본 폴백 infra-무인지(스로틀 자가증폭, 직전 rank3 한 층 위)·topic_pack `fit=True` 위조(인프라 실패가 '키워드 단독 전송 금지' 안전규정 우회)·SDK compat 직렬화/타임아웃 우회(야간 감사 상시 hang)·GUARDIAN 오류보고 사문화(`exc=` 오호출→학습 미기록)·즉시재실행 no-op·거짓 승인메시지. **대청소**: 죽은 파일·코드 12건 삭제(`.bak` 3·orphan 모듈 2·미사용 함수 클러스터)·**무소비 수집 난비 제거**(구독자 0으로 폐기되던 `collect_for_theme` 투기적 사전수집, 트렌드 수집 4회/일마다). *재시도·병목은 예외적 대비책, 정상은 1패스* 원칙 관철 | 전수 감사 |
...
gantt
    ...
    section 오전 발행 세트 (07:00)
        발행 전 자체수리 (Tier-1·LLM-0)    :active, 06:58, 2m
        경제 지표 수집 (JARVIS09)       :active, 07:00, 10m
        경제 브리핑 글 생성 (WRITER)    :active, 07:10, 15m
        AI 이미지 생성 (IMAGE)          :active, 07:25, 10m
        네이버·티스토리 발행 (PUBLISH)  :active, 07:35, 10m

    section 야간 발행 세트 (21:00)
        발행 전 자체수리 (Tier-1·LLM-0)    :20:58, 2m
        테마 트렌드 수집 (RADAR+J09)    :21:00, 10m
        테마주 분석 글 생성 (WRITER)    :21:10, 15m
        AI 이미지 생성 (IMAGE)          :21:25, 10m
        네이버·티스토리 발행 (PUBLISH)  :21:35, 10m
```

<sub>※ 실제 cron 잡은 **07:00 / 21:00 두 개의 세트 트리거**뿐입니다. 각 세트는 단일 콜백 안에서 *발행 전 Tier-1 자체수리(LLM-0, 수초)*→수집→글→이미지→발행을 *순차* 실행합니다. 비싼 LLM 심층 감사는 발행과 분리해 새벽 03:00(`j07_deep_audit`)으로 옮겨, 발행 지연을 0으로 만들었습니다(★ 2026-06-28). 인포그래픽 디자인 나이틀리 학습은 매일 05:00(`j06_design_learn`) — Phase0(실이미지)·Phase1(LLM 창작)·Phase2(결정론, 항상 보장). 위 간트의 하위 단계 시각은 흐름 이해용 예시입니다.</sub>

| 시각 | 잡 이름 | 내용 |
|------|---------|------|
| **07:00** | 경제 브리핑 세트 | 발행 전 Tier-1 자체수리(LLM-0) → 경제 지표 수집 → 글 작성 → 이미지 → **품질 게이트(사실성·매력도)** → 발행 |
| **21:00** | 테마주 분석 세트 | 발행 전 Tier-1 자체수리(LLM-0) → 트렌드 테마 선정 → 글 작성 → 이미지 → **품질 게이트(사실성·매력도)** → 발행 |
| **매일 03:30** | git 회고 | 전날 코드 변경 D-1 학습 자산화 |
| **매일 03:00** | 심층 코드 감사 | 미해결 backlog Tier1→Tier2(LLM, *실제 지문* 학습) + 광범위 코드 감사 → 패턴·밴딧 성장 |
...
| 🟡 통합 파이프라인 실발행 미검증 | 경제·테마 통합(`CollectedData`+`process_draft`)은 구조 검증(precommit 46종·전수 py_compile·단위테스트) + 테마 dry-run(블록 조립·이미지≥5·썸네일·grounding) 통과. 단 실제 스케줄 발행 E2E(경제 07:00·테마 21:00)는 첫 라이브에서 검증 | 첫 발행 모니터링 후 필요 시 튜닝 |
| 🟡 Max 구독 rate 천장 | 발행은 LLM 호출을 몰아서 함 → 인터랙티브 세션·데몬과 같은 Max 계정 공유 시 rate-limit 스로틀 가능. C1 배치(차트 설계 7→1회)로 완화, 발행은 무경쟁 시각(예약 07:00/21:00) 권장 | 발행 전용 API 키 분리 검토 |
