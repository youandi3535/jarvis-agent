...
import { fetcher, TrendData, Top10Item, Combined50Item, RecommendItem, TrendDelta, TopicCandidate } from "@/lib/api";
...
export default function RadarPage() {
  ...
  const mixed50:     Combined50Item[] = data?.combined_top50    ?? [];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 28 }}>
        <KpiCard label="수집 키워드" value={fmtNum(data?.today)} sub="오늘" color={C.primary} />
      ...
      {(google10.length > 0 || naver10.length > 0) && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
          <div style={{
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: C.primary, marginBottom: 16 }}>
              🔍 Google TOP {google10.length}
            </div>
            {google10.map(item => (
              <div key={item.rank} style={{
                display: "flex", alignItems: "center", gap: 12,
                padding: "8px 0", borderBottom: "1px solid var(--c-bdr)",
              }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: C.primary, minWidth: 24, textAlign: "right" }}>{item.rank}</span>
                <span style={{ fontSize: 16, color: "var(--c-text)" }}>{item.keyword}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: C.success, marginBottom: 16 }}>
              🟢 Naver TOP {naver10.length}
            </div>
            {naver10.map(item => (
              <div key={item.rank} style={{
                display: "flex", alignItems: "center", gap: 12,
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: C.success, minWidth: 24, textAlign: "right" }}>{item.rank}</span>
                <span style={{ fontSize: 16, color: "var(--c-text)" }}>{item.keyword}</span>
              </div>
            ))}
          </div>
      ...
      {mixed50.length > 0 && (
        <div style={{
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: "var(--c-text)", marginBottom: 16 }}>
            혼합 트렌드 TOP {mixed50.length} (Google + Naver)
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ color: "var(--c-text5)", textAlign: "left" }}>
                ...
                <th style={{ paddingBottom: 10, fontWeight: 600 }}>키워드</th>
                <th style={{ paddingBottom: 10, fontWeight: 600 }}>소스</th>
              </tr>
            </thead>
            <tbody>
              {mixed50.map((item, i) => {
                ...
                return (
                  <tr key={`${item.keyword}-${i}`} style={{ borderTop: "1px solid var(--c-bdr)" }}>
                    <td style={{ padding: "7px 0", color: "var(--c-text5)", fontWeight: 700 }}>{i + 1}</td>
                    <td style={{ padding: "7px 10px 7px 0", color: "var(--c-text)" }}>{item.keyword}</td>
                    <td style={{ padding: "7px 0", display: "flex", gap: 6 }}>
                      {hasGoogle && (
                        <span style={{ background: C.primary + "22", color: C.primary, borderRadius: 20, padding: "2px 8px", fontSize: 14, fontWeight: 600 }}>구글</span>
                      )}
                      {hasNaver && (
                        <span style={{ background: C.success + "22", color: C.success, borderRadius: 20, padding: "2px 8px", fontSize: 14, fontWeight: 600 }}>네이버</span>
                      )}
                    </td>
                  </tr>
                );
