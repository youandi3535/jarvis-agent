...
nohup ~/portfolio/jarvis-agent/.venv/bin/python ~/portfolio/jarvis-agent/jarvis_daemon.py \
  > /dev/null 2>&1 &
...
if [ "$COUNT" -eq "1" ]; then
    ...
    echo ""
    echo "📋 최근 로그:"
    tail -8 ~/portfolio/jarvis-agent/logs/daemon.log
else
    ...
    nohup ~/portfolio/jarvis-agent/.venv/bin/python ~/portfolio/jarvis-agent/jarvis_daemon.py \
      > /dev/null 2>&1 &
    sleep 3
    tail -5 ~/portfolio/jarvis-agent/logs/daemon.log
fi
