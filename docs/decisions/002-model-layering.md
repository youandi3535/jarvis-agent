...
| 코드 수정 (auto_repair·pattern_fixer) | Sonnet 4.6 | `code_fix` |
| 아키텍처 진단 (auto_repair 7-Layer) | Sonnet 4.6 | `repair_diag` |
| 자가 학습 평가 (eval_agent — A모델 후) | Sonnet 4.6 | `learn_eval` |
...
2. **정확도 보장**: 코드 수정·진단은 Sonnet 4.6 → 자동 수정 성공률 (현재 ~70%, learned_patterns hit 누적과 함께 상승).
