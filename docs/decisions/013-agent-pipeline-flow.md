...
> **★ 정정 (사용자 박제 2026-07-06 — 자비스09 단순 수집기 재설계):**
> 역할 경계 명확화 — **자비스09 = 수집만**(설계 + 티어순 15개 원시 문서). **fact 추출·차트 숫자
> 추출·종목 설명글 생성은 자비스02(작성기)** 가 받은 docs 로 수행. **자비스06** 은 02가 뽑아 넘긴
> 숫자로 그림만. "09가 매력 대본/종목 산문/추출까지" 하던 혼재를 분리. 수집 총량 = 신뢰 서열
> 15개(논문3·API7·나머지5, 이월 cascade). 아래 원문 흐름은 역사 보존.

- **상태**: 정정 (2026-07-06) / 원 적용 (2026-07-03)
