...
>
> **★ 코드 잔재 제거 (2026-07-29 — 사용자 승인):** 위 폐지는 *선언* 이었고 코드에는
> `collect_research(max_rounds=3)` 인자와 "④ 미충족 시 2라운드 재수집" 흐름 주석이 남아 있었다.
> AST 실측 결과 그 인자는 **본문에서 한 번도 읽히지 않고 재수집 루프도 0개** — 선언·인자·주석
> 셋이 서로 다른 말을 하는 상태였다. 인자와 주석을 제거해 **셋을 '안 한다' 로 통일**했다.
> 부족한 자료는 재수집이 아니라 `prepublish_gate` 의 `data_insufficient` 즉시 abort 로 처리한다.
> ※ **커버리지 *측정* 은 유지** — `evidence_pack` 이 `pack["coverage"]` 에 담고 로그로 남긴다.
...
### 1) JARVIS09 — 설계 → 수집 → 측정 (재수집 순환은 폐지)
...
| 수집 순환 | `collector_engine.collect_research` | 광역 스윕 ∥ 질문별 조준 수집 → 얇은 문서 전문 딥페치 → 팩 추출 → ~~미충족 질문만 2라운드 재수집~~ **(재수집 순환은 폐지 — 상단 고지 참조. 측정까지만 하고 한 번에 끝낸다)** |
