...
## ★★★ 모든 수정에 예외 없이 적용하는 3원칙 (사용자 박제 2026-07-22)

**사용자가 매번 말하지 않아도 적용한다. 말하게 만들면 그 자체가 규정 위반이다.**

| 원칙 | 뜻 | 위반 신호 |
|------|-----|----------|
| **① 단일 진입점** | 한 가지 일은 한 곳에서만 한다. 고칠 때 2곳 이상 손대야 하면 그게 분산 신호다 | 같은 로직·같은 상수가 두 파일에 |
| **② 동적 설계** | 값·목록·스키마는 *런타임 조회로 파생*. 문자열·숫자로 박지 않는다 | 코드에 박힌 숫자·목록·경로 |
| **③ 모든 글에 적용** | 네이버·티스토리 / 경제·테마 **4조합 전부**. 한쪽만 고치면 다른 쪽에서 재발 | 한 플랫폼·한 글종류만 수정 |

### 수정 전 — 3가지를 먼저 답한다
1. **이 로직의 주인은 어디인가?** 주인이 아닌 곳에 추가하려 하고 있지 않은가.
2. **내가 지금 박으려는 값이 이미 어딘가에 있는가?** 있으면 파생, 없으면 신설 후 파생.
3. **이 결함이 다른 플랫폼·다른 글종류에도 있는가?** 반드시 확인 후 함께 고친다.

### 수정 후 — 반드시 확인한다
- **같은 값이 두 곳에 남지 않았는가** (`grep` 으로 실제 확인)
- **정책이 *모든 통로* 에 걸렸는가** — 우회 경로가 남으면 정책은 새어나간다
  (실례 [474]: 발행 우선 규칙을 `invoke_text` 에만 걸어 `run_sdk_query` 로 우회됨)
- **프로세스 경계를 넘는가** — `threading.Event`·메모리 집합은 *같은 프로세스만* 방어한다
  (실례 [474]: 경제 브리핑은 subprocess 라 데몬이 발행 중인 줄 몰랐다)
- **효과를 동작으로 확인했는가** — 코드 존재는 적용의 증거가 아니다 (`patch_effective()` 표준)

### 왜 이 3개인가 — 실제 사고 이력
- ①위반 → [467] 채점 기준이 채점함수·프롬프트 두 곳에 박혀 서로 어긋남 (지시대로 써도 감점)
- ②위반 → [463] 하드코딩된 기준이 실제 채점과 드리프트 / [456] 스키마 복사본이 원본과 어긋남
- ③위반 → [468] 티스토리가 네이버 기준으로 채점됨(C-T 20점이 죽은 코드) /
  [472] 네이버만 고치고 티스토리 방치 / [474] 테마만 커버되고 경제(subprocess) 누락

### 자동 강제
```bash
python3 shared/precommit_check.py --category ssot       # ① 단일 진입점
python3 shared/precommit_check.py --category copytruth  # ② 복사본을 진실로 믿지 말 것
```
자동 검사로 못 잡는 ③(모든 글 적용)은 **수정 후 4조합 전수 확인이 의무**.
...
- **★ 오류 발생 시 첫 행동 — 통독하지 말고 *조준 검색* 하라 (개정 2026-07-27, ERRORS [534])**:
  ...
  print(incidents_brief("<증상·오류 메시지·traceback>", top_k=3))
  ```
  ERRORS.md 는 **9,855줄(1.2MB)·사고 477건·헛다리 373건** 이다. 통독은 두 가지로 실패한다 —
  ① 컨텍스트를 다 먹고 ② **긴 입력 자체가 정확도를 떨어뜨린다**(Context Rot: 프론티어 모델
  18종 전수에서 예외 없이 확인, 200K 창 모델도 50K 부터 저하. 게다가 *양식이 통일된 조밀한
  문서일수록* 주의를 더 갉아먹는다 — ERRORS.md 가 정확히 그 형태다).
  종전 규정을 `head -60` 으로 이행하던 Tier-2 는 **전체의 0.6%(최신 1건)** 만 읽고 있었다.
  → `search_incidents()` / `incidents_brief()` 가 하이브리드(키워드+시맨틱)로 조준한다.
  **⛔ 검색 결과의 '헛다리' 항목은 절대 다시 시도 금지** — 이미 실측으로 기각된 가설이다.
...
- **무한 루프 방지**: `max_attempts` 박제 (**기본 2회** — ★ 사용자 박제 2026-07-21: 어떤 재시도도 최대 2회. 2026-07-06 '3회 통일' 폐지). **단일 진실 소스 = `JARVIS00_INFRA/harness.DEFAULT_MAX_ATTEMPTS`** — 호출자는 `max_attempts=` 하드코딩 금지, 미지정으로 상속할 것. LLM 계층 재시도(`shared/llm`)도 이 상수에서 파생. 무배포 조정: `HARNESS_MAX_ATTEMPTS`. max 도달 시 GUARDIAN escalation + 사용자 텔레그램 + *송출 절대 안 함*.
