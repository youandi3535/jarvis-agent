...
| `JARVIS07_GUARDIAN/auto_repair.py` | 전체 코드 검토·수정 (새벽 04:30 `job_deep_audit`, Opus 4.6) | claude-code-sdk query |
...
| `JARVIS07_GUARDIAN/eval_agent.py` ★ | **A모델 분리** — 수정 결과 평가 + learned_patterns 등록 게이트 (정적 자동/llm Opus 4.6) |
...
게이트 의무: `pattern_fixer.record_pattern_hit()` 는 *반드시* `eval_agent.evaluate()` 통과 후만 등록. 정적 fixer 5종은 자동 통과(score=95, tier=static), `llm_patch` 는 Opus 4.6 (learn_eval alias) 으로 안전성·정확성·재사용 가치 3축 채점 후 80+ 만 통과. 거부 시 텔레그램 알림.
...
- **심층 감사 모델**: Opus 4.6 (`auto_repair._MODEL = "claude-opus-4-6"` — ★ 사용자 박제: 수정하는 건 무엇이든 Opus 4.6. ERRORS [184]: 정확한 모델 ID 명시, alias 금지)
