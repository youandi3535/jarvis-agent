...
| 3-tier 자가 학습 (캐시 → 정적 패턴 → LLM) | [ADR 005](docs/decisions/005-three-tier-learning.md) |
...
auto_repair 가 매일 2회 (07:00 경제 브리핑 세트 · 16:00 테마글 세트) 전체 코드 점검·수정 + 누락 시 보강 안내. 데몬도 다중 `_agent.py` 지원 (한 폴더에 여러 agent 파일 OK — 예: JARVIS07 의 eval + guardian).
...
| `JARVIS07_GUARDIAN/auto_repair.py` | 전체 코드 검토·수정 (07:00·16:00 발행 callback 선행, Opus 4.6) | claude-code-sdk query |
...
| `shared/jarvis.sqlite` | 루트 | 실제 DB 파일 (자동 생성) |
...
- **정책 본문**: `JARVIS02_WRITER/BLOG_SUPREME_LAW.md` 제8조(분량 **25문장 + 이미지 8+α 동적**, ★ 사용자 박제 2026-05-17) + **제8-B조(분량 표기 표준 — 1문장 ≈ 약 50자)** 단일 진실 소스.
...
- **검증 명령** (3종 — 모두 0행):
  ```bash
  # ① 발행 후 이미지 연속 (제4조 위반)
  python3 -c "import sqlite3,re; con=sqlite3.connect('shared/jarvis.sqlite'); rows=con.execute('SELECT id, original_html FROM post_analysis ORDER BY id DESC LIMIT 5').fetchall(); pat=re.compile(r'</figure>\s*(<[^/][^>]*>\s*)*<figure'); [print(f'⚠️ ID={r[0]}') for r in rows if r[1] and pat.search(r[1])]"
  # ② 빈 헤더 (제3조 위반)
  python3 -c "import sqlite3,re; con=sqlite3.connect('shared/jarvis.sqlite'); [print(r[0], h) for r in con.execute('SELECT id,original_html FROM post_analysis ORDER BY id DESC LIMIT 5') for h in re.findall(r'<h[1-6][^>]*>(.+?)</h[1-6]>\s*(?:<[^>]+>\s*)*(?:</?[^h][^>]*>)?', r[1] or '') if not h.strip()]"
...
- **AI 사진 폴백 체인 (고정 순서 — 변경 금지)**: Bing Image Creator(1순위) → HuggingFace FLUX(2순위) → Pollinations.ai(3순위). 모든 `generate_photo()` 호출은 이 체인을 자동 사용.
...
from JARVIS09_COLLECTOR import (
    collect_for_theme,      # 주제 관련 텍스트 자료 (뉴스·블로그·학술·금융기사)
...
- **★ 자동 수정 범위 — 학습 캐시 + 패턴 + LLM 폴백 (3-tier)**:
  1. **학습 캐시** (`JARVIS07_GUARDIAN/learned_patterns.json`) — 과거 수정 사례 fingerprint 매칭. `_fix_from_learned` 가 `_PATTERN_FIXERS` 최우선. 매칭 시 LLM 호출 0 + hit_count 자동 누적.
  2. **정적 패턴** (`pattern_fixer.py` 5종): 상대 import / NoneType slicing / NameError 오타 / NoneType attribute / ImportError 유사 심볼.
  3. **LLM 폴백** (`error_analyzer.py`) — 위 두 layer 모두 실패 시만.
...
| 파일 | 역할 |
|------|------|
| `JARVIS07_GUARDIAN/auto_repair.py` | 자가 진단·수정 엔진 (Claude Code SDK Opus 4.6) |
| `JARVIS07_GUARDIAN/eval_agent.py` ★ | **A모델 분리** — 수정 결과 평가 + learned_patterns 등록 게이트 (정적 자동/llm Sonnet 4.6) |
...
| `JARVIS07_GUARDIAN/error_analyzer.py` | 3-tier 분석 (학습 캐시 → 패턴 → LLM) |
...
| 컴포넌트 | 책임 | 주기 |
|---------|------|------|
| `auto_repair` + `pattern_fixer` | 진단·수정 *만* | 일 2회 (07:00·16:00 *통합 callback 1단계* — ★ 사용자 박제 2026-05-18 v2 "하나의 세트") |
| `eval_agent` ★ | 수정 결과 평가 + learned_patterns 등록 *게이트* | 수정마다 즉시 |
| `auditor` ★ | 헌법 위반·드리프트 검출 + Refine Rules 제안 | 주 1회 (일 04:30) |

게이트 의무: `pattern_fixer.record_pattern_hit()` 는 *반드시* `eval_agent.evaluate()` 통과 후만 등록. 정적 fixer 5종은 자동 통과(score=95, tier=static), `llm_patch` 는 Sonnet 4.6 (learn_eval alias) 으로 안전성·정확성·재사용 가치 3축 채점 후 80+ 만 통과. 거부 시 텔레그램 알림.

### 학습 엔진 3계층

#### 계층 1 — 자가 진단·수정 (JARVIS07_GUARDIAN/auto_repair.py — *발행과 하나의 세트*)
- **스케줄**: 별도 cron 없음. 발행 잡 (`j01_economic_post` 07:00 / `j01_theme_post_16` 16:00) callback 안에서 *순차 실행*.
  - Callback: `JARVIS02_WRITER.scheduler.run_self_repair_then_economic` / `run_self_repair_then_theme`
  - 흐름: ① 쿠키 점검 (티스토리·네이버) → 실패 시 발행 건너뜀 ② auto_repair 전체 코드 검토·수정 (max 15분) → ③ 코드 변경 발생 시 "데몬 재시작 권장" TG 알림 → ④ 발행 진입 (자가진단 실패해도 발행 계속)
- **즉시 반영 vs 데몬 재시작**: 코드 수정은 Python import 캐시 때문에 *현재 데몬 프로세스 무효* → 다음 데몬 재시작 후 발효.
- **모델**: Sonnet 4.6 (`--model claude-sonnet-4-6` 명시 — ★ 2026-05-29 ERRORS [184] 박제. 옛 `--model sonnet` alias 는 CLI 가 1M context 변형 자동 선택 → "Usage credits required" 유발)
...
**부분품 구성**: 광의 비전 = `JARVIS00_INFRA/{preflight.py, harness.py}` (흐름 *골격*) + `JARVIS07_GUARDIAN/` (학습 *두뇌*). 두 부분품은 *수직 협력*: harness 의 Layer 3 검증 실패 시 `error_collector.report()` 자동 호출 → GUARDIAN 3-Tier 수정 → 재실행. *직접 GUARDIAN 호출 금지* — harness 가 자동 위임.
