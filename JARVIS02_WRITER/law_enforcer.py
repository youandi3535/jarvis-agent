...
def _extract_claims(html: str) -> list[dict]:
    ...
    from shared.llm import invoke_text
    prompt = (
        "다음 블로그 본문에서 *틀리면 독자를 오도하는, 구체적이고 검증 가능한 사실 주장* 만 추출하라.\n"
        "추출 대상: 구체 수치·통계(예: 영업이익 12조원, 35% 증가), 특정 날짜·기간, "
        "특정 사건·계약·실적 같은 검증 가능한 구체 사실.\n"
        "★ 추출하지 말 것(차단 대상 아님): 널리 알려진 일반 배경지식(예: '삼성은 대기업이다'), "
        "주관적 감상·전망·해석·분석 의견(예: '강세를 보였다', '전략을 취하고 있다', '안정적 기반을 제공한다'), "
        "두루뭉술한 서술. — 블로그엔 합리적 분석이 정상적으로 들어가므로 이런 건 제외한다.\n"
        "각 주장을 JSON 배열로 반환: "
        '[{"text":"주장 원문(본문에서 그대로)","type":"numeric|date|specific_fact","key":true/false}]. '
...
def _ground_unsupported(claims: list[dict], corpus: str) -> list[str]:
    ...
    prompt = (
        "아래 [주장 목록] 중 *발행하면 안 되는 것* 만 골라라. 차단 기준은 둘 중 하나:\n"
        "(a) [출처]의 사실과 *모순*된다 (출처와 다른 수치·반대되는 사실).\n"
        "(b) 구체적·검증 가능한 사실인데 [출처]에 근거가 전혀 없고 상식으로도 추론 불가하다.\n"
        "★ 다음은 차단하지 마라(제외): 널리 알려진 일반 배경지식, 합리적 분석·전망·해석, "
        "출처에서 자연스럽게 추론되는 내용. *단지 '출처에 안 적혀 있다'는 이유만으로는 차단 금지* "
        "— 블로그엔 검증되지 않았지만 합리적인 서술이 정상적으로 들어간다.\n"
