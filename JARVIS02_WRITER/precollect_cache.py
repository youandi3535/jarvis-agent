...
무거운 fact·chart 추출 LLM 을 *저부하 창*(경제 06:05 / 테마 20:30)에서 미리 수행해 캐시하고,
발행창(경제 06:30 / 테마 21:00)은 캐시를 재사용한다 → 발행창 내 추출 LLM 0회 → 직후 writer 가
버스트로 열화되지 않은 Max 풀에서 실행(300s 스톨 조건 제거). "수집 데이터 전부 활용" 박제는
...
# 같은 발행 사이클 안에서만 유효(경제 06:05→06:30 25분·테마 20:30→21:00 30분). 하루 넘긴 잔재 무효.
_TTL_SEC = 6 * 3600
...
def pin_theme(theme: str) -> bool:
    ...
    선계산 잡(20:30)이 고른 테마를 마커로 고정해 발행(21:00)이 *같은 테마*를 쓰게 한다
