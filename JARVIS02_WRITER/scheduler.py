...
def run_self_repair_then_economic():
    ...
    *하나의 세트*: 자가진단 → 쿠키 점검(네이버만) → 경제 브리핑 발행. 시퀀스 보장.
    ...
    # ★ 네이버 쿠키는 *네이버 액션의* 전제조건이지 **세트 전체의 전제조건이 아니다** (2026-08-10).
    #   종전엔 여기서 `return` 해 티스토리까지 통째로 죽였다 — 08-08 21:00·08-09 07:00·
    #   08-09 21:00 세 슬롯이 전부 `발행 결손 2건: naver·tistory` 였던 이유다.
    #   대조군: 08-10 07:00 은 이 게이트를 통과해 네이버만 실패하고 티스토리는 발행됐다.
    #   CLAUDE_WRITER.md 의 "플랫폼 단위 끝까지 직렬 … 실패 격리" 는 harness 액션 *안* 에만
    #   걸려 있었고, 이 게이트는 harness 진입 *이전* 이라 원칙이 닿지 않았다.
    #   준비·네트워크 재시도·실패 박제는 그대로 두고 **중단 권한만 회수한다**.
    #   ※ 게이트는 pkl 만 보고, 발행자는 프로필 세션(step0)이라는 다른 경로도 가진다 —
    #     게이트 실패가 곧 발행 불가는 아니므로 시도할 가치가 실제로 있다.
    _naver_cookie_ready("경제 브리핑")
