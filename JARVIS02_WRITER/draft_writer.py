...
from shared.llm import writer_timeout as _writer_timeout
...
def _build_section_system_msg(supreme_block: str, platform: str) -> str:
    spec = PLATFORM_SPEC.get(platform, PLATFORM_SPEC["tistory"])
    # ★ 학습 지침 주입 (2026-08-08 — 사용자 지시 감사)
    #   이 함수는 폴백 경로 4곳(`_gen_section_call1~3` · `_gen_economic_ts_nv_parallel`)의
    #   **공통 system 조립 지점** 이다. 그런데 여기엔 지침이 없었다 —
    #   주 경로(662·1147행)만 `_load_learn_insights` 를 부르고 있었다.
    #   즉 **주 경로가 실패해 폴백으로 떨어지면 그 글은 학습 0 상태로 발행** 됐고,
    #   그걸 막는 테스트도 없었다. 한 곳에 넣어 넷을 동시에 덮는다(원칙①).
    _insights = _load_learn_insights("economic", platform)
...
def _gen_economic_ts_nv_parallel(
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            call1_content = call1_fut.result(timeout=_writer_timeout())
            call2_content = call2_fut.result(timeout=_writer_timeout())
            call3_content = call3_fut.result(timeout=_writer_timeout())
