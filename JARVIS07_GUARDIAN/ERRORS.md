...
## [554] ✅ 해결 — 판정 실패가 **멀쩡한 대본을 4번 다시 쓰게** 만들었다 (티스토리 미발행) (2026-08-04)

> **범위 주의**: 미발행의 *직접* 원인(`_nonessential` 이 회로 면제를 앞질러 SDK 미호출)은
> 같은 날 커밋 `6fcb1d7` 에서 이미 수정됐다. 이 항목은 **그 실패를 사고로 키운 증폭 경로**와,
> 그것을 쫓다 내가 **두 번 오진한 기록**이다. 오진 기록이 이 항목의 핵심 가치다.

- **증상**: 2026-08-04 21:00 테마 발행. 네이버는 21:26 '제지' 발행, **티스토리 0건.**
- **직접 원인 (기수정 `6fcb1d7`)**: `shared/llm.py` 평가 순서가 `if _nonessential:` → `elif 회로`
  라서, `fact_judge` 가 `_CIRCUIT_EXEMPT_ALIASES`(회로가 열려도 살릴 호출) 소속인데도
  **면제 분기에 도달조차 못 하고 즉시 폴백**했다. 실측: 그날 `fact_judge` 24회 중 **8회가
  `ok=0 · duration_ms=0`** — 0ms 는 실패가 아니라 **SDK 미호출**이다.
- **★ 증폭 경로 (이번에 고친 것) — 판정이 죽자 *대본* 을 다시 썼다**:
  게이트는 판정 불가를 `kind=infra_throttle` 로 내지만 `Issue(step=...)` 에는 *실제 대본 step
  이름* 이 담긴다. 그런데 `harness._find_resume_step` 은 **step 이름만 보고** 재개 지점을 골랐다.
  → 판정기가 안 불렸을 뿐인데 **멀쩡한 대본을 통째로 재생성**했다.
  ```
  21:06:20 writer_long_body 356.5s   21:08:38 fact_judge ❌(0ms, 미호출)
  21:17:17 writer_long_body 338.1s ← 재생성   21:20:49 fact_judge ✅ 86.1s → 21:26 네이버 발행
  21:32:49 writer_long_body 332.0s   21:34:54 fact_judge ❌(0ms)
  21:42:53 writer_long_body 297.4s ← 재생성   21:46:28 fact_judge ❌(0ms) → 재시도 소진 → abort
  ```
  재생성 4회 = **315,217 토큰** (그 창 414,613 의 **76%**), 매회 5~6분. 21:00 작업이 21:47 까지.
  `VERIFY_ONLY` 라는 장치가 *이미 있었고 docstring 이 이 상황을 정확히 서술* 하고 있었는데도
  **한 번도 발화하지 않았다** — 판정 기준이 `kind` 가 아니라 step 이름이었기 때문이다.
- **★★ 헛다리 2건 — 내가 세운 가설이 둘 다 실측에 뒤집혔다 (이 항목의 본론)**:
  ① **"5시간 창 소진"** — `llm_rate_limit_events` 21:00~22:00 **19건 전부 `status=allowed`**.
     토큰을 많이 쓴 것은 *증폭의 결과* 였는데 그걸 원인으로 지목했다.
  ② **"90초 타임아웃"** — 실패 행들의 *간격* 이 정확히 90초라서 "상한 초과"로 읽었다.
     **틀렸다.** `llm_token_usage.ts` 는 `DEFAULT (datetime('now','localtime'))` = **삽입=종료 시각**
     이고, 실패 7건은 전부 `duration_ms=0` 이다. 타임아웃이면 90,000ms 로 찍혔어야 한다.
     90초 간격은 게이트 자체의 재시도 간격이었지 호출 시간이 아니었다.
     → **간격(추론)을 보고 지속시간(사실)을 안 봤다.** 같은 표에 답이 있었다.
  ③ (앞서 기각) **"출력 상한으로 판정을 짧게"** — `MAX_OUTPUT_TOKENS=800` 실측 결과
     3,200 토큰·57초·**JSON 깨짐**. 상한이 응답을 잘라 파싱 불가로 만들었다.
- **해결 (3원칙)**:
  - **①** `harness._find_resume_step` 이 `_is_infra_issue` 를 재생성 사유에서 제외 → `VERIFY_ONLY`.
    재개 판단은 *step 이름이 아니라 `kind`* 로. 판단의 주인은 harness 한 곳.
  - **②** `shared/llm.judge_timeout()` 신설 — `writer_timeout()/3` 파생(→200s),
    `LLM_JUDGE_TIMEOUT_SEC` 로 무배포 조정. 게이트의 리터럴 `90` 2개 제거.
    *이번 사고의 원인은 아니지만* 실측 성공 최대 86.1s 대비 상한 90s 는 여유 **4.3%** 로
    다음 사고 후보였다. 죽은 `max_tokens=600` 선언도 함께 제거.
  - **①(구조)** `_nonessential` 분기가 `_CIRCUIT_EXEMPT_ALIASES`·`_essential` 를 존중하도록 교정
    + 즉시 폴백 시 사유 로그. `6fcb1d7` 은 *호출부* 를 `_essential=True` 로 바꿨고, 이쪽은
    *분기 자체* 를 고친다 — 미래의 다른 호출자가 같은 함정에 빠지지 않는다(이중 방어).
  - **③** 4조합 전부 같은 경로다. `prepublish_gate` 는 경제·테마 × 네이버·티스토리 공용,
    `_find_resume_step` 은 harness 단일 진입점 — 한 곳 고침이 넷을 덮는다(구조로 보장).
- **검증**: 재현 4종(실사고→`VERIFY_ONLY` / 진짜 사실성·매력도 결함→재생성 유지 / 혼재→진짜 결함
  우선) 통과. 골든 테스트 회귀 4종 신설 · precommit 60종 0건 ·
  재시작(PID 27749, 22:59:52) 후 **데몬이 로드한 코드로** 재확인.
- **파일**: `JARVIS00_INFRA/harness.py` · `shared/llm.py` · `JARVIS02_WRITER/prepublish_gate.py` ·
  `tests/test_publish_golden.py`
- **교훈**:
  ① **간격을 보고 지속시간을 추론하지 마라 — 지속시간 컬럼이 바로 옆에 있었다.**
     `duration_ms=0` 한 칸이 "타임아웃" 과 "미호출" 을 가르는데, 나는 timestamp 뺄셈으로
     그럴듯한 이야기를 만들었다. **그럴듯함은 증거가 아니다.**
  ② **타임스탬프의 의미(시작/종료)를 확인하기 전엔 어떤 타임라인도 신뢰하지 마라.**
     `ts` 가 종료 시각임을 안 순간 내 해석 전체가 뒤집혔다. 스키마 한 줄이 결론을 바꾼다.
  ③ **증상의 크기(76% 토큰)를 원인으로 착각하지 마라.** 가장 큰 숫자가 가장 깊은 원인은 아니다.
     재생성은 *결과* 였고 원인은 그 앞의 0ms 였다.
  ④ **실패의 *사유* 와 실패한 *지점* 은 다른 축이다.** 둘을 한 필드(step)로 판정하면
     "판정기가 아파서 글을 다시 쓰는" 일이 생긴다. 장치가 있어도 판정 기준이 틀리면 발화하지
     않는다 — **코드 존재는 적용의 증거가 아니다**([457] 재확인).
  ⑤ **비싼 재시도는 재시도가 아니라 증폭기다.** 판정 실패(0초)가 대본 재생성(6분·8만 토큰)을
     불렀다. 재시도 비용이 원인 비용보다 크면 그 재시도는 복구가 아니라 2차 사고다.
  ⑥ **한 저장소를 두 세션이 동시에 고치면 진단도 커밋도 엇갈린다.** 이 사고는 `6fcb1d7`(타 세션)과
     본 항목이 *같은 증상에 다른 원인* 을 짚었고, 그쪽이 옳았다. 고치기 전에 `git log` 를 볼 것.

---

## [553] ✅ 해결 — 내가 만든 '지침 준수 게이트' 가 네이버 발행을 통째로 막았다 (2026-08-04)

- **증상**: 2026-08-04 07:00 경제 브리핑. 네이버가 **4회 시도 전부 차단**돼 발행 0건.
  티스토리만 나갔다(07:36, 73.0점). 09:20 결손 감사가 `PublishGapEconomicNaver` 1건 박제.
- **★ 결정적 사실 — 품질은 네 번 다 통과했다**:
  ```
  네이버 루브릭 점수: 72.0 ✅ · 79.0 ✅ · 75.5 ✅ · 78.0 ✅   (임계 70)
  ```
  즉 **합의된 품질 기준을 넘긴 글을 내가 만든 다른 기준이 막았다.**
- **원인 (원칙① 위반 — 내가 어제 넣었다)**: [552] 후속으로 2026-08-03 에
  "주입한 지침을 지켰는지 아무도 안 본다" 며 `prepublish_gate` 에 **C축(지침 준수)** 를
  넣고 위반을 `kind="engagement"` Issue 로 냈다. 그 kind 는 재작성 사유라
  harness 가 2회 재생성 후 `abort` → 송출 차단.
- **★ 진단 자체가 틀렸다 (비직관 — 이게 핵심)**:
  주입 지침 8건을 100점 루브릭 항목과 대조하니 **8건 전부 이미 채점되고 있었다**.
  | 지침 | 루브릭 항목 |
  |---|---|
  | H3 소제목 3~4개 | `N3_h3_count`(3) + `N4_section_sents`(2) |
  | 도입부 4문장·핵심 수치 | `B1_intro`(6) |
  | H1 정확히 하나 | `T3_h1_count`(2) |
  | 내부 링크 1개 | `T8`(2) |
  | 면책·여백·롱테일·독창성·실용성 | 전부 존재 |
  **검사는 이미 되고 있었다. 다만 *감점* 이지 *차단* 이 아니었고, 그게 정상 설계였다.**
  나는 "검사를 안 한다" 로 오진하고, 품질 기준 옆에 **더 엄격한 두 번째 기준**을 세웠다.
- **왜 어제 검증에서 안 잡혔나**: 골든 테스트는 *게이트가 존재하는가* 만 봤다
  (`"[학습지침]" in gate_src`). **그 게이트가 정상 글을 막는지는 보지 않았다** —
  [552] 에서 "성공 경로만 검증했다" 고 반성해 놓고, 이번엔 *차단 경로만* 검증했다.
  양쪽을 다 봐야 한다는 교훈을 절반만 적용한 것이다.
- **해결**: 지침 위반을 **차단 사유에서 제거**. 판정과 로그는 남긴다(관측용).
  지침 미준수의 결과는 어차피 루브릭 점수로 내려가고, 그 감점 항목이 다음 회차
  개선 제안이 된다 — **폐쇄루프는 그 경로로 이미 닫혀 있다.**
  통합 1콜의 C축 자체는 유지(별도 LLM 호출 0, 관측 가치는 있음).
- **검증**: 골든 테스트를 *차단 요구* → **차단 금지** 로 교정하고 변이 검사로 확인 —
  차단을 되살리면 테스트가 잡는다(✅). 48개 통과 · precommit 60종 0건 ·
  재시작(PID 55284, 12:15) 후 차단 경로 부재 확인.
- **파일**: `JARVIS02_WRITER/prepublish_gate.py` · `tests/test_publish_golden.py`
- **교훈**:
  ① **"검사가 없다" 고 말하기 전에 *이미 어디서 검사하는지* 를 세어라.** 8건 전부 루브릭에
     있었다. 없어서 안 된 게 아니라 *다른 형태(감점)로* 되고 있었다 —
     이번 주에만 세 번째다(watchdog 판정 순서 · engagement_judge 회로 면제 · 이번).
  ② **품질 기준은 하나여야 한다.** 임계 70 을 통과한 글을 다른 규칙이 막으면,
     그 시스템의 실제 기준은 아무도 모르는 값이 된다.
  ③ **게이트를 추가할 때는 "정상 글이 막히지 않는가" 를 반드시 함께 시험하라.**
     차단 경로만 시험하면 과잉 차단을 못 본다. 통과 경로 시험이 곧 회귀 방지다.
  ④ **하루 만에 드러난 것은 운이 좋았다.** 발행 시각이 계약인 시스템이라 즉시 표면화됐다 —
     감시가 아니라 *운영 산출물* 이 다시 한 번 가장 빠른 검증이었다([552] 교훈 재확인).

---

## [552] ✅ 해결 — 내가 어제 넣은 감사·태그 코드가 실패할 때만 침묵했다 (자기결함 7건) (2026-07-29)

- **발단**: [550] 수정(1·2·6위)을 커밋한 뒤, 잔여 17건을 *수정 반영 후 기준* 으로 재검증하는
  워크플로(20 에이전트)를 돌렸다. 잔여 17건은 **하나도 해소되지 않았고**(전부 still_real),
  대신 **내가 방금 넣은 코드에서 신규 결함 13건** 이 나왔다. 재순위 1·4·5·7위가 전부 그것이었다.
- **★ 근본 원인 (하나다)**: **성공 경로만 검증하고 "완료" 를 보고했다.**
  [550] 에서 나는 "결손 0건 정상" 을 확인하고 끝냈다. **결손이 있는 경로를 한 번도
  통과시켜 보지 않았다.** 그래서 아래 ①이 운영에 나갔다.

### ■ 자기결함 ① — 결손 분기가 TypeError 로 죽었다 (critical)
- `report("publish", RuntimeError(...), error_type=...)` 를 불렀는데 `error_collector.report`
  에 **그런 인자가 없다**. 실측 `inspect.signature(report).bind(...)` → TypeError.
  루프가 try 밖이고 텔레그램 경보는 그 **뒤에** 있었다.
  → **결손을 알리려고 만든 코드가, 결손이 났을 때만 침묵했다.**
- 해결: 첫 인자에 파생 타입 문자열(`report:51` 이 `error_type = str(exc_or_type)`).
  루프를 try 로 감싸 박제 실패가 사용자 경보를 죽이지 못하게.

### ■ 자기결함 ② — 달력 날짜 판정 (연속 장애일수록 탐지가 꺼짐) (high)
- `WHERE date(created_at)=오늘` — 21:00 테마가 자정을 넘겨 끝나면(실측 07-21 00:51·01:06
  = 07-20 슬롯 산출물) ⓐ 그날은 결손 오신고 ⓑ 다음날엔 그 두 행이 '오늘 실적' 으로 세어져
  **진짜 실패가 초록불**. 최악의 방향이다.
- 해결: 판정 단위를 **슬롯 창**(이번 발행 시각 ~ 다음 발행 시각)으로. `current_slot()` /
  `published_in_slot()` 신설, `published_today`·`due_post_types` 폐기.
  자정 넘김과 minute-of-day 랩어라운드가 **구조적으로** 사라진다.

### ■ 자기결함 ③ — 감사 지연 50분 고정 (성공의 32%를 결손으로 오탐) (high)
- 실측 59건 중 **19건(32%)** 이 +50분 초과, 최대 +246분. 폐기한 `log_monitor` 의 위양성을
  *방향만 바꿔 재도입* 할 뻔했다.
- 해결: 리터럴 폐기 → `misfire_grace_time + 플랫폼수 × BLOG_ACTION_DEADLINE_SEC` 파생(140분).
  추가로 **발행 락이 잡혀 있으면 '결손' 이 아니라 '지연'** 으로 분류하고 박제하지 않는다 —
  아직 돌고 있는 것을 실패라 부르면 오탐이 학습까지 오염시킨다.

### ■ 자기결함 ④ — 태그 관문이 양방향으로 틀렸다 (medium)
- 실측 `response_is_tag_shaped('태그1\n태그2\n태그3\n태그4',4)` → **False**
  (한 줄에 하나씩은 태그 목록의 *정상* 형태인데 통째로 폐기 → 폴백 상시화),
  `'죄송하지만 … 만들 수 없습니다'` → **True** (짧은 거부문 통과).
- 폴백도 제목을 그냥 쪼개 `'내'`·`'3'`·`'4'`·`'어디쯤'` 을 공개 태그로 낼 참이었다.
- 해결: 쉼표·줄바꿈 **둘 다 구분자**, 조각 기준 3단 판정(① 태그 한도 2배 초과 조각이 있으면
  산문 혼입 → 전량 폐기 ② 태그다운 조각 2개 미만 → 목록 아님 ③ 태그다운 비율 절반 미만 →
  설명이 주인공). **여전히 어휘 목록은 쓰지 않는다.**
  폴백은 2자 이상·숫자단독 제외 + 긴 낱말 우선, 모자라면 억지로 채우지 않는다.

### ■ ★ 자기결함 ⑤ — "4조합 커버" 라고 보고했는데 실제로는 2조합이었다 (high)
- **21:00 테마 발행 결과를 보고 발견했다.** 네이버 태그가 4개인데 목표는 5개
  (`NAVER_HASHTAG_MIN`). 코드는 5를 내야 하는데 4가 나왔다 → **제3의 태그 경로** 가 있었다.
- 실측: `trend_theme_writer.py:256·302` 가 `tags=_stg([theme, sector, '테마주','주식','투자'])`
  로 **직접 만들어 넘기고** 있었다. 즉 발행자의 `_generate_smart_tags` 를 **아예 타지 않는다**.
  [550] 에서 통합한 것은 발행자 2벌뿐이었고, **테마 2조합은 손도 안 댄 상태**였다.
- 게다가 그 고정 템플릿은 ⓐ BLOG_SUPREME_LAW **제1-B조(고정 풀·고정 템플릿 금지) 위반**
  ⓑ 모든 테마 글이 같은 '테마주·주식·투자' 를 달아 **검색 변별력 0**
  ⓒ 네이버 4개로 `NAVER_HASHTAG_MIN`(5) 미달 → `post_scorer` N7_hashtags 감점.
- 해결: `generate_tags(..., seed_tags=[theme, sector])` 신설 — 테마의 *도메인 지식* 은
  seed 로 앞자리에 살리고 나머지는 LLM 이 채운다. 두 호출부를 이걸로 교체.
  **이제 4조합이 실제로 한 함수를 지난다**(소스 대조로 확인).

### ■ 자기결함 ⑥ — 발행 잡 판별이 3벌로 늘었다 (medium)
- `job_llm_priority.is_publish_callback()` 이 주석에 *"단일 진실 소스"* 라고 박혀 있는데,
  `job_registry:275` 가 문자열을 다시 검사하고 있었고 **내가 `publish_ledger` 에 세 번째
  사본**(`_PUBLISH_CALLBACK_MARK`)을 추가했다.
- 해결: `publish_post_type()` 을 마커 소유자에 신설, 나머지 둘은 그것만 부른다.
  코드에서 마커 문자열 잔존 **0**(콜백 경로·정의부 제외).

### ■ 자기결함 ⑦ — watchdog 판정 순서 (low)
- 종전 순서: `JARVIS_KILLABLE_SUBPROCESS` → `JARVIS_NO_WATCHDOG_KILL` → 데몬/keeper 보호.
  env 는 **모든 자손에 상속** 되므로 [550] 이 심은 표식을 손자가 물려받으면
  *명시적 금지와 데몬 보호보다 표식이 앞선다*.
- 해결: 순서 반전 — **금지가 항상 허용을 이긴다.** 5종 전수 확인
  (데몬·keeper 표식 상속 False / 명시 금지 우선 False / 테마·경제 True).

### ■ 검증
- ①: `sig.bind()` 성공 — 결손 분기 생존.
- ②: 07-20 23:20 감사 → 슬롯 `[07-20 21:00 ~ 07-21 07:00]`, 00:51·01:06 글이 제 슬롯 실적으로
  잡혀 결손 0. 07-21 감사는 중복 계상하지 않음.
- ③: 파생 140분 → 감사 잡 09:20·23:20 재파생.
- ④: 관문 6종 전수(정상 3종 수용 / 거부문 2종·빈응답 폐기), 폴백에서 `'내'`·`'3'`·`'4'` 제거.
- ⑤: 4조합 태그 경로 소스 대조 — 테마×2 는 `generate_tags` 직접, 경제×2 는 발행자 shim 경유.
- ⑥: 마커 문자열 코드 잔존 0. ⑦: 5종 전수 통과.
- precommit 60종 0건 · pytest 19 통과 · 잡 40개 콜백 실패 0 ·
  재시작(PID 17517, 22:06:54) `:9198` `:9199` `:8505` 200 ·
  **정상종료 경로 재확인**(시그널수신 2→3, 종료완료 155→156) ·
  현재 슬롯 감사 실행 `theme [07-29 21:00~07-30 07:00] 결손 없음`.
- **실전**: 2026-07-29 21:00 테마 양 플랫폼 발행 성공(naver 21:12 · tistory 21:23, 점수 62.0),
  태그 `['스마트카SMARTCAR','테마주','주식','투자']` — 사고 없음.
- **파일**: `JARVIS08_PUBLISH/{publish_ledger,tags}.py` · `JARVIS08_PUBLISH/CLAUDE_PUBLISH.md` ·
  `JARVIS02_WRITER/trend_theme_writer.py` · `JARVIS04_SCHEDULER/{job_registry,job_llm_priority}.py` ·
  `JARVIS00_INFRA/watchdog.py`

### ■ 교훈
① **★ 성공 경로만 통과시킨 것은 검증이 아니다.** 감시·경보·폴백 코드는 *실패했을 때* 도는
   코드다. 그 분기를 한 번도 밟지 않고 "고쳤다" 고 말하면, 정확히 필요할 때 침묵한다.
   → 앞으로 감시 코드는 **결함을 1건 주입해 경보까지 도달하는지** 를 검증에 포함한다.
② **감시 코드의 오탐·오음성은 감시 대상보다 위험하다.** 결손 26%를 잡겠다고 만든 것이
   성공의 32%를 결손이라 부를 참이었고, 자정을 넘기면 *연속 장애일수록 탐지가 꺼지는*
   방향이었다. 폐기한 `log_monitor` 의 병을 방향만 바꿔 재발시킬 뻔했다.
③ **★ "통합했다" 는 호출부를 전수로 세어 본 뒤에만 할 수 있는 말이다.** 나는 함수 정의 2곳을
   합치고 4조합을 덮었다고 보고했는데, 테마는 그 함수를 아예 타지 않았다. **정의가 아니라
   *소비* 를 세어야 한다.** 발견 계기가 코드 리뷰가 아니라 *실제 발행 결과의 태그 개수* 였다는
   점도 남긴다 — 운영 산출물을 보는 것이 가장 싼 검증이다.
④ **단일 진실 소스라고 *주석에 적힌* 것을 먼저 찾아라.** `is_publish_callback` 은 자기가
   SSOT 라고 명시하고 있었는데 나는 읽지 않고 세 번째 사본을 만들었다.
⑤ **허용 플래그는 금지 규칙보다 뒤에 평가한다.** env 는 자손에 상속되므로, 허용이 앞서면
   의도하지 않은 프로세스까지 허용된다.

---

## [551] ✅ 해결 — `_port_listeners` 만 PATH prepend 를 빠뜨려 orphan Next.js 가 19시간 방치됨 (2026-07-29)

- **증상**: `jarvis_daemon._watch_streamlit` 가 `RuntimeError: Next.js 3회 실패` 보고(GUARDIAN #4792).
  `logs/next_server.log` 에 `Error: listen EADDRINUSE: address already in use :::9199` 반복.
- **환경**: keeper(launchd, `com.jarvis.keeper.plist` — `EnvironmentVariables` 키 없음 → 최소 PATH)
  가 20:15 데몬을 재기동. 그 직후 `daemon.log`:
  `_port_listeners lsof 오류 (port 9199): [Errno 2] No such file or directory: 'lsof'`.
  실측(`ps -o lstart`) — 그 순간 포트 9199 를 쥐고 있던 `npm run dev`(PID 14614)/`next dev`
  (PID 14639) 는 **당일 새벽 02:21 시작, PPID=1(고아)** — 최소 3회 이상의 데몬 재시작을
  거치는 동안 단 한 번도 정리되지 못하고 살아남아 있었다.
- **원인**: `_port_listeners()`(jarvis_daemon.py) 의 `subprocess.run(["lsof", ...])` 호출에
  `env=_build_env()` 가 빠져 있었다. keeper/launchd 의 최소 PATH 엔 `lsof`(`/usr/sbin/lsof`)
  가 없어 `FileNotFoundError` → `except` 가 삼키고 **빈 리스트 반환** → `_kill_port()` 가
  "죽일 대상 없음"으로 오판 → 이전 세션의 orphan Next.js 를 못 죽인 채 새 `npm run dev` 가
  같은 포트 바인딩 시도 → EADDRINUSE 3연속 → 자동재시작 중단.
  같은 파일의 `_start_api`/`_start_next` 두 `Popen` 호출은 이미 `env=_build_env()` 를 쓰고
  있었는데 **`_port_listeners` 하나만 빠져 있었다** — CLAUDE_INFRA.md 에 4회 반복 박제된
  "launchd/keeper 최소 PATH" 버그의 동일 클래스가 같은 파일 안에서도 새는 지점이었다.
- **헛다리**: 없음 — 과거 유사사고([516] SDK 벽시계 타임아웃, [388] keeper 오분류)는 클래스만
  같고 이 사고와 직접 관련 없어 참조만 하고 채택하지 않음.
- **해결**: ① `_port_listeners()` 의 `subprocess.run` 에 `env=_build_env()` 추가.
  ② `_build_env()` `_EXTRA` 목록에 `/usr/sbin`·`/sbin` 추가(`lsof` 실제 위치 — 종전엔
  `/opt/homebrew/{bin,sbin}`·`/usr/local/bin` 만 있어 시스템 유틸 경로가 빠져 있었다).
  ③ 19시간 방치된 orphan(PID 14614/14639) 수동 정리 후 `restart_daemon.sh` 재기동 →
  `lsof` 정상 동작(경고 0건) + `🧹 이전 Next.js PID N 종료(SIGTERM)` 로그로 자동 정리 확인 +
  FastAPI·Next.js 둘 다 HTTP 200 재확인.
- **파일**: jarvis_daemon.py
- **교훈**: ① 같은 파일 안이라도 "이 subprocess 호출도 PATH 를 prepend 했는가"는 호출마다
  개별 확인해야 한다 — 옆 함수가 고쳐졌다고 전부 고쳐진 게 아니다. ② PATH 실패가 `except` 로
  조용히 삼켜지면 "포트 미사용"으로 오판되어 완전히 다른 증상(EADDRINUSE)으로 몇 단계 떨어져
  나타난다 — WARNING 로그 한 줄이 결정적 단서였다. ③ 포트 점유자를 못 찾으면 그 프로세스는
  데몬의 관리 범위 밖에서 영원히 산다 — 이번엔 최소 19시간·여러 재시작 세션 누적 방치됐다.

---

## [550] ✅ 해결 — 발행의 1/4이 사라지는 동안 계기판은 100% 초록불이었다 (전수 감사 1·2·6위) (2026-07-29)

- **발단**: 사용자 지시 — *"자비스를 전수 조사해서 현업 수준으로 디벨롭 할 것들 중요 순서대로"*.
  12축 병렬 감사 → 적대적 반증 → 순위화(27 에이전트). 발견 113건 중 반증 4건 기각, 최종 20건.
  그중 사용자가 **1·2·6위 선착수** 지시. 아래 셋은 전부 *실측으로 재확인* 후 수정했다.

### ■ 1위 — 발행 결손을 세는 코드가 저장소에 0줄
- **증상(실측)**: 2026-07-12~28, 기대 68건(4조합 × 17일) 중 **결손 18건 → 달성률 73.5%**,
  결손일 9일. 같은 기간 `job_runs` 는 경제 **23/23** · 테마 **14/14** 전부 `success=1`.
- **원인**: 발행 잡은 *자기가 예외 없이 끝까지 돌았는가* 만 안다. *글이 실제로 나갔는가* 는
  **아무도 묻지 않았다.** 둘은 다른 질문인데 앞엣것을 뒤엣것의 답으로 써 왔다.
  감지 장치 실측 — `EVENT_JOB_MISSED` 리스너 **0건** 인데 `daemon.log` 의 `was missed` 는 **101건**.
- **헛다리(기존 코드가 그 역할인 줄 알았던 것)**: `log_monitor.py` 의 잡 2개.
  실제 판정은 *로그 텍스트에 `"네이버"` 와 `"✅"` 가 각각 한 번이라도 있으면 성공* 이었다.
  30KB 로그에서 이모지 하나로 판정하니 10일 표본에서 **위양성 2일·위음성 1일**.
  틀린 초록불은 없느니만 못하다 → **파일째 폐기**(130줄, 소비자는 그 잡 2개뿐이었다).
- **해결**: `JARVIS08_PUBLISH/publish_ledger.py` 신설 — **DB 사실(`post_analysis` 행 존재)** 로 판정.
  기대 집합을 리터럴로 적지 않는다(원칙②):
  ...
  · platform  ← `JARVIS08_PUBLISH/platforms/` 의 `post_to_<플랫폼>` **AST 파생**(import 0 —
    `naver_poster` 를 import 하면 selenium·Quartz 까지 끌려오는데 감사 잡은 발행하지 않는다)
  잡은 쿠키 사전점검과 같은 형태로 **발행 잡 cron 에서 파생**(`_build_publish_audit_jobs`,
  발행 +50분) — 발행 시각을 옮기면 감사도 자동으로 따라간다.
  오류 타입도 파생한다: `publish_gap_error_type('economic','tistory')` → `PublishGapEconomicTistory`
  (ERRORS [547] 세분화 규정 — 중앙 매핑표를 두지 않는다).

### ■ 2위 — LLM 의 거부 산문이 그대로 공개 블로그 태그로 발행됐다 (당일 실사고)
- **증상**: 2026-07-29 07:00 경제 브리핑, `economic_20260729_070015.log:205`
  ```
  ✅ 태그 완료: ['주식', '투자관련키워드제공불가\n\n**설명:** 이 게시글은 "중위소득"이라는
    복지·통계 주제를 다루고 있으며', "종목·투자 테마와는 무관합니다…"]
  📎 https://blog.naver.com/youandi3535/224361270274      ← 공개 게시 완료
  ```
- **원인 2겹**:
  ① **검증 0** — `_raw.split(',')` 결과를 무검사 반환. `shared/seo.sanitize_tags` 는 실존했으나
     두 발행자 **모두 호출하지 않았다**.
  ② **프롬프트가 불가능한 것을 요구** — 네이버 쪽이 *"단독 '주식'·'투자' 금지. 반드시 테마명과
     결합"* 을 **글 종류와 무관하게** 강제했다. '중위소득'(복지·통계) 글에 억지 투자 태그를
     붙이라는 요구라 LLM 이 거부했고 **그 거부문이 태그가 됐다**. 테마글 기준을 경제글에
     그대로 적용한 원칙③ 위반.
- **해결**: `JARVIS08_PUBLISH/tags.py` 신설 — 종전 `_generate_smart_tags` 가 naver·tistory
  **두 벌**(거의 동일)이던 것을 하나로(원칙①). 두 발행자는 2줄 위임 shim 만 남겼다.
  · 프롬프트를 글 종류로 **분기하지 않는다** — 새 플래그를 발행 함수까지 배선하면 그걸 안 넘기는
    세 번째 호출자가 생기는 순간 재발한다. 대신 규칙을 *내용에 적응* 하도록 썼다
    ("종목·투자 글이면 …, 그 외 주제라면 그 주제의 핵심어").
  · 폴백도 고정 접미사('관련주'·'테마주')가 아니라 **제목에서 파생** — 종전 폴백은 경제글에
    `중위소득관련주` 같은 실재하지 않는 검색어를 만들었다.
- **★ 검증을 '금칙어' 가 아니라 '구조' 로 한 이유 (비직관)**: '불가'·'죄송' 같은 거부 표현
  목록을 박으면 **목록에 없는 새 거부문은 그대로 통과** 한다. 거부 어휘는 무한하고 모델을 바꾸면
  또 달라진다 — 목록은 반드시 낡는다. 그래서 *무엇을 말했는지* 가 아니라 **응답이 태그 목록의
  꼴인지** 를 본다. 개행이 있거나 총 길이가 태그 목록으로서 불가능하면 **응답 전체를 버린다**.
  실측 — 오늘 그 거부문(길이 231·개행 O)은 관문①에서 폐기, 정상 응답(42자·24자)은 통과.

### ■ 6위 — 재시작이 발행을 죽였다 (SIGTERM 무시 + 테마 워치독 사각지대 + keeper 오발화)
- **증상(실측)**: `daemon.log` 의 `데몬 v2 시작` **288회** 대 `통합 데몬 종료 완료` **153회**
  → **약 47% 가 정리 없이 끊겼다.** 실사고 07-23 21:06 테마 발행 시작 → 21:20
  `Scheduler has been shut down` → 그날 테마 **0건**.
- **원인 3겹**:
  ① **SIGTERM 핸들러 0** — 파이썬 기본 SIGTERM 은 프로세스를 *즉사* 시킨다. `try/finally` 도
     `atexit` 도 돌지 않는다. 이 데몬의 정리 로직은 **전량** `main()` 의 finally(락 해제·
     스케줄러 shutdown·자식 정리)에 있어 통째로 스킵됐다. `restart_daemon.sh` 의 pkill 이
     보내는 것도 SIGTERM 이라 **재시작할 때마다** 이 경로를 탔다. (keeper 는 이미 등록돼 있었다 —
     데몬만 빠져 있었다.)
  ② **테마만 워치독 사각지대** — `is_killable_subprocess()` 가 argv 의 `--scheduled` 로 판정하는데
     테마 기동에는 그 플래그가 없다. 실측 `트theme argv → False` / `economic argv → True`.
     kill 이 꺼지면 watchdog 은 **감시 자체를 종료** 하므로(`watchdog.py:170`) 테마 발행이
     얼어붙어도 아무도 깨우지 않았다. 원칙③ 위반.
  ③ **★ keeper 가 정상 데몬을 SIGKILL 했다** — `HANG_THRESHOLD = 360` 리터럴에 주석은
     *"6분 = 6 missed beats"*. 그 주석은 heartbeat 가 60초일 때만 참인데 **실제 주기는 180초**
     (`DEFAULT_JOBS` `infra_heartbeat` interval seconds=180) — 즉 **2회 지연이면 SIGKILL**.
     정상 부하로도 두 번 밀리는 일은 흔하고 2026-07-19 에 실제로 두 번 발화했다.
     **이게 ①보다 근본이다** — keeper 는 SIGKILL 을 보내므로(GIL 잠긴 hang 에 SIGTERM 이 무력해서)
     데몬 쪽 정상종료 처리가 **원리적으로 무의미** 해진다.
- **해결**: ① `main()` 진입부에 SIGTERM·SIGINT 핸들러(Event 만 세움 → 메인 루프 `wait()` 가 깨어
  finally 정상 수행) ② `_spawn_publisher` 공통 경로에 `JARVIS_KILLABLE_SUBPROCESS=1` **한 줄**
  (argv 문자열 판정에 의존하면 기동 명령이 바뀔 때 또 어긋난다 — 4조합 동시 해결)
  ③ `HANG_THRESHOLD` 를 `DEFAULT_JOBS` 의 실제 heartbeat 주기 × 허용결측 6회로 **파생**
  (180×6=1080초). 주기를 바꾸면 임계가 자동으로 따라와 **주석이 거짓말할 수 없다**.

### ■ 검증 (전부 *동작* 으로)
- **1위**: 감사 잡 2개 파생 등록 확인(07:50·21:50), 옛 log_monitor 0개, 총 잡 40개 유지.
  07:50 시뮬 → 대상 `['economic']`, 21:50 시뮬 → 대상 `['economic','theme']` + 결손 정확 검출.
- **2위**: 오늘 실제 거부문 재현 → **관문①에서 폐기**(길이 231·개행 O). 정상 응답 2종 통과.
  두 발행자의 `invoke_text` 직접 호출 **각 0건**(중복 제거 확인).
- **6위**: 별도 프로세스로 SIGTERM 실험 — 핸들러 없음 → `READY` 만 출력(finally 미실행) /
  핸들러 있음 → `READY SIGNAL 15 FINALLY_RAN`. **재시작된 데몬으로 실증**:
  `시그널 수신 로그 0건 → 1건`, `정상종료 완료 153 → 154`.
  로그 실물 `20:15:50 📥 시그널 15 수신` → `20:15:51 🛑 종료 완료`.
  워치독 — 표식 후 테마 argv `False → True`, 경제 `True` 유지, 데몬 `False` 유지(회귀 0).
- precommit 60종 0건 · pytest 19 통과 · 잡 40개 콜백 실패 0 ·
  재시작(PID 2897, 20:15:53) `:9198` `:9199` `:8505` 전부 200 · 부팅 후 신규 오류 0.
- **파일**: `JARVIS08_PUBLISH/{publish_ledger,tags}.py`(신설) ·
  `JARVIS08_PUBLISH/platforms/{naver,tistory}_poster.py` · `JARVIS04_SCHEDULER/job_registry.py` ·
  `jarvis_daemon.py` · `jarvis_keeper.py` · `JARVIS02_WRITER/scheduler.py` ·
  `JARVIS02_WRITER/log_monitor.py`(삭제) · `JARVIS00_INFRA/CLAUDE_INFRA.md`

### ■ 교훈
① **"잡이 성공했다" 는 "일이 됐다" 가 아니다.** 이 시스템에서 의미 있는 사실은 *글이 나갔는가*
   하나뿐인데, 그걸 묻는 코드가 없었다. 잡 성공률은 그 질문의 답이 아니라 **다른 질문의 답** 이다.
② **틀린 초록불은 없느니만 못하다.** `log_monitor` 는 있었지만 이모지 grep 판정이라 오답이 섞였고,
   그 존재가 오히려 "감시는 되고 있다" 는 착각을 만들었다. 감시를 붙일 땐 *판정 근거* 를 먼저 본다.
③ **금칙어 목록은 반드시 낡는다.** 거부 어휘를 나열하는 대신 *응답의 꼴* 을 봤다.
   무엇을 말했는지가 아니라 무엇처럼 생겼는지를 보면 어휘가 바뀌어도 관문이 살아 있다.
④ **같은 함수가 두 벌이면 사고는 한쪽에서만 난다.** 태그 생성이 두 발행자에 복사돼 있었고
   프롬프트가 어긋난 쪽에서만 터졌다. 원칙① 위반이 실사고로 이어진 교과서 사례.
⑤ **★ 주석에 계산 결과를 적으면 그 주석이 거짓말한다.** `360 # 6 missed beats` 는 heartbeat 가
   60초일 때만 참이었고, 주기가 180초로 바뀐 뒤에도 주석은 그대로였다 — 그래서 *2회면 SIGKILL* 인
   상태로 돌았다. 계산은 주석이 아니라 **코드로** 하라(`interval × tolerance`).
⑥ **상위 계층이 SIGKILL 을 보내면 하위의 정상종료 처리는 무의미하다.** ①만 고치고 ③을 안 고쳤으면
   핸들러는 달아 놓고 여전히 죽었을 것이다. 방어는 *가장 바깥에서 오는 힘* 부터 확인해야 한다.

---

## [549] ✅ 해결 — 문서가 없는 잡을 있다고 적어 3곳이 서로 다른 말을 했다 (2026-07-29)

- **발단**: 사용자 질문 — *"우리 에이전트 10개는 전부 '자율성 있는 에이전트'라 할 수 있나?"*
  10개 모듈을 실측 대조하다 **무증상 결함 3건** 이 나왔다. 셋 다 *고장은 아니다* —
  코드는 정상 동작한다. 다만 **문서가 없는 것을 있다고 적어** 다음 작업자를 헛수고로 보낸다.
- **증상 ① `scheduler_agent.py` 독스트링**: *"5) JARVIS04 자체 cron 잡(일일 브리핑) 등록"*
  실측 — `register()` 안 `add_job` 호출 **0** · `DEFAULT_JOBS` 에 브리핑 잡 **없음** ·
  `owner='jarvis04_scheduler'` 잡 **0개**.
  일일 브리핑은 잡이 아니라 `briefing.build_briefing_text()` 를 텔레그램·도구가 *요청 시* 부른다.
  → **나 자신이 이 문장에 속아** 잡 목록에서 브리핑 잡을 찾다 없어서 "수집이 죽었나" 로 의심했다.
- **증상 ② `vision_agent.py` 독스트링**: *"JARVIS04 DEFAULT_JOBS 에 1분 주기 수집 잡 등록"*
  실측 — `owner='jarvis05_vision'` 잡 **0개**. 수집은 잡이 아니라 **데몬 스레드** 이고,
  주기도 1분이 아니라 **30초**(`collector.COLLECT_INTERVAL`). *존재·종류·주기 셋 다 틀렸다.*
- **증상 ③ 09 의 갭 재수집 순환이 조용히 죽어 있었다** (★ 본건 핵심)
  `collect_research(..., max_rounds: int = 3)` 인자가 있었으나 **AST 실측: 본문 참조 0회
  (시그니처뿐) · 함수 안 `For`/`While` 0개** — 한 번도 돈 적이 없다.
  ADR 012 는 이미 2026-07-06 에 "갭 재수집 순환 폐지" 를 *선언* 했는데 인자·흐름 주석·본문 표가
  남아 **선언(폐지)·인자(3회)·주석(2라운드) 셋이 서로 다른 말**을 했다.
- **원인**: 폐기를 *선언* 만 하고 *코드 잔재* 를 안 지웠다. 인자는 호출자에게 "이 노브가 있다" 고
  광고한다 — 실제로는 아무 데도 안 닿는데. 문서 드리프트의 전형: **코드는 움직이고 문서는 안 움직인다.**
- **헛다리 (되살리기 검토 → 기각)**: "ADR 이 하라는 걸 코드가 안 하니 되살리자" 가 첫 반응이었다.
  **기각 사유는 ADR 012 자신이 적어 놓았다** — 커버리지 루프가 재수집을 무한정 돌려
  **06:30 발행이 2시간+ 정지**(ERRORS [383][385]). 발행 시각이 계약인 시스템에서
  수집이 발행창을 먹는 구조는 되살릴 값이 없다. 자료 부족은 재수집이 아니라
  `prepublish_gate` 의 `data_insufficient` 즉시 abort 로 처리한다. → **사용자 승인 후 정식 폐기.**
- **해결**:
  ① 독스트링 2건 정정 — "잡 0개(남의 잡만 관리)" · "잡 아니라 30초 데몬 스레드" 로 *실상* 기재.
     JARVIS04 가 자기 잡 0개인 것은 결함이 아니라 *정의* 라는 점도 함께 박제(다음 사람이 또 의심 안 하게).
  ② `max_rounds` 인자 제거 + 흐름 주석의 "④ 미충족 시 2라운드 재수집" 삭제(폐기 사유 명기).
  ③ ADR 012 상단 고지에 '코드 잔재 제거' 추가 + 본문 표 행 취소선 + 섹션 제목을
     "설계 → 수집 → **측정**(재수집 순환은 폐지)" 로 교정 → **셋이 같은 말을 하게** 만듦.
  ※ **커버리지 *측정* 은 그대로 살아 있다** — `evidence_pack._measure_coverage` 가 `pack["coverage"]`
    에 담고 로그로 남긴다. 부족을 *알리는* 것과 부족하면 *다시 도는* 것은 다른 문제이고,
    폐기한 것은 후자뿐이다. (측정까지 지우면 갭이 보이지 않게 된다.)
- **검증 (문서 주장 ↔ 실제 재대조)**: jarvis04 소유 잡 0 · jarvis05 소유 잡 0 ·
  `COLLECT_INTERVAL=30` · 시그니처에 `max_rounds` 없음 — **넷 다 정정된 독스트링과 일치**.
  precommit 60종 0건 · pytest 19 통과 · 잡 40개 콜백 해석 실패 0 ·
  재시작(PID 14585, 02:21:29) `:9198` `:9199` `:8505` 전부 200 ·
  VISION 수집 정상(10/10 online) · 부팅 후 신규 실 오류 0.
- **파일**: `JARVIS04_SCHEDULER/scheduler_agent.py` · `JARVIS05_VISION/vision_agent.py` ·
  `JARVIS09_COLLECTOR/collector_engine.py` · `docs/decisions/012-research-first-pipeline.md`
- **교훈**:
  ① **폐기는 선언이 아니라 삭제다** — ADR 이 "폐지" 라 적어도 인자·주석이 남으면
     다음 사람은 *셋 중 무엇이 진실인지* 알 수 없다. 선언한 날 잔재도 같이 지울 것.
  ② **쓰이지 않는 인자는 거짓 광고다** — `max_rounds=3` 은 "3라운드 돈다" 고 말하지만
     본문이 참조하지 않는다. 예외도 경고도 없이 조용히 무시된다.
  ③ **독스트링은 검증되지 않는다** — 코드는 테스트·import 로 깨지면 티가 나지만
     문장은 틀려도 아무도 안 죽는다. *잡 개수·주기 같은 사실* 은 문서에 적지 말고
     "런타임 조회로 파생" 하거나, 적었으면 **주기적으로 실측 대조** 할 것.
  ④ **없는 것을 있다고 적은 문서는 없느니만 못하다** — 이번엔 그 문장에 속은 게 나였다.
     찾다가 못 찾고 "기능이 죽었나" 로 오진했다. 문서 드리프트의 비용은 *읽는 사람의 시간* 이다.

---

## [548] ✅ 해결 — "동적 설계니까 대시보드도 따라오겠지" 가 절반만 맞았다 (2026-07-29)

- **발단**: 사용자 질문 — *"동적 설계가 되어 있으니 코드가 바뀌면 웹 대시보드도 자동으로 바뀌어 있겠지?"*
  확인해보니 **절반만 맞았다.** 확인 안 했으면 "네 자동입니다" 라고 답할 뻔했다.
- **맞은 절반**: 대시보드·API 에 오류 타입 **하드코딩이 0** 이었다.
  `dashboard/app/errors/page.tsx` 는 API 가 준 `error_category` 를 그대로 렌더하고,
  `/api/errors` 는 `severity.describe_category()` 를 부른다. 사본 없음 — 설계대로.
- **틀린 절반 ①: 파생 함수가 새 타입을 몰랐다.**
  `describe_category` 는 `_CATEGORY_LABELS`(파이썬 *예외 이름* 표)만 본다. [547] 이 만든
  도메인 타입은 거기 없으니 **전부 `기타`** 로 떨어졌다 —
  `HarnessFactuality`(사실성 차단)·`HarnessInfraThrottle`(인프라 스로틀)·`WatchdogFreeze`(정지)가
  화면에서 한 덩어리. **타입만 세분화하고 분류는 그대로면 절반만 고친 것이다.**
  → `_DOMAIN_PREFIX_LABELS` 접두사 파생 신설. 타입마다 표에 줄을 추가하는 방식은
    새 kind 가 생길 때마다 낡는다 — **접두사만 등록하면 세부 타입이 몇 개 늘든 자동 분류**(원칙②).
    `Harness→발행 검증` · `Watchdog→정지 감지` · `Posting→발행 대응` · `DraftFix→대본 즉시수정`.
- **틀린 절반 ②: 부착이 한 곳에만 있었다.**
  `/api/errors` 는 카테고리를 붙이는데 `/api/guardian/stats` 의 `recent` 는 **안 붙였다.**
  대시보드는 두 응답을 **같은 타입**(`ErrorRow`)으로 쓰므로, 같은 오류가 화면에 따라
  `발행 검증(HarnessFactuality)` 로도 `HarnessFactuality` 로도 보였다.
  → `api_server._with_error_category()` 단일 헬퍼로 통일(원칙①). 라벨 자체는 만들지 않는다 —
    `severity.describe_category` 파생(여기서 매핑하면 그게 곧 사본).
- **곁다리**: `ExternalEdit` **975건**·`GitCommit` **2,116건** 이 화면에서 `기타` 였다.
  오류가 아니라 *변경 기록* 인데 오류와 섞여 보인 것. `_policy_types()`(= `error_collector.
  _MANUAL_POLICY_TYPES` 파생)에 해당하면 **`변경 기록`** 으로 라벨링 — 새 목록을 만들지 않았다.
- **검증**: 데몬 재시작 **후** 실제 API 응답 대조 —
  `/api/guardian/stats` 와 `/api/errors` 가 **동일하게** `변경 기록(ExternalEdit)` 반환.
  (재시작 전에는 stats 가 `(없음)` 이었다 — 실행 중 API 는 옛 코드였다.)
  precommit 60종 0건 · `bandit`·`repair_history` selfcheck 위반 0.
- **파일**: `JARVIS07_GUARDIAN/severity.py` · `api_server.py`
- **교훈**:
  ① **"동적 설계니까 자동으로 되겠지" 는 확인 전까지 가설이다** — 사본이 없어도
     *파생 함수가 새 값을 모르면* 조용히 기본값으로 떨어진다. 예외도 경고도 없다.
  ② **표시 로직이 두 엔드포인트에 있으면 반드시 한쪽이 빠진다** — 같은 오류가 화면마다
     다르게 보였다. 부착은 *지나가야만 하는 문* 하나로.
  ③ **분류는 타입마다가 아니라 접두사로 파생하라** — 타입은 앞으로도 늘어난다.
     줄 단위로 표를 관리하면 [547] 이 만든 새 타입처럼 매번 누락된다.
  ④ **API 를 고쳤으면 데몬을 재시작하고 *응답으로* 확인하라** — 코드는 맞았지만
     실행 중 프로세스는 옛 코드였다. 재시작 전 검증은 검증이 아니다.
...
**Tier 1 이 왜 26.9% 밖에 못 잡는가** 를 재고 고쳤다.

- **증상**: 없음. Tier 1 이 조용히 못 잡고 Tier 2(Claude·최대 10분)로 넘길 뿐이었다.
- **★ 먼저 지표부터 틀려 있었다**: 자동 수리 성공률을 재려는데 `error_log.status='resolved'` 185건이
  분모를 오염시키고 있었다. 실물을 보니 **자동 수리가 아니라 사람이 일괄 정리한 것** —
  `false_positive — log scanner pattern fix` 59건, `코드 수정으로 근본 원인 해결 (2026-05-25)` 49건,
  `fixed_file` 이 있는 건 **2/185**, `llm_attempts>0` 은 **0/185**. 이걸 걷어내야 진짜 깔때기가 보인다.
  ```
  자동 수리 대상 108건 (fixed 67 + wontfix 41)
    ├ Tier 1 성공  29건 (26.9%)
    ├ Tier 2 시도  68건 → 성공 38 (55.9%)
    └ Tier 2 생략  11건
    최종 62.0% · 재발률 14.3%
  ```
- **원인**: `_normalize_message` 가 숫자·경로·시각은 정규화하면서 **harness 액션 이름은 그대로 뒀다.**
  harness 오류 메시지는 `[harness:theme-publish-<테마>-naver] ...` 로 시작한다 —
  실측 336회 출현 중 `theme-publish-*` 가 **54종 고유**. "데드라인 초과" 하나가
  고령화/주류업/음원/석유화학… 테마 수만큼 갈라져 **매번 새 지문**이 됐다.
  덤으로 `302s`·`5건`·`12개` 처럼 **숫자에 단위가 붙으면** `\b\d+\b` 가 단어 경계를 못 찾아
  통째로 빠져나갔다(한글·영문 단위 모두 `\w`). "멈춤 302s > 300s" 와 "305s > 300s" 가 다른 지문.
- **★ 헛다리 2건 (실측으로 기각 — 다시 시도 금지)**:
  - **"error_type 이 전부 RuntimeError 라서 매칭이 안 된다"** — 그럴듯했다. harness 소스는
    2026-05~07 **전 기간 100% RuntimeError** 이고 실패 41건 중 34건(83%)이 RuntimeError 다.
    그래서 `context.kind`(abort/factuality/draft_failed/…)를 `error_type` 으로 승격하면
    나아질 거라 봤다. **시뮬레이션 결과 효과 0** (29.8% → 29.8%).
    이유: 지문은 `타입::메시지` 인데 **타입이 전부 같으면 타입은 지문 변별에 기여를 안 한다.**
    갈리는 건 오로지 메시지다. → *증상을 원인으로 읽은 오진.*
  - **"kind 만으로 지문을 만든다"** — 재매칭 91.2% 로 가장 높지만 고유 지문이 **30개**뿐이라
    339건을 뭉갠다. 서로 다른 사고에 남의 패치를 적용하게 된다.
- **해결** (`_normalize_message` 정규식 3줄):
  - 액션 이름의 *주제* 만 `<TOPIC>` 으로. **`theme` 을 박지 않고 `<종류>-publish-<주제>` 규약에서
    파생** — 새 post_type 이 같은 규약을 쓰면 자동으로 따라온다(원칙②).
  - **플랫폼 접미사는 보존** — 네이버/티스토리는 코드 경로가 달라 같은 사고가 아니다.
    ★ 첫 구현이 이걸 놓쳤다: 두 번째 정규식이 **첫 번째 결과를 다시 먹어**(`<TOPIC>-naver`→`<TOPIC>`)
    플랫폼 구분이 사라졌다. `(?!<TOPIC>)` 부정 전방탐색으로 차단.
  - 단위 붙은 숫자(`302s`·`5건`) 정규화 추가.
  - 이름에 변수가 없는 액션(`경제 브리핑 발행 — 네이버` 3종, `auto-repair` 등)은 **건드리지 않는다.**
- **검증** (harness 오류 339건 시간순 leave-one-out — 이전 지문과 같으면 Tier 1 이 LLM 0회로 재적용 가능):
  | 설계 | 재매칭 | 고유 지문 |
  |---|---:|---:|
  | 종전 | 29.8% | 238 |
  | 액션명 정규화 | 46.0% | 183 |
  ...
  **오병합 검사**: 병합된 53그룹 전부 `context.kind` **단일**(섞인 그룹 0/53).
  즉 합쳐진 것들은 진짜 같은 사고다 — "수정 불가 N건 패턴 반복" 13건, "종목 데이터 N개" 13건,
  "[Layer4] 발행 실패" 8건 등.
  자기검사 5종 통과(멱등 · 테마 병합 · 테마 NV/TS 구분 · 경제 NV/TS 구분 · 초 병합).
  기존 학습 패턴 51개 재정규화 시 **오병합 0**(나머지 메시지가 실제로 달라 정상). precommit 60종 0건.
- **파일**: `JARVIS07_GUARDIAN/pattern_fixer.py`
- **교훈**:
  ① **지표의 분모부터 의심하라** — "자동 수리 성공률 12.9%" 는 `resolved`(사람 정리분) 185건이
     섞인 값이었다. 걷어내니 **62.0%** 였다. 5배 차이다.
  ② **가장 눈에 띄는 이상이 원인은 아니다** — "100% RuntimeError" 는 강렬한 신호였고 실제로
     결함이지만(타입 기반 게이트 무력화), *지문 매칭이 안 되던 이유는 아니었다.*
     시뮬레이션이 없었으면 엉뚱한 걸 고치고 "개선했다" 고 적었을 것이다.
  ③ **정규화는 "무엇을 같다고 볼 것인가" 의 선언이다** — 너무 적게 하면 같은 사고가 흩어지고
     (테마명), 너무 많이 하면 다른 사고가 뭉친다(kind 단독). 그 사이를 **오병합 검사**로 잡는다.
  ④ **정규식을 겹쳐 쓸 때 뒤 규칙이 앞 결과를 먹는지 확인할 것** — 플랫폼 구분이 조용히 사라졌고,
     자기검사가 없었으면 못 봤다.

---

## [545] ✅ 해결 — 한국어 어절 토큰이 검색을 죽이고 있었다 (문자 n-gram 전환) (2026-07-28)

[544] 가 2단계(리랭커)를 붙여 개선했지만 병목은 1단계에 남아 있었다. 그 1단계를 고쳤다.

- **증상**: 없음. 검색은 늘 뭔가를 돌려주거나 빈손을 돌려줬다.
- **진단 (실측)**: top-100 밖으로 탈락한 정답 40개 중 **32개(80%)가 키워드 점수 0.000** — 질의 단어가 문서에 **하나도** 안 겹쳤다. 벡터도 약했다(탈락 정답 0.250 < 무관 문서쌍 평균 **0.361**, 즉 정답이 노이즈보다 아래).
- **원인**: 토크나이저가 `[가-힣]{2,}` 로 **어절을 통째로** 잘랐다. 한국어는 교착어라 두 가지가 동시에 깨진다 —
  ① **활용형**: "붙어/붙는다/붙었다" 가 전부 다른 토큰
  ② **어휘 격차**: 사용자는 "사진", 문서는 "이미지" / 사용자는 "붙어", 문서는 "연속 배치"
  ```
  질의:  "사진 두 장이 설명 없이 딱 붙어 있다"
  문서:  "이미지 연속 배치 — 텍스트 없이 차트 두 장 연속 발행"
  겹침:  {없이} 하나뿐 — 그것도 IDF 가 낮은 흔한 말
  ```
- **★ 헛다리 2건 (추측으로 넣을 뻔했다 — 전부 실측으로 기각)**:
  - **슬롯별 청킹 + max-pool** — "사고 하나를 통짜 벡터로 만드니 증상이 희석된다" 는 그럴듯한 가설이었다. 실측 recall@100 **63.6% → 63.6%**, 효과 **0**. n-gram 과 결합하면 오히려 **79.1% → 68.2%** 로 떨어졌다(청킹이 벡터 다리를 흐린다).
  - **RRF(Reciprocal Rank Fusion) 결합** — 업계 표준이고 두 다리의 점수 스케일 차이를 없애준다. 실측 **76.4%** 로 가중합(79.1%)보다 낮았다. 게다가 RRF 는 절대 점수를 순위로 바꿔 **"빈손" 계약을 파괴**한다(임계를 걸 수가 없다).
- **해결**:
  - `_char_ngrams()` 신설 — 문자 2-gram 으로 색인·질의를 **대칭** 처리. 형태소 분석기 설치 0.
    `NGRAM_N=2` 인 이유: 3-gram 은 전 구간에서 낮았다(recall@100 72.7% vs 79.1%). 한국어 어근이 대개 1~2음절이라 2-gram 이 어간을 더 잘 공유한다.
    ※ ERRORS [534] 가 기각한 `t in text` 부분문자열과는 **다른 기법** — 질의·문서를 같은 방식으로 n-gram 화해 *집합 교집합* 으로 세고 IDF 가 흔한 n-gram 을 누른다.
  - 가중치 `0.5/0.5 → **0.7/0.3**` (키워드 쪽). 키워드 다리가 되살아나 최적점이 이동했다.
- **★ 임계 2개가 함께 낡았다 (점수 분포가 바뀌면 임계는 자동으로 낡는다)**:
  - 리랭커 임계 −1.0 → **0.0** (재스윕). 재랭킹 입력도 비교했다 — `_row_text` **전문**이 "제목+증상+원인" 변형보다 전 구간 우월.
  - 1단계 폴백 임계는 0.41 **유지** — 1단계 단독은 어떤 임계에서도 못 쓴다(정밀도 12~37%). 재순위를 못 하는 열화 상태에서는 조용히 있는 게 맞다(계약: 오답 < 빈손).
- **★ 임계 선택에서 한 번 틀렸다 (이게 가장 값진 교훈)**: 처음엔 계약만 보고 임계 **1.0** 을 골랐다 — 정밀도가 41.8% 로 가장 높았으니까. 그런데 그 임계에서 질의 "사진 두 장이 딱 붙어 있다" 가 **빈손**이 됐다([39] 점수 0.04 < 1.0). CLAUDE.md 가 *"[39][103][170][171] 4회 반복 박제"* 로 못 박은 **바로 그 사고군**이다.
  **지표를 올리다 그 지표가 대표하려던 것을 잃었다.** → 0.0 으로 정정.
- **검증**:
  | 지표 | [544] 이전 | [544] 후 | **[545] 후** |
  |---|---:|---:|---:|
  | 1단계 recall@100 | 63.6% | 63.6% | **79.1%** |
  | 최종 recall@5 | 14.5% | 17.3% | **25.5%** |
  | 정밀도 | 16.7% | 47.5% | **33.3%** |
  | 빈손율 | 72% | 64% | **28%** |

  대표 질의 통과 확인(임계 0 에서 [39] ★정답). `golden_check()`·`selfcheck()` 위반 0. precommit 60종 0건.
  골든셋 회귀 기준 상향: `GOLDEN_MIN_RECALL` 0.12→**0.18**, `GOLDEN_MAX_EMPTY` 0.72→**0.40**.
- **⚠️ 남은 것**: 1단계 recall@100 이 **79.1%** — 정답의 20.9% 는 여전히 리랭커에게 도달조차 못 한다. 다음 후보는 **doc2query**(사고마다 "사용자는 이렇게 물어볼 것" 질의를 미리 생성해 색인에 심기 — 진단이 가리키는 어휘 격차의 직접 처방, 색인 1회 비용·검색 0). **단 방법론 주의**: 골든셋도 LLM 이 ERRORS.md 로 만든 것이라 doc2query 와 어휘 분포가 인위적으로 맞을 수 있다 → **손으로 쓴 질의로 교차 검증 필수**.
- **파일**: `JARVIS07_GUARDIAN/repair_history.py`
- **교훈**:
  ① **한국어 검색에 어절 토큰을 쓰지 말 것** — 교착어라 활용형마다 토큰이 갈린다. 형태소 분석기가 없으면 문자 n-gram 이 실용적 대안이고, 설치 비용 0 에 +15.5%p 였다.
  ② **추측한 개선안은 넣기 전에 재라** — 후보 4개 중 **2개(청킹·RRF)가 실측에서 기각**됐다. 둘 다 그럴듯했고 하나는 업계 표준이었다. 안 쟀으면 그냥 넣었을 것이다.
  ③ **점수 체계를 바꾸면 그 위의 임계는 전부 낡는다** — 이번에 임계 2개가 동시에 무효가 됐다. 1단계를 손대면 리랭커 임계 재스윕은 *선택이 아니라 절차*다.
  ④ **총계 지표만 보고 임계를 정하지 말 것** — 반드시 통과해야 하는 대표 질의를 함께 확인하라. 정밀도 8.5%p 를 사려다 플래그십 사례를 잃을 뻔했다.

---

## [544] ✅ 해결 — 조준 검색이 10번 중 7번 빈손이었다 + 어댑터 2개가 LLM 보호를 우회 (2026-07-28)

여섯 건을 한 묶음으로 고쳤다. 공통점은 **전부 무증상** — 예외도 로그도 없이 조용히 틀리고 있었다.

### ① 조준 검색이 사실상 작동하지 않았다 (가장 큰 건)

- **증상**: 없음. `search_incidents()` 는 늘 뭔가를 돌려주거나 빈손을 돌려줬고, 아무도 그게 맞는지 재보지 않았다.
- **실측 (골든셋 36질의 / 정답 110개)**: recall@5 **14.5%** · 빈손율 **72.2%** · 임계 통과분 정밀도 **16.7%**(정답4/오답20).
  즉 CLAUDE.md 가 *"통독하지 말고 조준 검색하라"* 고 규정한 도구가 **10번 중 7번 빈손**이었고, 뭔가 나올 때도 **5번 중 4번이 오답**이었다.
- **대표 실패**: 질의 "이미지가 연달아 붙는다" → 무관한 [534](조준검색 사고)가 0.548 로 1위 통과, 정답 [172] 0.360 / [39][103][170][171] 은 top-6 에도 없음.
  원인은 **자기참조 오염** — [534] 본문이 그 문구를 *예시로 인용* 해 키워드 레그가 만점을 줬다. CLAUDE.md 가 "4회 반복 박제" 로 못 박은 바로 그 사고군을 못 찾고 있었다.
- **헛다리**: "임계값(0.41)이 높아서" 가 아니다. `min_score=0.0` 으로 낮춰도 정답 다수가 **점수 0.0** — top-20 후보에조차 못 든다. 임계가 아니라 **순위** 문제다.
- **해결 (2단계 검색)**:
  - `shared/embeddings.rerank()` 신설 — cross-encoder(`mmarco-mMiniLMv2`, 무료·로컬·CPU). bi-encoder 는 질의와 문서를 *따로* 벡터화해 "단어 겹침" 에 속지만, cross-encoder 는 **붙여서 한 번에** 읽어 "인용이지 이 사고가 아니다" 를 구분한다. LangChain 경유가 아니다 — `sentence_transformers` 직접 로드가 합법인 파일이 여기 하나뿐이라 리랭커도 여기가 주인.
  - `search_incidents` 가 1단계 상위 **100건**(`RERANK_POOL`)을 리랭커에 넘긴다. 풀을 넓힌 이유는 실측 recall@5 14.5% / @20 36.4% / @100 **63.6%** / @200 82.7% — 리랭커는 *후보에 올라온 것만* 재정렬하므로 풀이 곧 상한이다. 100건 재점수 실측 **1.2초**(CPU), 이 함수는 발행 임계경로가 아니다.
  - fail-open — 모델 없거나 실패하면 1단계 결과를 종전 임계로 그대로 쓴다. 킬스위치 `GUARDIAN_RERANK=0`.
- **★ 골든 쿼리셋 신설** (`JARVIS07_GUARDIAN/golden_queries.json`, 36질의/정답 110개):
  `ERRORS.md:91` 이 *"골든 쿼리셋 30개 박제"* 라고 적어놓고 **실물이 없었다**. 없으면 임계를 만지는 순간 "좋아졌다" 를 증명할 수단이 사라진다.
  질의는 **원문 인용이 아닌 자연어 증상 서술** — 원문을 베끼면 그게 곧 자기참조 오염의 재생산이라, 생성 후 *적대적 검사* 로 3단어 이상 겹치면 재작성시켰다.
- **임계 결정 — 스윕 실측**:
  | 임계 | recall@5 | 정밀도 | 빈손 |
  |---|---|---|---|
  | 없음(기준선) | 14.5% | 16.7% | 26/36 |
  | -5 | 21.8% | 34.8% | 11/36 |
  | **-1 (채택)** | **17.3%** | **47.5%** | **23/36** |
  | 0 | 14.5% | 53.3% | 25/36 |
  -1 을 고른 이유: 세 지표가 **모두 기준선 이상**이면서 이 저장소의 계약 *"오답 < 빈손"* 을 -5 보다 잘 지킨다(오답 45→21). 오답은 무관 사고의 **"⛔ 헛다리"** 를 LLM 프롬프트에 주입해 *능동적으로 오도* 하므로 빈손보다 비싸다.
- **검증**: 최초 실패 질의 재실행 — 리랭커 OFF 는 오답 [364] 1건, ON 은 **정답 [39]**. `selfcheck()` 에 `[S5] golden_check()` 레그 신설(recall·빈손율 회귀 감시).
- **⚠️ 부분 개선이지 해결이 아니다**: 진짜 병목은 **1단계 recall** 이다(@100 이 63.6% 라 리랭커 상한 자체가 낮다). 다음 작업 — 질의 확장·키워드 레그 보정·인덱스 텍스트 재구성.

### ② LLM 어댑터 2개가 보호 계층을 통째로 우회

- **증상**: 없음. 호출은 되고 답도 온다.
- **원인**: `ClaudeSDKChatModel._generate`(LangChain) 와 `ClaudeSDKLLM.call`(CrewAI) 이 `invoke_text_result` 가 아니라 **`_run_sdk_sync` 를 직접** 불렀다 → `_bind_alias`(alias 귀속·**alias별 캐시정책**) · 회로차단 · 재시도 · `ok` 계약이 전부 안 걸린다. ERRORS [474]([540]) 와 같은 병 — *"한 통로에만 걸면 나머지로 샌다"*.
- **★ 왜 못 잡았나 (이게 핵심 교훈)**: `alias_selfcheck()` 가 검사하는 통로가 **손으로 적은 이름 두 개**(`invoke_text_result`·`invoke_vision`)뿐이었다. 그 목록이 두 어댑터를 다 놓쳤다.
- **해결**: 목록을 버리고 **AST 파생**으로 교체 — 원시 SDK(`_run_sdk_sync`/`_invoke_sdk_vision`)를 *직접* 부르는 함수를 소스에서 뽑아 전부 검사한다(중첩 함수는 자기 몸통 호출만 세어 팩토리 오탐 방지). 파생으로 바꾸자마자 **보고서가 놓친 CrewAI 누수를 즉시 검출**했다. 두 어댑터를 `invoke_text_result` 경유로 재구성.
  (CrewAI 어댑터는 이후 [543] 에서 프레임워크째 삭제돼 통로 자체가 사라졌다 — 더 강한 조치.)

### ③ 수집 자료 최대 77% 가 fact 추출 전에 버려지고 있었다

- **원인**: 같은 노브의 기본값이 **한 파일에 셋**이었다 — `_docs_block(per_doc_chars=0)` / `_extract_facts(=1200)` / `build_evidence_pack(=900)`. 실제로 걸리는 건 제일 바깥값 900 이라, 바로 위 docstring 이 선언한 *"티어별 자수컷 폐지 — 문서 전문 주입"(사용자 박제 2026-07-17)* 이 **조용히 무효**였다.
- **실측 손실**: 팩당 **35,000~47,000 토큰분**(문서 15개 중 11~12개가 900자 초과, 중앙 1,618~3,449자).
- **해결**: `PER_DOC_CHARS_DEFAULT = 0` 단일 상수로 통합. 비상 축소는 코드가 아니라 env `JARVIS_EVIDENCE_PER_DOC_CHARS` 로만.

### ④ harness state 에 살아있는 Selenium 드라이버 — 직렬화 불가 + 정리 미아

- **원인**: `state` 는 step 사이를 흐르는 dict 인데 거기에 WebDriver 객체가 들어 있었다.
  ① 직렬화 통째 불가(실측 msgpack `TypeError`) ② 액션이 끝나면 state 는 버려지므로 `quit()` 을 부를 주인이 없다.
- **실제 피해**: 경제 브리핑은 티스토리 driver 를 만들어 state 에 넣는데 **소비처가 0**이고 `quit()` 은 *실패 분기에만* 있었다 → **성공할 때마다 Chrome 프로세스가 남았다.**
- **해결**: `JARVIS00_INFRA/resources.py` 신설 — state 엔 `<이름>_key` 문자열만, 핸들은 레지스트리. harness 가 `finally` 에서 `close_scope(action_def.name)` 로 **어떤 경로로 끝나든** 정리(정리를 *지나가야만 하는 문* 으로). 스코프는 `state[ACTION_NAME_KEY]` 에서 파생 — 테마는 액션 이름이 동적이라 박을 수도 없다.
- **회귀 방지**: `precommit --category harness` 의 `harness/live-handle-in-state`. 금지 접미사 목록의 주인은 `resources.LIVE_HANDLE_SUFFIXES` 한 곳(원칙②), 못 읽으면 fail-closed.

### ⑤ 발행 escalation 알림에 누를 게 없었다

- **원인**: `_notify_escalation` 이 *"호스트에서 수동 검토 필요"* 라는 **글만** 보내고 끝났다 — 텔레그램에서 사용자가 할 수 있는 일이 0.
- **해결**: `ActionDefinition.retry_job_id` 신설 → "🔁 지금 다시 실행" / "🗑 이번 회차 보류" 버튼. 4조합(경제·테마 × NV·TS) 전부 배선.
- **★ 비직관 — 대기 딕셔너리를 쓰면 안 된다**: `_PENDING_*` 는 **데몬 메모리**인데 경제 발행은 **subprocess** 다. 서브프로세스가 등록해도 데몬 봇 루프는 못 본다(ERRORS [474] 와 같은 클래스). → 잡 ID 를 **`callback_data` 에 실어** 보낸다(26바이트, 상한 64). 서버측 상태 0.
- **★ "그래도 발행" 버튼은 만들지 않았다** — 헌법(ADR 009) *"결함 있는 결과물은 영원히 송출되지 않는다"*. 재실행은 검증 순환을 **처음부터 다시** 타는 것이다.

### ⑥ 문서 드리프트 3건

`harness.py` docstring "기본 DEFAULT_MAX_ATTEMPTS=3" ↔ 실제 **2** / "1800(30분)" ↔ 실제 `BLOG_ACTION_DEADLINE_SEC` **2400** / `CLAUDE_INFRA.md` 의 `__patch_applied__` 는 **코드 히트 0건**(실제는 `__nv_send_attempted__`). 숫자·이름을 문서에 박는 대신 **상수 참조·확인 명령**으로 교체.

---

- **파일**: `JARVIS07_GUARDIAN/repair_history.py` · `golden_queries.json`(신규) · `shared/embeddings.py` · `shared/llm.py` · `shared/precommit_check.py` · `JARVIS00_INFRA/{resources.py(신규),harness.py,bot.py,CLAUDE_INFRA.md}` · `JARVIS02_WRITER/{economic_poster,trend_theme_writer}.py` · `JARVIS09_COLLECTOR/evidence_pack.py`
- **교훈**:
  ① **손으로 적은 목록을 검사에 쓰지 말 것** — `alias_selfcheck` 의 통로 목록 2개가 어댑터 2개를 다 놓쳤다. AST 파생으로 바꾸자 즉시 잡혔다. *목록을 유지해야 하는 검사는 반드시 낡는다.*
  ② **재보지 않은 도구는 작동한다고 믿지 말 것** — 조준 검색은 도입 후 줄곧 recall 14.5%·빈손 72% 였는데 예외를 안 던져 아무도 몰랐다. 검색·랭킹·추천처럼 *조용히 틀리는* 기능에는 정답이 적힌 시험지(골든셋)가 코드보다 먼저다.
  ③ **기본값이 세 곳에 있으면 제일 바깥 것만 산다** — 안쪽 두 개는 장식이고, 문서는 안쪽을 설명하고 있었다.
  ④ **개선을 해결이라고 부르지 말 것** — 리랭커로 recall 14.5→17.3%, 정밀도 16.7→47.5% 가 됐지만 병목(1단계 recall)은 그대로다. 수치를 그대로 남겨 다음 작업자가 속지 않게 한다.
...
- **환경**: `shared/llm.py` · `JARVIS02_WRITER/draft_writer.py` · `JARVIS07_GUARDIAN/quality_learner.py`. 경제·테마 모두 네이버→티스토리 **12~13분 간격 직렬** 발행.
- **원인 (오진 3단계를 거쳐 도달)**:
  1. **1차 오진 — "TTL 5분을 넘겨서"** ([541] 이 이 전제로 캐시를 껐다). 반증: 동일 system 을 **13분 25초** 간격으로 발사해 2회차 `read` 회수 확인. 12분 간격은 TTL *안*이다.
  2. **2차 오진 — "앞부분만 고정하면 그만큼 회수"**. 반증: 앞 7,666자 동일 + **꼬리 한 줄만** 다른 system 3회 연속 → `read` 전부 **0**. **블록 내부 부분 회수는 없다.**
  3. **진짜 원인**: system 이 바이트 불일치였다. 범인은 딱 두 개 — ① `- 문체: {spec['tone']}` **한 줄** ② 학습 지침 블록(UCB 가 매 호출 다른 8개를 뽑음).
- **캐시 동작 — 실측으로 확정한 3원칙** (이게 이 항목의 핵심 자산):
  | 조건 | 결과 |
  |---|---|
  | system 동일 · user 다름 | **system 만큼 회수** (read 23,875) |
  | system 다름 · user 동일 | **전부 무효** (read 0) — prefix 라 앞이 깨지면 뒤도 죽는다 |
  | 블록 앞 27K 동일 · 꼬리 1줄 다름 | **그 블록 통째로 무효** |
- **부수 발견 1 — 쓰기 배수가 1.25 가 아니라 2.0**: `cost_usd` 617건을 4단가 동시 회귀(R²=0.994) → 캐시쓰기 6.17 $/MTok(입력 3.0 대비 **2.06배**). 가설검정 총액오차 1.25 가정 **19.96%** vs 2.0 가정 **4.53%**. → 손익분기가 재사용 0.28배 → **1.111배**로 뒤집힌다.
- **부수 발견 2 — [541] 이 만든 감시기가 죽어 있었다**: `_CACHE_POLICY_SINCE = "2026-07-27T14:30:00"` 인데 `ts` 는 SQLite `datetime('now','localtime')` 가 쓴 **공백 구분자**다. 문자열 비교라 `ts >= '...T14:30:00'` 이 **항상 0행** → [C2] 레그가 무증상 통과. 고친 뒤 8 alias 를 읽고 **못 보던 위반 1건**(`analyzer_evidence` 재사용 0.00배)을 즉시 잡았다.
- **헛다리**:
  - **"앵커 블록을 정해 큰 쪽을 살린다"** (테마는 user 가 크니 user 를 앵커로) — **성립 안 함**. prefix 라 system 이 깨지면 user 도 못 쓴다. 무조건 system 이 먼저다.
  - **"cache_create 가 system+user 합과 일치하니 하나의 단위"** — 아니다. *둘 다 캐시에 쓴다*는 뜻일 뿐, 매칭은 블록별로 일어난다.
  - **"system 을 동일하게 만들면 켤 수 있다"** — **아니다**. 아래 참조.
- **해결**:
  - `draft_writer.build_platform_block(platform)` **신설 — 플랫폼별 지시의 유일한 출구**. system 3곳(경제 `_gen_economic_ts_nv` · 폴백 `_build_section_system_msg` · 테마 `_gen_theme`)에서 `spec[...]` 보간 제거 → user 로. **부수 수리**: 폴백 call2·call3 은 원래 user 에 문체 지시가 없어 system 에 의존했다 — 함께 넣었다(그대로 뒀으면 문체 지시를 통째로 잃을 뻔).
  - `quality_learner._pinned_pick()` — 지침 묶음을 **발행쌍 단위로 고정**(TTL 60분). ★ **사용 기록은 플랫폼별 2건 그대로 유지** — `_match_analysis` 가 `platform=''` 이면 '가장 이른 글' 하나만 잡으므로 1건으로 줄이면 두 번째 글의 채점 신호가 학습에서 사라진다. 곁다리로 UCB 공정성도 복구(종전엔 첫 플랫폼 호출이 `uses` 를 올려 두 번째가 흔들린 랭킹을 받았다).
  - `_CACHE_WRITE_MULT`/`_CACHE_READ_MULT` 상수화 + 흩어진 `1.25` 사본 5곳 제거, `datetime(?)` 경유 비교.
  - `precommit_check --category cache` **신설** — system 블록 안 `spec[...]` 차단. 금지 키는 `PLATFORM_SPEC` 을 AST 로 파싱해 *플랫폼마다 값이 다른 키만* 파생(원칙②) → 축이 늘면 자동 추적. 목록 파생 실패 시 `cache/self-check` 위반(fail-closed).
- **★ 그런데도 캐시는 켜지 않는다 — 이유가 바뀌었다**: system 은 이제 md5 까지 동일하다. 문제는 **호출 횟수**다. 손익분기가 **2.11회**인데 발행쌍은 **정확히 2회**(실측 3쌍 전부 재시도 0). 실데이터 검산 — 캐시 ON 191,632 vs OFF 101,378 vs 동일화 후 ON 107,285 → **여전히 OFF 가 5.8% 싸다.** 켜는 조건은 "system 동일" 이 아니라 **"같은 프리픽스 3회 이상"**.
- **검증**: 4조합 프롬프트를 실제 조립해 바이트 diff(LLM 0회) — 경제 system `ba9991154e`/`ba9991154e`, 테마 `8291695c00`/`8291695c00` **md5 일치**. `pair_pin_effective()` True. `cache` 검사 음성테스트(위반 주입 → exit=1, 원복 → 통과). 전체 precommit 59종 0건.
- **파일**: `shared/llm.py` · `JARVIS02_WRITER/draft_writer.py` · `JARVIS07_GUARDIAN/quality_learner.py` · `shared/precommit_check.py`
- **교훈**: ① **오진 위에 처방을 쌓으면 증상만 덮인다** — [541] 은 "TTL 을 넘겨서" 로 캐시를 껐고, 그 판단은 결과적으로 옳았지만 *이유가 틀려서* 진짜 원인(바이트 불일치)이 그대로 남았다. ② **성능 가설은 실험으로 끝낸다** — TTL·블록 단위·부분 회수 셋 다 코드를 읽어선 알 수 없었고, LLM 6회(약 12만 토큰)로 확정했다. 설계가 정반대로 갈리는 지점에서는 그게 가장 싼 길이다. ③ **"이렇게 하면 이득" 을 계산 없이 말하지 말 것** — 조사 단계의 "−24~28% 절감" 은 1.25배 가정이었고, 2.0배에서는 켜는 것 자체가 손해였다. ④ 감시기를 만들었으면 **잡는 것까지** 확인할 것 — 구분자 한 글자로 4일간 무증상 통과했다.

---

## [541] ✅ 해결 — 회수 못 할 캐시에 프리미엄을 내고 있었다 (alias별 캐시 정책) (2026-07-27)

- **증상**: `writer_long_body` 캐시 재사용 **0.00배**(생성 191,224 / 읽기 0), `writer_short_analysis` **0.08배**. 캐시를 켜 둔 채 회수를 못 하니 쓰기 프리미엄이 순손실.
- **환경**: `shared/llm.py` `MODELS` · `_sdk_env()`. 종전엔 캐시 on/off 노브 자체가 없었다.
- **원인**: 캐시는 *한 번 비싸게 쓰고 여러 번 싸게 읽어야* 이득인데, 재사용이 손익분기 미만인 alias 가 섞여 있었다. 전역 일괄 정책이라 alias 별로 끌 수단이 없었다.
- **해결**: `ModelSpec.cache: bool` 신설 → `_sdk_env(alias)` 가 `DISABLE_PROMPT_CACHING` 을 **alias 별로** 방출(텍스트·비전 두 통로의 env 조립을 이 헬퍼 하나로 통일 — [540] 과 같은 형태의 통로별 누락 방지). `writer_long_body`·`writer_short_analysis` 를 `cache=False`. 낡은 판정을 감시할 `cache_selfcheck()` 동봉.
- **★ 사후 정정 (→ [542])**: 이 항목이 적은 **판단 근거 두 개가 실측으로 반증**됐다 — ① "TTL 5분" (실측 13분 25초 이상) ② 쓰기 배수 1.25 (실측 2.0). 그리고 `cache_selfcheck()` 는 `_CACHE_POLICY_SINCE` 의 `'T'` 구분자 때문에 **한 번도 판정한 적이 없었다**. `cache=False` 라는 *결론* 은 유지되지만 *이유* 는 [542] 로 교체됐다.
- **파일**: `shared/llm.py`
- **교훈**: ① 정책을 넣었으면 **ERRORS 에 즉시 박제할 것** — 이 항목은 코드 주석이 `[541]` 을 참조하는데 실물이 없었고(규정 위반), 그 공백 때문에 다음 작업자가 근거를 코드 주석에서만 읽어 오진을 물려받았다. ② **끄는 판단이 맞아도 이유가 틀리면 재발한다.**
...
## [566] 검문소가 2달간 안내문 없이 서 있었다 — `set -e` 가 판단 코드를 통째로 덮음 (2026-08-05)

- **증상**: pre-commit 훅에 "경고 모드 / `JARVIS_STRICT=1` 이면 차단" 두 모드가 문서 5곳에 적혀 있었으나 **둘 다 한 번도 실행된 적이 없다**. 위반 시 3원칙 상기 문구·우회 안내가 전부 사라졌다.
- **환경**: 2026-06-07 최초 커밋 ~ 2026-08-05. 그 사이 커밋 446건.
- **원인**: 훅 14행 `set -e`("명령 하나라도 실패하면 즉시 멈춰라")와 25행 `status=$?`("실패를 받아서 내가 판단하겠다")가 **같은 커밋에 동거**. `set -e` 가 먼저 이기므로 검사기 실패 즉시 스크립트가 죽고, 그 아래 코드(3원칙 안내·모드 분기·우회 안내)는 **도달 불가**가 된다. 위반이 없으면 38행 `exit 0` 으로 빠지므로 42~51행은 *어느 쪽으로도* 실행되지 않는 죽은 코드였다.
- **왜 2달간 안 들켰나**: ① 그 길을 지나려면 *커밋 시점에 위반이 있어야* 하는데 위반이 계속 0건이었다 ② **증상이 목표와 똑같이 생겼다** — README 의 3주 계획이 향하던 종착점이 "차단" 인데 코드는 1일차부터 차단하고 있었다. 보이는 게 전부 기대와 일치했다.
- **헛다리**: "누가 언제 망가뜨렸나" 를 git 으로 찾는 것. 최초 커밋은 480파일 일괄 임포트고 훅 README 의 마지막 수정일(5/16)이 커밋(6/7)보다 3주 앞선다 — **범인 커밋이 존재하지 않는다.**
- **해결**: `set -e` 삭제. 죽은 경고 모드·`JARVIS_STRICT` 분기를 **되살리지 않고 삭제**(2달간 차단으로 446 커밋이 멀쩡했다 — 느슨하게 할 이유가 없다). 3원칙 안내를 **검사기 호출 앞**으로 이동(뒤에 있으면 *3원칙을 어겨서 위반이 잡힌 바로 그 순간에만* 사라진다 — 의도와 정반대였다). 검사기 부재 시 `exit 0` → `exit 1`(fail-closed). 문서 4곳 실상대로 정정. **임시 git 저장소에서 훅을 실제로 실행하는 골든 테스트** 신설 — 이게 없어서 2달간 몰랐다.
- **파일**: .githooks/pre-commit · .githooks/README.md · CLAUDE.md · README.md · .github/workflows/ci.yml · tests/test_publish_golden.py
- **교훈**: **관용구는 설계를 모른 채 덮는다.** `set -e` 는 생각 없이 붙는 한 줄인데, 붙는 순간 "실패를 받아서 판단" 하는 코드가 전부 죽고 그게 눈에 안 보인다. 그리고 **죽은 코드는 스스로 신고하지 않는다** — 그 길을 지나가는 테스트가 없으면 영원히 모른다.

## [567] "위반 0건" 이 사실은 "검사가 안 돌았다" 였다 — crossproc 자가검사가 태생부터 무력 (2026-08-05)

- **증상**: `crossproc` 의 잡 래퍼 직렬화 검사가 **쓰인 이래 한 번도 실행된 적이 없다**. system·venv 양쪽에서 `ModuleNotFoundError: No module named 'JARVIS04_SCHEDULER'`.
- **원인**: 훅은 `python3 shared/precommit_check.py` 로 실행하므로 `sys.path[0]` 이 `shared/` 다. 저장소 루트가 없어 내부 모듈 import 가 실패하는데, 그걸 `except Exception: pass` 가 삼켰다 → 화면엔 계속 "통과, 위반 0건".
- **헛다리 아님 — 이미 알던 함정**: 같은 파일의 `_collect_api_names()` docstring 에 *"importlib 로 실제 로드하면 sys.path[0]=shared/ 인 실행에서 조용히 실패해 검사가 통째로 무력화된다"* 고 **이미 적혀 있었다.** 그쪽은 AST 파싱으로 피했는데 여기만 빠졌다 — 같은 병의 두 번째 발현.
- **해결**: 호출 전 `sys.path` 에 루트 삽입 + `except: pass` → `crossproc/self-check` 위반 보고(fail-closed, `collect/self-check` 와 동일 형태). 검증은 **selfcheck 가 실제로 불렸는지 기록해서** 확인(구조 검사는 뮤테이션에서 가짜 통과를 냈다).
- **부수 발견**: ① 요약 문구의 "60종" 은 각 검사가 손으로 더한 값이라 실제와 어긋나 있었다 → 실행한 카테고리에서 파생. ② CI 가 `JARVIS_STRICT=1` 을 넘기는데 읽는 코드가 0곳 — 있으나 마나 한 설정이 "켰다" 는 착각을 만든다 → 제거. ③ 문서 9곳의 "27종"(실제 19 카테고리) → 숫자를 아예 뺐다. ④ CLAUDE.md·README 의 *"데몬 부팅 시 자동 실행"* 은 거짓 — `jarvis_daemon.py` 에 호출 0건.
- **파일**: shared/precommit_check.py · .github/workflows/ci.yml · CLAUDE.md · README.md · .githooks/README.md · JARVIS07_GUARDIAN/auditor.py
- **교훈**: **검사가 없는 것과 통과한 것은 다르다.** `except: pass` 는 그 둘을 같은 화면으로 만든다. 그리고 화면에 뜨는 숫자를 손으로 세면, 어느 순간 그 숫자가 조용히 거짓말을 시작한다.

## [568] 내 테스트가 '내 맥북' 을 검사하고 있었다 — CI 첫 실전에서 즉시 발각 (2026-08-05)

- **증상**: 로컬 101 통과 · CI `3 failed, 98 passed`. PR #60 이 브랜치 보호에 막힘(BLOCKED).
- **환경**: 브랜치 보호를 켠 직후 첫 PR. CI 는 **깨끗한 체크아웃** 이라 `.env`·쿠키·`logs/` 가 전혀 없다(전부 gitignore).
- **원인**: 새로 쓴 테스트 3개가 *런타임 산출물의 존재* 를 전제로 삼았다.
  ① `_safe_path('.env') is None` ② `secret_files()` 에 `.env` 포함 ③ 로그 디렉터리 ≥2개.
  **테스트가 환경을 검사하면 환경이 바뀔 때마다 거짓말을 한다.**
- **★ 그런데 ①②는 테스트 결함이 아니라 진짜 구멍이었다**: `secret_files()` 가
  `if f.exists()` 로 걸렀고 이 함수는 **프로세스당 1회 캐시** 된다. 즉 `.env` 가 아직
  없을 때 한 번 불리면 그 프로세스는 이후 `.env` 를 영영 비밀로 보지 않는다.
  CI 에서 실제로 `_safe_path('.env')` 가 경로를 반환했다 — 무승인 도구가 읽을 수 있는 상태.
  **비밀인지는 *경로 규약* 이 정하는 것이지 파일이 지금 있느냐가 아니다.**
- **해결**: ① `.env`·`credentials/` 를 존재 여부와 무관하게 비밀 경로로 등록.
  ② `redact_logs(root=)` 인자 신설 → 테스트가 **합성 트리**(logs 3개 + notlogs 1개)로
  "전부 훑고 logs 아닌 곳은 안 건드리는가" 를 환경 무관하게 검증.
  ③ 검증 절차에 **`git archive` 로 깨끗한 체크아웃을 만들어 돌려보기** 추가 — 실측 103 통과.
- **헛다리**: 실패한 테스트를 CI 에서만 skip 시키는 것. 그러면 ①②의 진짜 구멍이 그대로 남는다.
- **파일**: shared/secrets.py · tests/test_publish_golden.py
- **교훈**: **CI 가 첫 실전에서 값어치를 했다.** 내 맥북에는 런타임 산출물이 쌓여 있어
  테스트가 초록이었지만, 그건 코드가 옳아서가 아니라 *환경이 우연히 맞아서* 였다.
  그리고 이번에도 형태는 같다 — "있다고 가정한 것" 과 "실제로 있는 것" 의 차이.

## [569] "놓친 슬롯 자동 복구" 는 고칠 게 없었다 — 처방을 원인 확인 전에 냈다 (2026-08-05)

- **증상**: 30일 가동률 57.5%(69/120). 내가 "놓친 슬롯 자동 복구" 를 2위 과제로 제안했다.
- **실측 해부 (날짜별 원인 분류)**: 결손 14일이 두 종류뿐이었다.
  ① **기계가 꺼져 있었음 8일** — heartbeat 07-31 4회·08-01 2회(정상 400~2400회). 휴가.
  ② **잡은 돌았는데 안에서 막힘 12일** — heartbeat 274~2428(데몬 정상) + 발행 잡도 실행됨.
  → **'슬롯을 놓친' 경우는 사실상 0.** 재실행할 대상이 없다. 처방이 원인과 무관했다.
- **②의 실제 원인은 이미 고쳐진 것들**: 학습지침 게이트 오차단·회로차단기 면제 우회·
  fact_judge 90초 하드코딩·하네스 오라우팅·중복 발행. 08-05 07:00 이 2/2 발행 + 2/2 채점.
- **★ 정책 결정 (사용자)**: **복구 정책 A — 놓친 슬롯을 재발행하지 않는다.**
  발행은 07:00·21:00 뿐이고, 다른 시각 발행은 그 규칙을 어긴다. 손실은 손실로 둔다.
  만들 것은 '복구' 가 아니라 **정확한 회계와 기록**.
- **적대적 검토가 잡은 것 (구현 전)**:
  · `report(..., severity=)` 는 **존재하지 않는 인자** — 설계대로 짰으면 TypeError.
  · **정책 A 위반은 코드가 아니라 문구에 있었다** — 내가 전날 만든 `recovery_hint()` 가
    «잡ID 지금 실행» 을 권하고 있었다. 정책 결정 *전* 에 만든 것이라 몰랐다.
  · 절전 기상 탐지기가 `pmset -g sched` 만 봐서, 사용자가 처방을 정확히 따라도
    영원히 "미설정" 이라 나무라는 감시가 될 뻔했다(실제 설정은 `-g custom`).
    → 코드 90줄 + preflight 훅을 폐기하고 **문서만** 으로 축소.
- **만든 것**: `slots_between`(임의 구간 슬롯 — 경계 계산 단일 소스) · `slot_key`(연도 포함
  안정 키) · `record_publish_gap`(박제 단일 진입점 + 중복 억제) · `missed_slots` ·
  `job_history.mark_outcome`(잡 성공 사후 보정, 3분기) · `JARVIS00_INFRA/downtime.py`
  (복귀 시 공백 회계) · `EVENT_JOB_MISSED` 리스너 · `job_registry.heartbeat_*`(파생 주인).
- **파일**: JARVIS08_PUBLISH/publish_ledger.py · JARVIS00_INFRA/downtime.py · infra_agent.py ·
  JARVIS04_SCHEDULER/{job_history,job_registry}.py · jarvis_keeper.py · api_server.py ·
  JARVIS07_GUARDIAN/severity.py · docs/RUNBOOK.md
- **교훈**: **원인을 세기 전에 처방부터 냈다.** 날짜별로 갈라보는 데 10분 걸렸고,
  그 10분이 "재실행 엔진" 을 짓는 하루를 막았다. 가동률 같은 *합계 지표* 는 원인을 숨긴다 —
  57.5% 는 하나의 문제가 아니라 성질이 다른 두 문제였다.

## [570] 대시보드가 "한 번이라도 성공했나" 를 "마지막 실행" 이라고 표시했다 (2026-08-05)

- **증상**: 발행 잡 이력 323건이 **전부 success=1**, 실패 기록 0건. 그런데 실제 결손은 14건.
  스케줄러는 자기가 한 번도 실패한 적 없다고 믿고 있었다.
- **원인**: ① APScheduler 는 콜백이 예외 없이 끝나면 성공으로 친다 — 글이 안 나갔어도.
  ② `api_server.py:741` 이 `MAX(success) GROUP BY job_id` — 이건 '마지막 실행' 이 아니라
  **'한 번이라도 성공했나'** 다. 잡이 오늘 실패해도 과거에 한 번 성공했으면 영원히 초록불.
- **해결**: `job_history.mark_outcome()` 신설 — 슬롯 창의 잡 행을 사후 보정.
  **예외를 던지지 않는다**(정책 A: 던지면 재예약이 붙어 정규 시각 밖 발행 위험).
  3분기 반환(`corrected`/`already`/`no_row`) — ④가 넣은 misfire 행을 "기록 없음" 으로
  오독하지 않기 위해서다. `MAX(success)` → 진짜 마지막 실행 행 조회로 교체.
  **둘을 같은 커밋에** — 보정만 하고 조회를 안 고치면 화면은 그대로라 효과를 확인할 수 없다.
- **실증**: 08-04 21:00 테마(티스토리 미발행) 행이 `success=1` → `0` + 사유로 보정됨.
  재호출은 `already`(진짜 예외 메시지를 안 덮음), 없는 창은 `no_row`.
- **교훈**: 집계 함수는 질문을 조용히 바꾼다. `MAX(success)` 는 문법이 맞고 뜻이 틀렸다.

## [571] 발행 파이프라인을 테스트가 한 줄도 실행하지 않았다 — 게이트를 꺼도 전부 통과 (2026-08-05)

- **증상**: 테스트 115개 중 `process_draft`·`enforce_supreme_law`·`prepublish_quality_issues` 를 부르는 것이 **0건**. `PREPUBLISH_FACT_GATE=0` 으로 사실성 게이트를 통째로 꺼도 115개가 전부 통과했다.
- **왜 여태 몰랐나**: 기존 테스트는 전부 *정적 계약 검사*(inspect·AST·grep)였다. "코드가 이렇게 생겼는가" 는 봤지만 "돌리면 어떻게 되는가" 는 아무도 안 봤다.
- **해결**: `tests/test_publish_smoke.py` 신설 — **네트워크 0·실 LLM 0** 으로 4조합 전부 실행.
  가짜는 4개뿐(대본 LLM 1회·판정 LLM·픽셀 생성·파일 쓰기), 나머지는 진짜로 돈다.
  · `tests/conftest.py` 에 `_no_external` — 실 LLM 을 부르면 터지고, **삼켜져도 teardown 에서 드러난다**(이 저장소는 `except Exception: log.warning` 이 도처라 "부르면 터진다" 만으로는 약하다).
  · **반-동어반복 테스트**: 같은 대본·같은 판정에서 킬스위치만 끄면 통과해야 한다 — 그래야 위 테스트가 *게이트를 무력화하는 뮤테이션에 실제로 죽는다*. 실증 완료.
- **작업 중 배운 것 3가지 (전부 실측으로 교정)**:
  ① 게이트 issue 는 **dict** 다. `getattr(i,"kind","")` 로 읽으면 전부 `""` 라 어떤 검사도 통과한다(가짜 초록).
  ② 본문이 `_MIN_BODY`(200) 미만이면 게이트가 **조기 반환** 한다 — 짧은 대본으로 테스트하면 "막았다" 가 아니라 "안 봤다" 인데 초록이다. 길이를 게이트에서 파생해 채운다.
  ③ 블록 타입은 `image` 다. `figure` 로 테스트했다가 "연속이 안 고쳐진다" 는 **있지도 않은 버그** 를 볼 뻔했다 — `block_assembler` 가 내는 타입은 `image` 뿐(실측 5곳).
- **자기 규칙에 걸린 것 2건**: ① 저장소 전역 `INFRA_KIND` 등가비교 금지 규칙이 *파생 검증 테스트* 를 잡았다 → `tests/` 예외를 사유와 함께 명시. ② `copytruth/patch-unverified` 가 monkeypatch 효과 미검증을 잡았다 → **패치가 실제로 소비됐는지** 세는 검증 추가(테스트가 더 좋아졌다).
- **파일**: tests/test_publish_smoke.py · tests/conftest.py · tests/test_publish_golden.py
- **교훈**: **정적 검사는 "코드가 어떻게 생겼나" 만 답한다.** 게이트가 실제로 무엇을 막는지는 돌려봐야 안다. 그리고 가짜를 심었으면 *그 가짜가 소비됐는지* 반드시 확인해야 한다 — 안 지난 경로는 "통과" 처럼 보인다.

## [572] 백업 2.0GB 를 한 번도 검증한 적 없었다 + 로그 정리가 4곳 중 1곳만 (2026-08-05)

- **증상**: ① 저장소 전체에 `integrity_check` **0행** — 백업이 복원 가능한지 확인한 적이 없다. ② 07-31·08-02 백업이 조용히 빠져 있었다(경보 0). ③ 로그 정리 정책이 `JARVIS02_WRITER/logs` 한 곳만 덮어 나머지 3곳 **53.5MB 가 정책 밖**.
- **해결**: `shared/db.verify_backup()`(0.8초/159MB) + `backup_gaps()` 신설. 검증을 **retention 앞**에 둔다 — 순서가 정책이다(새 백업이 깨졌는데 옛 것을 지우면 성한 사본이 하나도 안 남는다). 깨진 파일은 `.corrupt` 로 남기지 않고 **즉시 폐기** — 그러면 retention glob 밖이라 영구 잔존한다(선례: `jarvis_premask_2026-08-02.sqlite` 176MB 가 그렇게 관리 밖). 결손 감사는 **새 잡 없이** 기존 `job_db_backup` 앞에 붙였다(그 잡은 백업이 실패해도 매일 돈다). `file_cleanup.log_dirs()` 는 `rglob("logs")` 실물 파생 — `secrets.redact_logs` 가 먼저 앓고 고친 형태를 그대로 베꼈다.
- **실증**: `backup_gaps(7)` → `['2026-08-02','2026-07-31']` 정확 검출.
- **교훈**: 백업은 "만들었다" 가 아니라 **"복원된다"** 여야 백업이다. 그리고 *결손을 감시하지 않는 자동화* 는 조용히 멈춰도 성공처럼 보인다.

## [573] 자가수리 게이트 — 설계를 대폭 축소했다 (2026-08-05)

- **배경**: 매주 토 03:00 `auto_repair` 가 `bypassPermissions` 로 저장소를 편집하는데 사후 검증이 SDK 종료코드뿐이었다.
- **★ 적대적 검토가 설계를 무너뜨렸다**: 초안은 스냅샷 롤백을 넣으려 했는데, `_snapshot_py_files()` 실측 결과 **1,380개 중 1,039개가 `dashboard/node_modules`·`.next`** 였고 학습 산출물(`learned_patterns.json` 등)도 전부 포함이었다. "안전 게이트" 라는 이름으로 **저장소에서 가장 파괴적인 경로**를 신설할 뻔했다.
...
## [576] 강화학습 2계열이 "학습하는 척" 하고 있었다 — 화면은 초록불, 실측은 정지 (2026-08-07)

- **사용자 질문**: "오류 학습·글품질 학습 둘 다 제대로 학습되고 제대로 적용되는지 꼼꼼하게 체크해봐."
- **판정**: ① 오류 학습 = **11일 정지** ② 글품질 = **절반만 동작**(주입 ✅, 보상이 순위를 못 바꿈)

**① 오류 밴딧 — 왜 멈췄나**
- `bandit_state.json` 마지막 갱신 07-27(11일), 07-28 이후 `[BANDIT]` 로그 0줄, 8월 GUARDIAN 실제 코드 수정 0건, 패턴 재사용률 49개 중 6개(12.2%).
- **원인**: `claude_sdk_compat.run_sdk_query` 가 `try` 로 끝나 어떤 경로에서 **암묵적 `None`** 을 반환 → 호출자가 `result["elapsed"]` 로 즉시 `'NoneType' object is not subscriptable`(전체 21회). 그런데 `guardian_agent` 의 `except Exception` 이 `log.warning` 한 줄로 삼켜 **어느 줄인지 아무도 몰랐다**. `llm` arm 의 유일한 보상 경로가 조용히 막힌 채 `wontfix` 만 쌓였다(07-27 이후 17건 중 16건).
- **해결**: 계약 주인(`run_sdk_query`)이 **어떤 경로로도 None 을 내지 않도록** 종단 return 추가(①). 호출부는 계약 위반 시 크래시 대신 사유 기록. 삼키던 `except` 는 traceback + `report()` 로.

**② 글품질 — 왜 순위가 안 바뀌나**
- 반사실 실측: 보상 288건을 **전량 제거해도 top-8 이 16칸 중 15칸 그대로**. 진짜 순위 주인은 재발견 카운터(+0.5/회)였고, 보상 1회 기여는 +0.057 — **재발견 1회 ≈ 보상 9회**. 재발견은 *"주입했는데 안 지켜져 또 지적됐다"* 는 **실패 신호인데 순위를 올린다**.
- **원인 1 — 하향 불가**: 갱신식 `w ← w + α(reward − 0.5)`, `reward = 점수/100`. 실측 점수는 **59~77(중앙값 69), 50점 미만 0건** 이라 `Δw` 최솟값이 **+0.027, 항상 양수**. "검증된 지침만 생존" 이 아니라 **"쓰인 지침은 전부 생존"** 이었다.
  → 중립점을 **실측 중앙값에서 런타임 파생**(`reward_neutral()`). 68.5 를 박지 않는 이유: 글이 좋아지면 분포가 통째로 올라가 또 전부 양수가 된다. 중앙값이면 언제나 절반은 내려간다.
- **원인 2 — 관측 창 어긋남**: 보상 재시도 창 **21일** vs 재채점 창 **약 1일**(보상 잡 cron 파생). 21일 창 안의 미채점 글이 재채점 창 밖이라 영원히 채점 불가였고, 조회는 **"할 일 0건"** 이라 보고했다(창 밖이라 안 보였을 뿐). 미귀속 606행.
  → `reward_cutoff()` 를 `reward_retry_days()` 에서 파생. **사장돼 있던 39건이 즉시 드러났다.**
- **원인 3 — credit assignment 붕괴**: 한 배치의 지침 8개가 전부 같은 보상(그 글의 점수)을 받아 배치 53개 전부 `count(distinct reward)=1`. 그런데 `prepublish_gate` 는 *어느 지침이 안 지켜졌는지* 를 **이미 계산해 놓고 `log.info` 한 줄로 버리고** 있었다.
  → `insight_usage.violated` 신설 + 게이트가 신호를 흘려보냄. 차단은 여전히 안 한다(08-04 과잉차단 교훈) — **신호만** 남긴다.

**★ 화면이 거짓말을 하고 있었다 (코드보다 먼저 고친 것)**
- `/status` 는 `arm_count`·`feature_dim` 이라는 **구조 상수만** 읽어 11일 정지 중에도 "fixer 9종 학습" 을 냈다. → `stats()` 에 생존 지표(`observed_arms`·`last_update_h`·`stalled`) 추가, 정지면 🛑 로 표시.
- 대시보드는 `llm_saved` 를 "LLM 절약 58회" 로 보여줬는데, `auto_repair` 가 그 자리에 `actionable_hits` 를 그대로 넣은 값이었다(21회차 전부 58/58). 저장 패치 47개 중 **복원 가능은 3개(6.4%)**, 실측 재적용은 81일간 1건. → `_real_llm_saved()` 로 교체(모르면 0).
- **③원칙** — 표시 통로가 둘인데 `/status` 만 고칠 뻔했다. 대시보드에도 같은 파생(`bandit.stats()`)을 노출하고 정지 배너를 넣었다.

**★ 뮤테이션이 내 가짜 파생을 잡았다**: 정지 임계를 "발행 주기에서 파생" 한다며 `24.0 * 2 / 1` 을 썼다 — **슬롯 수와 무관한 상수 48**. 파생인 척한 사본이다. `24/슬롯수 × 4` 로 교정.

- **파일**: JARVIS07_GUARDIAN/{bandit,quality_learner,guardian_agent,auto_repair}.py · shared/{db,claude_sdk_compat}.py · JARVIS02_WRITER/prepublish_gate.py · JARVIS03_RADAR/post_quality_analyzer.py · api_server.py · dashboard/app/learning/page.tsx
- **교훈**: **강화학습은 사슬이라 한 고리만 끊겨도 "학습하는 척" 이 된다.** 그리고 그 상태에서 화면은 초록불이다 — 구조 상수는 학습이 멈춰도 그대로이기 때문이다. 생존은 *마지막 갱신 시각* 이 답한다.

## [577] 감사 서브에이전트가 운영 DB 에 가짜 행 32건을 넣었다 (2026-08-07)

- **증상**: 강화학습 감사 중 서브에이전트가 `insight_usage` 에 테스트용 사용 기록 32행(4배치, 4조합)을 INSERT 했다. 지우려다 **권한 시스템이 DB 쓰기를 차단**해 남았다.
- **위험**: 오늘 발행은 0건인데 23:45 `j07_quality_learn` 이 이 행을 집어가면 보상 귀속이 오염된다.
- **데이터 손실 없음**: 926행 결번 0, 백업 대비 정상 증가(742→822→870→926). 삭제가 아니라 **삽입**이 문제였다.
- **해결**: 백업 후 `used_at LIKE '2026-08-07 09:42%'` 32행만 제거(사용자 실행). 잔여 894행.
- **교훈**: **조사 에이전트에게 운영 DB 쓰기를 허용하면 안 된다.** 읽기만으로도 감사는 충분했다. 권한 가드가 2차 피해(잘못된 DELETE)를 막은 것은 설계대로 동작한 것이다.
## [578] ✅ 해결 — 중복 발행 방어선이 **한 프로세스 안에서만** 살아 있었다 (2026-08-07)

- **증상**: 같은 슬롯에 같은 (글종류·플랫폼) 글이 2건 이상 나간 날이 있었다.
  가장 뚜렷한 것은 **2026-07-20 21:00 — 네이버에 테마 글 3건**
  (mRNA 관련주 / 항공기부품 관련주 / 모바일솔루션 테마주 — *서로 다른 테마*).
- **원인 (원칙① 위반)**: 중복 방지가 harness state 의 불리언 4개
  (`__nv_send_attempted__` 류)뿐이었고, 그 로직이 `economic_poster._send_platform` 과
  `trend_theme_writer._send_theme_platform` 에 **똑같이 두 벌** 적혀 있었다.
  플래그는 *한 프로세스·한 액션* 안에서만 산다. 실측으로 못 막은 경로:
  ```
  · 같은 슬롯에 발행 잡 2회 기동 — 최근 90일 12회 (job_runs)
  · 재시도 콜백이 발행 전체를 재실행 — 07-20 네이버 3건
  ```
  둘 다 **새 state 로 들어오므로** 플래그가 구조적으로 못 본다.
- **★ 비직관 — 개별 경로를 고치는 것으로는 끝나지 않는다**:
  07-20 의 *생성기*(재시도 콜백이 플랫폼 수만큼 등록되던 것)는 커밋 `bb436a9` 에서
  이미 제거됐다. 그런데도 이 항목을 만든 이유는, 그 방식이 **다음 재시도 경로가 생기면
  또 새기** 때문이다. 마지막 방어선은 *모든 경로가 반드시 지나는 곳* 에, 그리고
  **프로세스 밖(DB)** 에 있어야 한다.
- **해결**: `publish_ledger.already_published_this_slot()` 신설 — 판단 본체는 발행 도메인
  단독이고 두 발행자는 부르기만 한다. 슬롯 창은 `publish_slots()`(DEFAULT_JOBS 파생)에서
  계산(리터럴 0). 경제·테마 × 네이버·티스토리 **4조합 전부**.
- **★ 헛다리 — `current_slot()` 을 그대로 쓰면 안 된다 (구현 중 발견)**:
  `current_slot()` 은 *시각* 기준(글종류 무관)이라 06:30 에 물으면 **직전 테마 창** 을
  답한다. 실측으로 경제 잡은 스케줄이 07:00 이 되기 전 **06:30 에 돌았다**. 그 시각에
  시각 기준 창으로 "경제가 이미 나갔나" 를 물으면 *어제 경제 글* 이 잡혀
  **오늘 발행을 영구히 막는다** — 중복보다 나쁜 오탐이다.
  → 글종류 기준 창(`owning_slot`)을 따로 파생하고 **창 밖이면 억제하지 않는다**(fail-open).
- **★ 경계 결함 (시험 중 발견)**: `slots_between` 이 끝 경계를 배타적으로 보므로
  `07:00:00` 정각에 "창 밖" 이 나왔다. 발행 잡이 **정각 기동**이라 하필
  *가드가 가장 필요한 순간에 꺼진다*. `+1초` 로 교정(`current_slot` 과 같은 처리).
- **정직한 한계**: 글이 플랫폼에 올라갔는데 발행자가 실패로 보고하면(ack 유실)
  `post_analysis` 행이 안 생기므로 여기서도 못 잡는다. 그 창을 닫으려면 플랫폼 사실
  조회가 필요하다(네이버는 `_fetch_recent_naver_posts` 가 이미 있다). 별건.
- **동반 수정 — precommit `copytruth` 오탐**: pytest `monkeypatch` 픽스처를 *운영
  몽키패치* 로 오인해 정상 테스트를 위반으로 잡았다. 픽스처는 자동 원복되고 **패치가
  안 먹으면 그 테스트가 실패** 하므로 테스트 자체가 효과 검증이다 —
  `patch_effective()` 를 요구하면 "검증을 검증하라" 는 순환이 된다.
  `monkeypatch.` 접두만 좁게 면제. 변이 시험으로 **운영 몽키패치는 여전히 검출** 확인.
- **검증**: 신규 골든 16개 · 변이 3/3 검출(테마 가드 제거 / 정각 경계 되돌림 / 창 밖 억제
  유발) · 양성(08-06 경제 실발행 → 억제 True) · 음성(08-07 미발행 → False) ·
  슬롯 경계 9/9 · pytest 162 · precommit 20 카테고리 0건 ·
  재시작(PID 95351, 14:43:55) 후 **데몬이 로드한 코드로** 재확인.
- **파일**: `JARVIS08_PUBLISH/publish_ledger.py` · `JARVIS02_WRITER/economic_poster.py` ·
  `JARVIS02_WRITER/trend_theme_writer.py` · `tests/test_publish_dedupe.py`(신규) ·
  `shared/precommit_check.py`
- **교훈**:
  ① **"이 판단이 프로세스보다 오래 사는가" 를 물어라.** 재시도·재기동·다른 잡이 같은
     자원을 건드리는 시스템에서 메모리 플래그는 *가장 중요한 순간에만* 백지가 된다.
  ② **개별 경로를 고치는 방식은 경로가 늘면 진다.** 07-20 의 생성기를 지운 것은 옳았지만,
     그것만으로는 다음 생성기를 막지 못한다. 관문을 하나로 만들어야 끝난다.
  ③ **가드를 만들 때 "정상 산출물을 막는 경우" 를 먼저 시험하라.** 이번에도 최악 오탐
     (어제 글로 오늘 발행 차단)이 설계 초안에 그대로 있었고, 시험이 잡았다.
     [553] 과 같은 축이다 — *막는 쪽이 항상 안전한 것이 아니다.*
  ④ **검사기가 정상 코드를 잡으면 검사기를 고쳐라 — 단, 좁게.** 규칙을 넓게 풀면
     검사기가 죽는다. 면제 후 **진짜 위반이 여전히 잡히는지** 변이로 확인할 것.

---

## [579] 🔍 조사완료(결함아님) — `log_file` 소스 TypeError 는 [578] 동반수정의 지연 재포착 (2026-08-07)

- **증상**: `source=log_file`, `module=daemon.log`, `error_type=TypeError`,
  `message="'NoneType' object is not subscriptable"`. traceback 은
  `guardian_agent.py:486 _try_sdk_targeted_fix` — `(error_record.get('traceback', ''))[:2000]`.
- **원인**: 이 버그 자체는 이미 14:50(파일 mtime)에 GUARDIAN 자가수리가
  `(error_record.get('traceback', '') or '')[:2000]` 로 고쳤고 커밋 `1b550340`
  ([578] 동반수정)에 이미 반영·기록됨. 그런데 버그는 고쳐지기 *직전*인 14:49:40 에
  1회 발동해 daemon.log 에 Traceback 블록을 남겼고, 5분 주기 `job_scan_logs`
  (`_LogFileHandler._scan_file`, 이전 스캔 위치 이후 신규 바이트만 tail)가 다음 스캔
  (14:54:00)에서 그 텍스트를 "처음 보는 신규 내용"으로 정상적으로 수집해 `source=log_file`
  오류(#5260/#5261)를 별도 생성했다. 로그 스캐너 자체는 정상 동작(과거분 재수집 아님) —
  단지 고쳐지기 직전 순간의 흔적을 뒤늦게 주웠을 뿐.
- ⛔ **헛다리(다시 시도 금지)**: 없음 — `guardian_agent.py:486` 재수정 시도 불필요
  (이미 `or ''` 가드 존재, git blame 확인).
- **해결**: 코드 변경 0건. 이 오류(log_file 소스)는 `wontfix` 처리 대상 —
  근본 원인은 [578] 동반수정에서 이미 해소됨.
- **파일**: JARVIS07_GUARDIAN/ERRORS.md (본 항목만)

---

## [580] ✅ 해결 — 텔레그램 알림 4건이 전부 *고칠 것이 없는* 오류였다 (접수 오분류 2종) (2026-08-07)

- **증상**: 🚨 "송출 차단 — 수동 검토 필요" + "GUARDIAN 자동수정 실패" 4건이 연달아 왔다.
  전부 **사용자가 조치할 것이 없는** 알림이었다.

> **[579] 와의 관계** — 같은 증상을 다른 세션이 먼저 조사해 *"[578] 동반수정의 지연
> 재포착, 결함 아님"* 으로 결론냈다. 그 진단은 **맞다**. 이 항목은 거기서 한 걸음
> 더 간다: *왜 지연 재포착이 **고칠 수 없는 별건**으로 남는가* 를 고쳤다.
> 원인은 재포착 자체가 아니라 `module=log_file.name` 이었다 — 그 한 줄 때문에
> 재포착분이 원본과 다른 사건이 되고, 자동수리가 반드시 실패하고, 사용자에게
> "수동 검토" 로 올라갔다. 재포착은 계속 일어나도 되지만, **같은 사건으로** 접수돼야 한다.
- **★ 오분류 ① — "이미 돌고 있다" 를 사고로 취급 (원칙① 위반)**
  harness 동시성 락이 막은 호출을 GUARDIAN 박제 + escalation 알림으로 올렸다.
  고칠 코드가 없으니 Tier 1·2 는 **반드시** 실패하고, 그 실패가 또 알림이 된다.
  실측 — 전체 이력 3건이 전부 같은 모양: 05-18 `concurrent_test` · 07-31 `트렌드 수집` ·
  08-07 `분석 fallback`. `jarvis_daemon.py:552` 가 부팅 즉시 `job_analyzer_fallback` 을
  돌리고 5분 뒤 `analyzer_fb` 인터벌 잡이 같은 동작을 부른다 — **설계상 정상인 레이스**다.
  (08-07 14:44 부팅 → 14:49 인터벌 → 오류 시각과 정확히 일치)
  ```
  같은 뜻을 세 곳이 서로 다르게 처리하고 있었다
    interpreter_shutting_down()   → deferred (실패 아님)              ← 옳음
    job_history._on_job_missed    → MAX_INSTANCES 를 *일부러* 안 들음   ← 옳음
    harness 동시성 락              → GUARDIAN + 🚨                     ← 틀림
  ```
- **★ 헛다리 — 그냥 `deferred` 로 바꾸면 이중 발행이 열린다 (구현 중 발견)**
  `economic_poster` 가 `deferred` 를 **먼저** 검사한다. 동시중복도 deferred 의 한 종류라
  그 분기에 영영 도달하지 못해 `_concurrent_blocked` 가 안 서고 **티스토리가 그대로
  진행**된다 → 인터리브 이중 발행. 게다가 종전 판정은
  `"동시 실행 중복 차단" in escalation_reason` **문자열 비교** 라 문구 한 글자만 바꿔도
  가드가 조용히 죽는다.
  → `ActionResult.concurrent_blocked` 구조화 필드 신설 + 경제 분기 순서 교정.
  ※ 테마는 `deferred` 면 티스토리를 아예 건너뛰어 이 함정이 없었다(원칙③ 확인 완료).
- **★ 오분류 ② — '어디로 들어왔나' 를 '어디서 났나' 자리에 적음 (원칙② 위반)**
  `error_collector._scan_file` 이 `module=log_file.name`(=`daemon.log`). traceback 안에
  진짜 위치가 있는데 쓰지 않았다. 셋이 한꺼번에 망가진다:
  ① 같은 사건이 갈린다(직접 report 는 실제 모듈, 스캐너는 `daemon.log`)
  ② Tier 1·2 가 반드시 실패한다(`daemon.log` 는 고칠 수 있는 소스가 아니다)
  ③ 그래서 **이미 고친 버그를 수동 검토하라**는 알림이 간다
  실측: `guardian_agent.py:486` 이 14:49:40 에 고쳐졌는데(id 5256 `fixed`),
  14:54:00 로그 스캔이 같은 traceback 을 읽어 **2건을 더** 만들었다(5260·5261 `wontfix`).
- **해결**: `_tb_origin()` 신설 — **저장소 안쪽의 마지막 프레임**을 고른다.
  맨 끝 프레임을 그냥 쓰면 `.venv/selenium/...` 같은 *남의 코드* 가 원인으로 박제된다.
  저장소 밖 프레임만 있으면 빈 값 → 호출부가 파일명으로 폴백(**위치를 지어내지 않는다**).
- **★ 내 테스트가 처음엔 허수아비였다 — 변이 시험이 잡았다**
  초판은 변이 3종 중 **1종만** 검출했다. 원인은 둘 다 *검사 방식* 이었다:
  · `code.find("concurrent_blocked")` 가 분기가 아니라 앞쪽 **지역변수 초기화**를 집었다
  · 주석 제거기가 토큰을 공백으로 이어붙여 `log_file.name` → `log_file . name` 이 돼
    정규식이 빗나갔다
  → 둘 다 **AST** 로 교체(분기 조건 노드 / `catch(module=)` 인자 노드) + 범위를
    `_scan_file` 안으로 좁힘(파일 전체 `catch` 7건에 걸어 거짓 실패했었다). 재시험 **3/3 검출**.
- **검증**: 신규 골든 7개 · 변이 3/3 · pytest 169 · precommit 20 카테고리 0건 ·
  재시작(PID 3313, 15:19:21) 후 부팅 레이스가 **조용히** 지나가고 누적 오류 3건 유지 확인.
- **파일**: `JARVIS00_INFRA/harness.py` · `JARVIS02_WRITER/economic_poster.py` ·
  `JARVIS07_GUARDIAN/error_collector.py` · `tests/test_error_intake.py`(신규)
- **교훈**:
  ① **알림은 "조치할 것이 있을 때만" 울려야 한다.** 고칠 코드가 없는 사건을 자동수리에
     넣으면 *실패가 보장* 되고, 그 실패가 또 알림이 된다 — 소음이 2배로 는다.
  ② **기록은 '무엇이 났나' 를 담아야 한다. '어디로 들어왔나' 가 아니라.**
     접수 경로를 원인 자리에 적으면 dedup·자동수리·보고가 **동시에** 망가진다.
     ([547] 오류 세분화와 같은 축 — 그때는 *타입*, 이번엔 *위치*.)
  ③ **"실패 아님" 을 도입할 땐 호출자의 분기 순서를 반드시 보라.** 새 상태가 기존
     상태의 *부분집합* 이면, 먼저 검사하는 쪽이 이겨서 새 처리가 영영 실행되지 않는다.
  ④ **변이 시험 없이 통과한 테스트는 통과한 게 아니다.** 이번에도 초판이 3중 2를
     놓쳤고 둘 다 *텍스트 검사* 탓이었다. 구조를 볼 일에는 AST 를 쓸 것.

---

## [581] ✅ 해결 — "내가 노트북을 껐다" 를 시스템이 *고장* 으로 세고 있었다 (2026-08-07)

- **배경 (사용자 박제)**: 이 시스템은 **개인 노트북** 에서 돈다. 사용자는 자비스 전용이
  아니라 다른 개인 작업에도 그 맥북을 쓰고, **일부러 끄는 날도 많다.**
  그때 발행이 안 되는 것은 *결함이 아니라 사실* 이다 — 조용히 기록하고 넘어가야 한다.
  반대로 **전원·네트워크가 원인이 아닌 미발행은 원인을 정확히 가려 기록하고 고쳐야 한다.**
- **증상**: 결손 감사가 **모든** 미발행을 `reason="audit"`(진짜 실패)로 박았다. 전원을 끈
  날에도 🚨 가 울리고 GUARDIAN 이 고칠 것 없는 일에 Tier-2 를 열었다.
- **★ 원인 — 사유가 *누가 먼저 발견했는가* 로 갈렸다 (원칙① 위반)**
  ```
  데몬 복귀 시 report_boot_downtime() 이 먼저 보면  → daemon_down (조용)
  감사 잡이 먼저 보면                                → audit (🚨 + GUARDIAN)
  ```
  같은 원인이 **레이스로** 다르게 분류됐다. 분류 장치(`reason`·`kind`·`severity` 등록)는
  이미 있었는데, **감사 잡이 그 질문을 하지 않았다** — `record_publish_gap(..., reason="audit")`
  가 리터럴로 박혀 있었다.
- **해결 (원칙①②③)**
  - **①** 생존 판정은 신호의 주인이 한다 — `downtime.downtime_in_window(start, end)` 신설.
    `publish_ledger` 는 묻기만 하고 heartbeat 를 직접 읽지 않는다.
  - **②** 임계·창을 전부 파생: 정지 임계 = `downtime_threshold_sec()`(발행 잡
    `misfire_grace_time` 파생) · 판정 창 = `audit_lag_minutes(misfire_grace_for(잡ID))`.
    `misfire_grace_for()` 를 `job_registry`(DEFAULT_JOBS 주인)에 신설 — 그 파생이
    **세 번째 사본**이 될 참이었다.
  - **③** `publish_ledger` 는 4조합 공용이라 한 곳 고침이 넷을 덮는다.
  - 전원 오프로 판정되면 **알림도 잡이력 보정도 하지 않고** 기록만 남기고 끝낸다.
- **★ 구현 중 잡은 결함 2건 (둘 다 *진짜 고장을 전원 탓으로 덮는* 방향이라 더 위험)**
  ① **창이 너무 넓었다** — 슬롯 창 전체(경제는 07:00~21:00, 14시간)를 보면
     **낮에 노트북을 닫은 것만으로 아침의 진짜 실패가 '전원 오프' 가 된다.**
     → 발행이 실제로 일어나야 하는 구간(슬롯 시작 ~ +140분)으로 좁혔다.
     실측 검증에서 08-07 07:00 이 넓은 창이면 323분 공백에 덮였을 것이 좁히니 남았다.
  ② **슬롯 *이전* 의 정지가 안 보였다** — 밤새 꺼뒀다 07:09 에 부팅한 날, 창 안 첫 공백은
     9분뿐이라 "정상 가동" 으로 읽혔다. → 직전 heartbeat 를 첫 마크로 포함.
- **★ 판정 불가일 때는 '꺼져 있었다' 고 하지 않는다** — heartbeat 조회 실패 시 `(False, 0)`.
  진짜 고장을 전원 탓으로 덮는 쪽이, 반대(알림 한 번 더)보다 나쁘다.
- **검증 — 실제 결손 10건이 올바로 갈렸다**:
  ```
  💤 전원 오프(조용)  8건 — 07-30 21시 · 07-31 양쪽 · 08-01 양쪽 · 08-06 21시 · 08-07 양쪽
  🚨 진짜 실패(알림)  2건 — 08-04 07:00 경제 · 08-04 21:00 테마
  ```
  🚨 2건은 **실제로 그날 고친 코드 결함**이다([553] 지침 게이트 과잉차단 ·
  [554] 판정 타임아웃). 기계는 그때 살아 있었다(공백 3~5분).
  골든 4개 신설 · 변이 3/3 검출 · pytest 173 · precommit 20 카테고리 0건 ·
  재시작(PID 7710, 15:49:54) 후 단일 인스턴스·수정본 확인.
- **파일**: `JARVIS00_INFRA/downtime.py` · `JARVIS08_PUBLISH/publish_ledger.py` ·
  `JARVIS04_SCHEDULER/job_registry.py` · `tests/test_error_intake.py`
- **★ 부수 관찰 (미해결 — 별건)**: 재시작이 두 번 실패했다. 데몬 우아한 종료가
  `restart_daemon.sh` 의 대기(3~4초)보다 오래 걸려(실측 ~90초) 새 인스턴스가
  "중복 실행 거부" 로 즉사하고, 그 사이 keeper 가 옛 데몬을 되살려 **인스턴스 2개** 가 됐다.
  종료가 느린 원인은 `[GUARDIAN/store] 파일락 타임아웃 10s` 반복(오늘만 2,180회).
  21:00 발행 전 1개로 정리 완료했으나, **스크립트 대기시간과 파일락 경합은 남은 문제**다.
- **교훈**:
  ① **"실패" 의 정의에 *사용자의 의도* 가 들어가야 하는 시스템이 있다.**
     개인 기기에서 도는 자동화는 "안 돌았다" 와 "안 돌렸다" 가 다르다. 그걸 안 가르면
     완결률이 기계 사용 습관을 뒤쫓고, 진짜 고장이 그 소음에 묻힌다.
  ② **분류 장치가 있어도 *묻지 않으면* 없는 것과 같다.** `reason`·`kind`·severity 등록이
     전부 있었는데 감사 잡이 리터럴 `"audit"` 을 박아 넣어 전부 무력했다
     ([554] 의 `VERIFY_ONLY` 와 같은 모양 — **코드 존재는 적용의 증거가 아니다**).
  ③ **판정 창의 폭이 곧 판정의 정확도다.** 넓히면 진짜 고장이 덮이고 좁히면 정상 지연이
     실패가 된다. 그 폭은 감으로 정하지 말고 *발행이 실제로 필요한 시간* 에서 파생하라.

---

## [582] "왜 CLAUDE.md 를 읽고도 안 지키나" — 사람 손에 맡긴 규칙만 샌다 (2026-08-07)

- **사용자 질문**: "분명히 CLAUDE.md 를 먼저 읽고 숙지한다고 했는데 왜 안 지키나. 새 창을 열면 또 잊더라. 진짜 근본 원인이 뭐냐."
- **사실 정정**: 커밋은 `git add -A` 로 하고 있었다(최근 4회 2·15·13·9 파일, 런타임 산출물 포함). 남은 2개는 *내 커밋 이후* 다른 세션·데몬이 만든 변경이고, **커밋 후 트리를 다시 안 본 것**이 실제 위반이다.
- **근본 원인 3가지**:
  ① **CLAUDE.md 는 "읽는" 게 아니라 세션 시작 시 컨텍스트에 "떠 있는" 것**이다. 떠 있다 ≠ 그 순간 확인한다. 900줄·20도메인 규정이 특정 작업 중에는 주의를 못 받는다. `patch_effective` 원칙이 문서에도 그대로 적용된다 — **존재는 적용의 증거가 아니다.**
  ② **기계가 안 보는 규칙만 샌다.** 실측: `pre-commit` 의 잔여 검사 **0곳**, `precommit_check` 의 커밋 위생 카테고리 **없음**. ①②는 `precommit_check` 가, ③은 2026-08-05 `symmetry` 가 강제하는데 커밋 위생만 사람 손이었다 — 그래서 그것만 반복해서 어긋났다.
  ③ **세션 간 기억이 없다.** CLAUDE.md 는 재로드되지만 *지난번에 뭘 틀렸는지* 는 ERRORS.md 에 있고 그건 검색해야 읽힌다(9,855줄, 통독은 정확도를 떨어뜨림 — ERRORS [534]). 매 세션이 같은 실수를 할 준비가 된 상태로 시작한다.
- **해결 — "더 잘 기억하겠다" 가 아니라 훅에 건다**: `pre-commit` 이 `git status --porcelain` 으로 미스테이징(`^[^MADRC]`)·미추적(`??`)을 검출해 **차단**. 검사기 호출 *앞* 에 둔다(뒤면 위반 시 도달 못 함 — ERRORS [566] 과 같은 함정).
- **검증**: 잔여 생성 → 차단(rc=1) / 스테이징 → 통과(rc=0) / 뮤테이션 1/1 — 검사를 지우면 골든 테스트가 잡는다.
- **파일**: .githooks/pre-commit · CLAUDE.md · tests/test_publish_golden.py
- **교훈**: **사람(과 에이전트)의 기억에 기대는 규칙은 반드시 샌다.** 오늘 하루 종일 고친 병이 정확히 이것이었는데(③원칙만 자동검사가 없어 ③만 샜다), 같은 병이 *규정을 지키는 주체인 나 자신* 에게도 있었다. 답은 언제나 같다 — 기계가 보게 만든다.

## [583] ✅ 해결 — 파일락이 **자기 자신과 교착** 하고 있었다 (모든 쓰기가 10초) (2026-08-07)

- **증상**: 데몬 로그에 `[GUARDIAN/store] 파일락 타임아웃 10s — 락 없이 진행:
  pipeline_activity.json` 이 **누적 2,937회**(하루 ~2,180회). 그 지연이 데몬 우아한 종료를
  **~90초** 로 늘려 `restart_daemon.sh`(대기 3~4초)가 실패했고, 새 인스턴스는 "중복 실행
  거부" 로 즉사하는데 그 사이 keeper 가 옛 데몬을 되살려 **인스턴스 2개** 가 됐다.
- **★ 원인 — 경합이 아니라 *자기 잠금* 이었다 (원칙① 위반)**
  `shared/pipeline_activity.py` 가 `json_store.locked()` 와 **같은 `.lock` 파일**에
  자체 `_file_lock()`(flock)을 따로 걸고 있었다. 그런데 그 안에서
  `_write()` → `json_store.write_json()` → `locked()` 로 **중첩**된다.
  `flock` 은 *open file description* 단위라 **같은 프로세스의 두 fd 도 서로를 막는다.**
  ```
  실측 재현
    단독 locked()                : 0.0001s
    자체 _file_lock 안에서 locked(): 10.0099s   ← 매 쓰기가 여기 걸렸다
  ```
- **★ 가장 아픈 부분 — 교훈은 이미 옆 파일에 적혀 있었다**
  `json_store.locked()` 는 재진입 가드(깊이 카운트)를 갖고 있고 docstring 이
  *"★ 재진입 필수 (비직관 — 안 하면 자기 자신과 데드락)"* 라고 못 박아 두었다.
  `pipeline_activity` 는 그 가드가 없는 **사본** 을 따로 갖고 있었다.
  더구나 2026-08-05 에 *원자 저장*(tmp→os.replace) 사본은 "①위반 시정" 이라며 걷어내면서
  바로 옆의 **락 사본은 남겼다** — 그때 함께 지웠으면 이 사고는 없었다.
- **해결**: 자체 `_file_lock()` 34줄 삭제 + 호출부 5곳을 `json_store.locked(_DATA_FILE)` 로.
  `fcntl` import 도 제거(유일 소비자였다). `threading.Lock` 은 **남긴다** — flock 재진입
  가드는 깊이만 세지 스레드 배타를 주지 않는다.
- **검증 (실환경)**
  ```
  쓰기 5회 : 회당 10s → 0.0005s   (약 20,000배)
  재시작 후 안정 상태 5분 : 락 타임아웃 0건 (종전 기대 ≈22건)
  ```
  · 잔존 6건은 **옛 데몬이 아직 종료 중이던 겹침 구간**(단일 수렴까지 115초) 한정 —
    진짜 교차 프로세스 경합이고 자기 잠금이 아니다.
  · 골든 2개 신설(동작 기반 + 사본 재발 검출) · 변이 2/2 검출 · pytest 177 ·
    precommit 20 카테고리 0건 · 재시작(PID 13342, 16:24:56) 후 확인.
- **함께 고친 것 — Tier-2 브리지 회귀 테스트 (다른 세션이 고친 코드의 *방어*)**
  `guardian_agent.py:486` 의 `(error_record.get('traceback','') or '')[:2000]` 수정은
  이미 적용돼 있었으나 **되돌려도 아무도 못 잡는 상태** 였다.
  ★ **다른 세션이 제안한 테스트("TypeError 가 안 난다")는 무력하다** — 그 아래
  `except Exception` 이 예외를 잡아 기록만 하고 밖으로 던지지 않기 때문이다.
  메모리 뮤턴트로 실증: *브리지 도달* 은 True/False 로 갈리는데 *예외 전파* 는 양쪽 다 False.
  → **판별식은 `run_auto_repair_targeted` 도달 여부 하나뿐**. 그 형태로 작성했고
  변이(`or ''` 제거)에서 실제로 실패하는 것을 확인했다.
- **파일**: `shared/pipeline_activity.py` · `tests/test_error_intake.py`
- **교훈**:
  ① **"경합" 이라고 부르기 전에 *혼자서도 나는지* 확인하라.** 2,937건이 전부 자기 잠금이었다.
     상대가 없는데 기다리고 있었다.
  ② **사본을 걷어낼 때는 *옆에 있는 것까지* 보라.** 원자 저장 사본은 지우고 락 사본은
     남긴 이틀이 이 사고를 만들었다. 같은 파일, 같은 문단이었다.
  ③ **회귀 테스트의 단언이 `except` 뒤에 있으면 그 테스트는 통과만 한다.**
     "예외가 안 난다" 는 예외를 삼키는 코드 앞에서 아무 의미가 없다 —
     *관측 가능한 결과*(여기서는 브리지 도달)로 단언할 것.
  ④ **재시작이 느린 것은 증상이지 원인이 아니다.** 종료 90초를 스크립트 대기시간으로
     맞추려 했다면 진짜 원인(락)은 그대로 남았을 것이다.

---

## [584] ✅ 해결 — `.get(키, 기본값)` 이 **NULL 앞에서 무력** 했다 (22곳 + 사전 검사 신설) (2026-08-07)

- **병의 정체 (파이썬 함정)**: `dict.get(k, D)` 는 **키가 없을 때만** D 를 쓴다.
  키가 *있고 값이 None* 이면 None 을 그대로 돌려준다 → `[...]` 에서
  `TypeError: 'NoneType' object is not subscriptable`.
  **DB 의 NULL 이 정확히 이 꼴로 들어온다** — `error_log.traceback` 실측 **4,159/5,076(82%)** NULL.
  즉 *"키가 없으면"* 은 막았는데 *"값이 비어 있으면"* 은 안 막은 것이고, 이 시스템의
  데이터는 후자가 압도적이다.
- **★ 이미 치른 값 — 추상적인 얘기가 아니다**
  같은 병이 `guardian_agent._try_sdk_targeted_fix` 에서 터져 **Tier-2(LLM 수리) 브리지가
  통째로 막혔다**. 7/27 이후 llm 시도 결과:
  ```
  wontfix  18건   ← 프롬프트 조립 중 터져 시도조차 못 됨
  fixed     4건
  ```
  밴딧 강화학습의 `llm` arm 은 **유일한 보상 경로가 끊긴 채** 돌고 있었다.
  (그 1곳은 다른 세션이 선수정 — 이 항목은 *나머지 22곳* 과 재발 방지다.)
- **왜 위험한가 — 조용히 죽는다**: 대부분 `except` 에 잡혀 그 층만 일을 안 한다.
  뜨거운 경로가 섞여 있었다 — `error_fixer:332`(모든 자동수리) ·
  `pattern_fixer:961·1054`(Tier-1 매번) · `post_quality_analyzer:553-554`(발행 글마다) ·
  `plotly_renderer:328`(이미지마다) · `naver_collector:76`(수집마다).
- **★ 헛다리 — "인덱스 접근은 `or` 로 안 낫는다" 는 반증됐다**
  조사 단계에서 *기본값이 비어있지 않은 4곳(`[[]]`·`[{}]*n`·`['','']`·`(0,0)`)은
  기계적 치환 금지* 라는 경고가 나왔다. 실증(12/12)으로 **틀렸다**:
  ```
  값=None  종전 TypeError  → or가드 정상
  값=[]    종전 IndexError → or가드 정상   ← or 는 falsy 전부를 기본값으로 바꾼다
  값=정상   종전·or가드 동일
  ```
  `or` 는 None 뿐 아니라 **빈 컬렉션까지** 기본값으로 대체하므로 인덱스 접근의
  IndexError 도 함께 막는다. → **단일 패턴** 으로 갈 수 있었다(①원칙에도 부합).
- **★ 내 일괄 치환 스크립트에 결함이 있었다 (자기 검증이 잡음)**
  `new.replace(old, rep, 1)` 로 치환했는데 이건 **파일의 첫 일치**를 바꾼다.
  `plotly_renderer` 에 같은 문자열이 245·328 두 곳 있어 **245(무관)를 고치고 328(대상)을
  남겼다.** diff 를 줄 단위로 대조해 발견 → 328 은 손으로 정확히 고쳤다
  (같은 표현식 안에서 `spec.get("data")` 를 **두 번** 부르던 것도 한 번으로 정리).
  245 는 되돌리지 않았다 — 거기도 `units[i]` 를 쓰므로 가드가 이득이다.
- **해결**
  - **22곳** `d.get(k, D)[...]` → `(d.get(k) or D)[...]` (AST 로 위치 특정, 문법 검증 후 기록)
  - **면제 1곳** `os.environ.get('PATH','')[:200]` — 환경변수 값은 항상 str, 언어 계약상 None 불가
  - **사전 검사 신설** `precommit --category symmetry` 의 `symmetry/get-default-unguarded`
- **★ 왜 *사전* 검사인가**: `pattern_fixer._fix_none_slicing` 이라는 **사후 수리기**는
  이미 있었다. **터진 다음 고치는 코드는 있는데 들어오는 걸 막는 코드가 없었다.**
  22곳을 고쳐도 내일 23번째가 들어오면 그만이다.
- **★ 순서를 지켰다 (안 지키면 저장소가 잠긴다)**
  pre-commit 훅에 경고 모드가 없다. 검사를 먼저 켜면 22곳이 즉시 위반이 되어
  **그 순간부터 아무도 커밋을 못 한다**(동시 작업 중인 다른 세션까지). 그래서
  **22곳 수정 → 잔존 0 확인 → 그다음 검사 신설** 순으로 했다.
- **검증**
  ```
  변이 3/3 검출 — ① 가드 되돌림 ② 새 파일로 위반 도입 ③ os.environ 오탐 없음
  precommit 20 카테고리 0건 · 소요 3.44s → 3.59s (+0.15s, 2-phase 사전필터)
  pytest 179 · 수정 모듈 import 11/11 · 재시작(PID 23178, 17:18:05) 후 오류 0·파일락 0
  ```
- **파일**: `bot.py` · `seo_learner.py` · `seo_standards.py` · `analyzer.py` ·
  `naver_collector.py` · `post_quality_analyzer.py` · `design_learner.py` ·
  `plotly_renderer.py` · `auditor.py` · `auto_repair.py` · `error_fixer.py` ·
  `pattern_fixer.py` · `chart_data.py` · `measure_collection.py` ·
  `shared/precommit_check.py` · `tests/test_error_intake.py`
- **교훈**:
  ① **기본값을 적어 뒀다고 안전한 게 아니다.** `.get(k, D)` 는 *키 부재* 만 막는다.
     NULL 이 흔한 스키마에서는 **거의 아무것도 막지 못한다** — 82% 가 그랬다.
  ② **일괄 치환은 위치로 하라, 문자열로 하지 말고.** `replace(old, new, 1)` 은 첫 일치를
     바꾼다. 같은 표현이 두 번 나오는 파일에서 **엉뚱한 곳을 고치고 대상을 남긴다.**
     AST 로 찾았으면 AST 위치로 바꿔야 한다.
  ③ **사후 수리기가 있다는 건 사전 검사가 없다는 뜻이다.** `_fix_none_slicing` 의 존재
     자체가 "이 병이 반복된다" 는 증거였는데, 아무도 그걸 *들어오는 문* 으로 옮기지 않았다.
  ④ **검사를 켜는 순서가 곧 가용성이다.** 경고 모드가 없는 훅에서는
     *고치고 → 켠다* 말고 다른 순서가 없다.

---

## [585] ✅ 해결 — 블로그 품질 학습의 배관 7곳이 새고 있었다 (2026-08-08)

- **배경**: 사용자 지시 — *"학습이 잘 되고 잘 적용돼서 블로그 품질이 높아지는가"*.
  감사 결과 **루프는 돌지만 배관 7곳이 샜다.** 점수는 오르는데(t=13.85) 오르는 것이
  *검사 통과 점수* 였고 독자 체감 품질이 올랐다는 증거는 없었다.
- **★ ① 결측을 0 으로 채워 91%를 '나쁜 사례' 로 만들고 있었다 (가장 깊은 것)**
  ```
  naver   438행: actual_views 전부 0  · naver_rank 전부 관측
  tistory  42행: actual_views 전부 >0 · naver_rank 전부 NULL
  ```
  **두 신호가 정확히 상보적** 인데 `build_target` 이 하나만 골랐다. `42 >= min_signal(20)`
  이라 조회수를 채택했고, `float(r["actual_views"] or 0)` 이 네이버 438행을
  **"조회수 0 = 나쁨"** 으로 만들었다 → `trend_score` 계수 음수 → `max(0,v)` 절단 →
  `w_trend=0.0` → **정렬키 사망**(opportunity_score 고유값 07-22 **48종** → 07-23 **6종**
  → 지금 50개 중 **49개가 4.5 동점**).
  · 결측은 NaN 으로 표시해 학습에서 제외 — **0 은 "나빴다" 이지 "모른다" 가 아니다**
  · 두 신호를 **그룹 내 백분위** 로 통일해 함께 사용 (척도가 달라도 상대 위치는 비교 가능)
  · 음수 절단 제거 → **결과(변별력)로 판정**, 정렬키를 죽이면 학습 거부
  실측: y 고유값 1→**67종** · 표본 42→**480행** · 점수 고유값 2→**117종**
- **★ ② 장치는 있는데 관문이 하나 비어 있었다**
  `pipeline_controlled_items`(08-07 신설)가 `weak_items`(새 지침 *생성*)에만 걸려,
  **이미 weight 를 쌓은 지침은 그대로 주입**됐다. 장치 신설 이후에도 메타 지침 5건 주입,
  `id=704(theme w=2.097)`·`id=669(economic w=2.096)` 가 최상위. 작성 LLM 은 메타 설명을
  만들 수 없다(발행자에 `meta_desc` 0건, `document.body.innerHTML` 에만 쓴다 —
  발행글 212편 전부 meta 태그 0편). 시켜봐야 **지어내는 것** 뿐이다.
  → 주입 관문(`build_insights_block`)에도 필터. 부품은 이미 있던 것 재사용(①).
- **★ ③ 반복이 보상받고 있었다**
  본문 첫 문단 감성 상투구 **32.9% → 70.1%**. `_AI_OPEN` 이 `퇴근길에…`·`요즘…` 을
  전부 통과시키고 'AI 회피' 보너스를 줬다. → 금지어를 박지 않고 **최근 발행글에서 파생**.
- **④ 학습 게이트가 항등식**: `score = 70 - 15×fail` · `ok = score >= 70` → `fail=0` 이면
  항상 참. 패턴 54개 중 **49개(91%)** 가 LLM 판정 없이 등록, 사유 1위가
  `No module named 'dotenv'` **19건** — LLM 실패가 아니라 **환경 결함**.
  → 구조적 실패는 통과 금지·박제. 일시적 실패는 종전대로(원 의도 보존).
- **⑤ 주 1회 학습 잡이 13일 정지**: 유예 1~2시간뿐이라 08-02(일) 02~06시 수면 중
  3개가 유실, 다음 기회가 **일주일 뒤**. → 유예를 **주기에서 파생**(1/4=42h).
  발행 잡은 제외 — *시각이 계약* 이라 정책 A와 충돌.
- **⑥ 가장 값진 자산의 사본이 0개**: `learned_patterns.json`(523KB)·`bandit_state.json`
  이 git 밖 + DB 아님. → 매일 백업 잡에 태움. 목록을 박지 않아 `design_recipes.json` 등
  4개 자동 포함.
- **⑦ 폴백은 학습 0 으로 발행**: 주 경로만 지침을 주입하고 폴백 4함수는 0회.
  → 공통 조립 지점 한 곳에 넣어 넷을 동시에 덮음.
- **★★ 헛다리 — 이번에도 내 손이 네 번 틀렸고 검증이 잡았다**
  ① **상투구 감점 초판이 전부 0.00** — 2어절 n-gram 만 봐서. 실측하니 3어절 공유 **0종**,
     **첫 어절은 40편 중 23편(58%)이 단 6개 단어**. 상투구는 문장 *앞* 에 산다.
  ② **부호 음수를 거부로** 만들었다가 철회 — `freshness` 음수는 진짜 발견일 수 있다
     (갓 나온 키워드는 검색 노출이 없다). **원래 코드가 박은 가정을 나도 똑같이 박았다.**
  ③ **T7 배점 제거** 를 했다가 철회 — 다른 세션 설계와 중복이고 의도적 테스트 3개를 깼다.
     진짜 구멍은 배점이 아니라 *주입 관문* 이었다.
  ④ **회귀 테스트 2개가 허수아비** — **내 주석 문구** 에 속았다(오늘 네 번째). 동작 검사로 교체.
     그리고 변이 시험에서 `replace(...,1)` 이 **엉뚱한 곳**을 바꿔 "테스트가 안 문다" 고
     오판했다 — 같은 문자열이 3곳이었다. AST 로 위치를 특정해 재시험하니 정상.
- **검증**: 신규 골든 15개 · **변이 8/8 검출** · pytest 239 · precommit 20 카테고리 0건 ·
  배점 드리프트 0 · 재시작(18:01:19) 후 7개 지표 전수 확인.
- **파일**: `JARVIS03_RADAR/learning.py` · `JARVIS02_WRITER/post_scorer.py` ·
  `JARVIS07_GUARDIAN/quality_learner.py` · `JARVIS07_GUARDIAN/eval_agent.py` ·
  `JARVIS04_SCHEDULER/job_prereq.py` · `shared/db.py` · `JARVIS02_WRITER/draft_writer.py` ·
  `tests/test_learning_signal.py`(신규)
- **교훈**:
  ① **결측과 0 은 다르다.** "모른다" 를 "나빴다" 로 적으면 학습은 그 91%를 열심히 배운다.
     이 한 줄(`or 0`)이 주제 랭킹을 3주간 상수로 만들었다.
  ② **가정을 코드에 박으면 데이터가 말하는 것을 못 듣는다.** `max(0,v)` 의 주석은
     "전부 클수록 좋음" 이었는데 그 가정 자체가 틀렸다. 나도 같은 실수를 반복했다 —
     **가정이 아니라 결과로 판정하라.**
  ③ **장치가 있어도 관문이 하나 비면 그대로 샌다.** 08-07 에 만든 필터가 *생성* 만 막고
     *주입* 은 안 막아, 만들어 둔 지침이 계속 나갔다([554]·[581] 과 같은 모양).
  ④ **테스트가 자기 설명 문구에 속는 일이 오늘만 네 번.** 구조를 볼 일에는 AST,
     동작을 볼 일에는 실행. 소스 문자열 검사는 주석·docstring·지역변수에 전부 속는다.

---
## [586] ✅ 해결 — 네이버 조회수는 **공개 페이지에 애초에 없었다** (3개월 0건의 진짜 이유) (2026-08-08)

- **증상**: `post_analysis.current_views` 가 네이버 46편 중 **2편만 >0**, 합계 3.
  티스토리는 15편/25 로 정상. `radar_perf` 잡은 89회 실행 중 88회 "성공" 이었다 —
  **오류를 내지 않으면서 아무것도 못 가져오고 있었다.**
- **환경**: `JARVIS03_RADAR/performance_collector.py` `_collect_naver_views`,
  네이버 블로그(개인), 2026-08-08.
- **원인**: 종전 구현은 `m.blog.naver.com` 공개 페이지를 받아 정규식 8종으로 조회수를
  찾았다. 실측하니 **응답 10만자 안에**
  ```
  "조회" 0회 · visitorCount 0회 · viewCount 0회
  ```
  조회수가 **문서에 존재하지 않는다.** 네이버는 글별 조회수를 공개 렌더에 넣지 않고
  관리자 통계로만 준다. → 패턴을 몇 개를 더 붙여도 없는 값은 못 찾는다.
  · **왜 3개월간 안 드러났나** — `_collect_naver_views` 가 못 찾으면 `0` 을 돌려줬고,
    0 은 "조회수 0회" 와 구분되지 않았다. 잡은 초록불, DB 엔 0. **실패가 정상값의 탈을
    쓰면 아무도 모른다.** ([581] 의 "전원 오프 vs 진짜 실패" 와 같은 병.)
  · 이 0 이 [585] ① 의 재료였다 — 네이버 438행 `actual_views=0` 이 학습기를 통째로 망가뜨렸다.
- **헛다리** (⛔ 다시 시도 금지):
  1. **공개 페이지 정규식 보강** — 없는 값을 찾는 일. 패턴 문제가 아니다.
  2. **API 주소 열거** — `BlogStatistics.naver` · `api/blogs/{id}/post-views` ·
     `BlogStat.naver` · `StatisticsMain.naver` · `MyStatistics.naver` ·
     `blog/daily/post/cv` · `blog/rank/post/cv` · `stat/post` → **전부 404.**
     [279] 가 이미 *"URL 형태 열거 = 헛다리"* 로 박제한 그 길을 그대로 다시 걸었다.
  3. `admin.blog.naver.com/{uid}/stat/today` 를 **그대로 읽기** — body 가 539자뿐.
     실데이터는 iframe 8개 중 하나 안에 있다.
- **해결**: 추측을 멈추고 **페이지가 알려주는 링크를 따라갔다.**
  ```
  admin.blog.naver.com/{uid}/stat/today
    → iframe  blog.stat.naver.com/blog/daily/daily/cv      (블로그 전체)
    → 인기글 링크 blog.stat.naver.com/blog/article/{logNo}/cv   ★ 글별
  ```
  그 페이지에 `날짜 조회수` 표가 있고 `2026.08.05. (수) 7` 꼴로 일별 값이 온다.
  구현 = `_naver_log_no()` + `_collect_tistory_stats_batch` 와 **같은 자리·같은 형태**의
  `_collect_naver_stats_batch()`. 브라우저는 한 번만 띄워 전부 순회한다.
  · **레이블로 읽고 위치로 읽지 않는다** — 통계 페이지엔 조회수·공감수·댓글수가 섞여
    나온다. "N번째 숫자" 로 집으면 네이버가 항목 하나만 늘려도 **엉뚱한 값이 조용히
    학습에 들어간다**. `YYYY.MM.DD. (요일) 숫자` 행 꼴만 취해 합산한다.
  · **미수집을 0 으로 단정하지 않는다** — 통계 화면이 안 열리면(로그인 만료 등) 캐시에
    담지 않는다. 이번 사고가 정확히 "실패를 0 으로 적어" 생긴 것이다.
- **파일**: `JARVIS03_RADAR/performance_collector.py` · `tests/test_learning_signal.py`
- **검증 (실계정 실측)**:
  ```
  통계 배치         30/31건 수집
  네이버 조회수>0    2편(합계 3) → 25편(합계 225)
  learn_log        네이버 조회수신호 0 → 11
  train_weights    n=495 · r²=0.450   (종전: 신호 소실로 학습 불가)
  ```
  골든 4개 · **변이 4/4 검출**(공개페이지 회귀 / 위치기반 파싱 / 로그인가드 제거 /
  logNo 추출 약화) · pytest 249 · precommit 20 카테고리 0건
- **교훈**:
  ① **"찾지 못함" 과 "0" 을 같은 값으로 적으면 고장이 영원히 안 보인다.** 이 저장소가
     오늘 이 병을 세 번 봤다 — [581] 전원오프, [585] ① 결측, 그리고 여기.
     결측은 **결측으로 남겨야** 하류(학습기)가 제외할 수 있다.
  ② **주소를 짐작하지 말고 페이지에 물어라.** 404 세 번이면 방법이 틀린 것이다.
     로드된 문서의 링크·iframe 을 열거하는 데 든 시간이, 추측에 쓴 시간보다 짧았다.
  ③ **초록불이 성공의 증거가 아니다.** 88/89 성공률이 3개월간 0건 수집을 가리고 있었다.
     외부에서 값을 가져오는 잡은 *가져온 개수* 를 성공 판정에 넣어야 한다.

---
## [587] ✅ 해결 — 테스트 4개가 **내 맥북에만 있는 파일**에 기대 CI 에서만 빨갰다 (2026-08-08)

- **증상**: PR #62 의 GitHub Actions `동작 검증 (pytest)` 이 **4 failed / 245 passed**.
  로컬은 249 전부 통과. `precommit_check`·`Python Syntax` 는 초록.
- **환경**: `tests/test_publish_golden.py`(3건) · `tests/test_learning_signal.py`(1건),
  GitHub Actions ubuntu / Python 3.10.20.
- **원인**: 네 테스트 모두 **git 이 추적하지 않는 파일**을 읽거나 그 존재를 전제했다.

  | 테스트 | 기댄 것 | CI 에 없는 이유 |
  |---|---|---|
  | `학습자산이_백업에_동반된다` | `JARVIS07_GUARDIAN/learned_patterns.json` | `.gitignore` — 런타임 학습 산출물 |
  | `트렌드검증이_정적시드에_속지_않는다` | `JARVIS03_RADAR/data/` **폴더** | `data/*.json` 무시라 폴더째 없음 → `write_text` 가 FileNotFoundError |
  | `eval_훅이_venv_파이썬을_쓴다` | `.claude/settings.json` | `.gitignore:60` — 로컬 Claude Code 설정 |
  | `학습원장에_복구경로가_있다` | 실 원장을 다시 써서 `.bak` 생성 | 원본이 없으니 백업할 것도 없음 |

  · **왜 로컬에선 안 보였나** — 이 파일들은 *운영 중 자연히 생기는 것* 들이라 내 맥북엔
    항상 있다. 테스트를 쓸 때 "있는 게 당연" 해서 의존인 줄 몰랐다.
  · **이미 아는 병이었다** — ERRORS [568] 이 *"DB 를 읽는 테스트는 내 맥북에서만 통과가
    된다"* 로 박제했고, 그 교훈으로 만든 `test_publish_dedupe.py` 는 실제로 이 함정을
    피했다(docstring 에 '기계 독립' 이라 적혀 있다). **교훈이 한 파일에만 적용됐다** —
    같은 저장소의 다른 테스트 파일로는 건너가지 않았다.
  · **④ 는 더 나빴다** — `_save_learned(_load_learned())` 로 **운영 학습 원장을 덮어썼다.**
    검사가 부작용을 남기고 있었다.
- **헛다리** (⛔ 다시 시도 금지):
  1. **CI 에 파일을 만들어 주기**(워크플로에 `mkdir`·더미 JSON 추가) — 검사가 *합성 데이터를
     보고 통과* 하게 되어 보호력이 0 이 된다. 고칠 곳은 CI 가 아니라 테스트다.
  2. **`.gitignore` 에서 빼기** — `.claude/`·학습 원장 무시는 *의도적 결정* 이다
     (팀원 빈 상태가 운영 학습을 덮어쓰는 것 차단, 2026-06-07 박제). CI 를 위해 그 결정을
     뒤집지 않는다.
  3. **그냥 skip 으로 도배** — 없으면 넘어가게만 하면 *CI 가 아무것도 검사하지 않는* 채
     초록이 된다. skip 은 저장소가 **원리적으로** 가질 수 없는 것에만 쓴다(③ 한 건뿐).
- **해결 — 검사 대상을 '내 컴퓨터의 파일' 에서 '규칙' 으로 옮겼다**:
  ① `learning_asset_files(root=None)` 로 탐색 뿌리를 인자화하고, 테스트는 **합성 트리**를
     넘긴다. 대상 3종·비대상 3종을 심어 *꼴로 집어내는가* 를 본다. 목록·패턴은
     `LEARNING_ASSET_DIRS`/`LEARNING_ASSET_PATTERNS` 상수에서 파생(②).
     운영 경로용으로 `test_백업자산_탐색이_실저장소에서도_돈다` 를 따로 두되
     **존재를 단정하지 않고 규약**(실존·소유폴더·패턴부합·저장소 안)만 본다.
  ② 폴더가 없으면 만들고, **우리가 만든 경우에만** 되돌린다(저장소에 흔적 0).
  ③ 설정 부재 시 **사유를 밝히고 skip**. 저장소가 갖지 않은 것을 저장소 검사가 보장할 수
     없다. venv 부재도 같은 방식(폴백 분기는 문자열 검사로 이미 확인).
  ④ `tempfile` 에 두 세대를 써서 **`.bak` 이 직전 세대인지 내용까지** 확인한다.
     실 원장은 건드리지 않는다.
...
- **교훈**:
  ① **"로컬 통과" 는 통과의 증거가 아니다.** 저장소 검사는 *저장소가 가진 것* 만 봐야 한다.
     ...
     CI 를 기다려 알아내지 말 것.
  ② **교훈은 파일 단위로 적용되지 않는다.** [568] 을 알고 그 대책까지 만들어 둔 상태에서,
     같은 사람이 다른 파일에 같은 병을 넣었다. 원칙③(모든 통로에 적용)이 *코드* 뿐 아니라
     *테스트* 에도 걸린다.
  ③ **테스트가 운영 자산을 쓰면 안 된다.** ④ 는 통과하면서 학습 원장을 덮어쓰고 있었다.

---
## [588] ✅ 해결 — 검사기를 **통째로 지워도 초록**이던 테스트 2건 (내가 쓴 것) (2026-08-08)

- **배경**: [587] 로 "CI 에서만 빨간" 테스트 4건을 고친 뒤, *같은 병이 다른 곳에도
  있는가*(원칙③)를 11개 에이전트로 전수 감사했다. **14건**이 나왔고 그중 2건이
  내가 어제 쓴 `tests/test_error_intake.py` 였다. 이쪽은 [587] 과 반대다 —
  **CI 도 로컬도 초록인데 아무것도 검사하지 않았다.**
- **증상 ① `test_저장소에_미가드_get_첨자가_없다`**:
  `subprocess` 로 `precommit_check.py --category symmetry` 를 돌리고
  `"get-default-unguarded" not in r.stdout` 을 단언했다.
  그런데 `main()` 은 위반을 **전부 `file=sys.stderr`** 로 찍는다. stdout 엔
  성공 문구 한 줄만 간다. **위반 문자열이 stdout 에 닿을 경로가 없다.**
  - 변이 실증: `mv shared/precommit_check.py` 로 **검사기를 통째로 치우고** 실행 → `1 passed`.
    진짜 위반(`rec.get("msg","")[:50]`)을 심어도 → `2 passed`.
    검사기가 실제로 잡아냈는데(stderr 에 출력, exit 1) 테스트는 초록이었다.
- **증상 ② `test_get_미가드_검사가_실제로_잡는다`**:
  "검사기가 허수아비가 아닌지 본다" 는 이름으로, **판정 AST 로직을 테스트 안에 베껴**
  그 사본을 채점했다. 배포 검사기를 한 줄도 부르지 않는다.
  - 같은 변이(검사기 삭제)에서 역시 `passed`.
  - **바로 위 테스트가 선언한 원칙과 스스로 어긋났다** — ①의 docstring 은
    *"검사기와 같은 판정을 다시 쓰지 않는다(원칙①). 판정 로직이 두 벌이 되면 서로 어긋난다"*
    라고 적혀 있는데, ②가 정확히 그 두 벌째를 만들었다.
    CLAUDE.md **'복사본을 진실로 믿지 말 것'** 정면 위반.
- **원인 (공통)**: **검사 대상이 아니라 검사 *흔적* 을 봤다.**
  ①은 프로세스의 stdout 이라는 *부산물* 을, ②는 로직의 *사본* 을 봤다.
  둘 다 "진짜가 죽어도 흔적은 남는(혹은 안 남는) " 자리라 판정이 성립하지 않는다.
- **헛다리** (⛔ 다시 시도 금지):
  1. **`r.stdout` → `r.stdout + r.stderr` 로 바꾸기** — 여전히 종료코드·스트림에 기댄다.
     프로세스가 import 단계에서 죽으면 두 스트림 다 비고 다시 조용히 통과한다.
     실제로 [587] 의 프로브가 그렇게 죽어 *엉뚱한 진단* 을 냈다.
  2. **저장소 안에 임시 `.py` 를 만들어 검사기를 돌리기** — 다른 세션이 동시에 커밋 중이라
     그 순간 위반 파일이 커밋될 수 있다(실제로 오늘 여러 번 서로의 파일이 섞여 커밋됐다).
  3. `monkeypatch.setattr(pc, "ROOT", tmp)` **만** 하기 — `_iter_py(root=ROOT)` 의
     기본값은 *정의 시점* 에 묶여 있어 안 먹는다. 순회 자체도 갈아끼워야 한다(실측).
- **해결 — 배포되는 그 객체를 부른다**:
  ① `from shared.precommit_check import run` → `run(["symmetry"])` 의
     **`Report.violations`** 를 본다. 스트림도 종료코드도 안 쓴다.
     `assert "symmetry" in rep.ran` 으로 *검사가 실제로 돌았는지* 도 함께 본다
     (카테고리 이름이 바뀌면 조용히 0건 통과하는 구멍 차단).
  ② `monkeypatch` 로 `pc.ROOT` 와 `pc._iter_py` 를 **둘 다** tmp_path 로 돌리고
     `pc.check_symmetry(Report())` 를 직접 호출한다. 위반/정상/면제 3종을 먹여
     `check_id == "symmetry/get-default-unguarded"` 로 판정. 저장소엔 파일 0개.
- **파일**: `tests/test_error_intake.py`
- **검증**: **변이 3/3 검출** (초판은 A 를 통과시켰다)
  ```
  A 검사기 통째로 삭제            → 2 failed  (초판: 2 passed)
  B 판정에서 Subscript 조건 제거   → 2 failed
  C os.environ 면제 제거(오탐)     → 2 failed
  복원 → 2 passed · CI 모사 252 passed/1 skipped · 로컬 253 · precommit 0건
  ```
- **★ 감사가 찾은 나머지 12건** (이번 커밋 범위 밖 — 별건 처리):
  | 테스트 | 증상 |
  |---|---|
  | `test_시크릿_파일_권한이_소유자전용` | CI 에서 **단언 0줄 실행**. `naver_cookies.pkl` 은 로컬에도 없어 그 단언은 어디서도 안 돈다 |
  | `test_감사는_발행락을_지우지_않는다` | `.posting.lock` 부재 → `False == False` 공허 통과 |
  | `test_도달불가_지문이_매칭후보에서_빠진다` | 로컬 26건 검사 / CI 0건 검사 (원장이 `.gitignore`) |
  | `test_시크릿모듈이_env를_스스로_적재한다`·`test_마스킹이_실제로_먹는다` | **단독 실행 시 실패**. 전체 실행 땐 `shared/llm.py` 의 `os.environ.setdefault` 센티널 덕에 통과하는데, 그때 검사 대상은 저장소에 평문으로 박힌 공개 상수 1개뿐 |
  | `test_사실성_게이트가_거짓수치를_막는다` 등 smoke 2건 | `.env`·네트워크 의존 |
- **교훈**:
  ① **"검사기를 지워도 통과하는가" 가 테스트의 최소 자격 시험이다.** 새 회귀 테스트를
     쓰면 *보호하려는 대상을 없애 보고* 빨개지는지 확인한다. 5초면 된다.
  ② **부산물을 보지 말고 반환값을 보라.** stdout·종료코드·로그는 진짜가 죽어도
     그럴듯한 모습을 남긴다. in-process 호출이 가능하면 언제나 그쪽이다.
  ③ **원칙 위반은 옆줄에서 난다.** ①이 "판정을 두 벌 만들지 말라" 고 적어 둔 바로
     아래에서 ②가 두 벌째를 만들었다. 규정을 *쓰는 것* 과 *지키는 것* 은 다른 일이다.

---
## [589] ✅ 해결 — 대시보드가 화면 밖으로 삐져나갔다 · `1fr` 은 "똑같이 나눠라" 가 아니다 (2026-08-08)

- **증상**: 사용자 신고 — 홈 "일별 출력 추세" 의 마지막 두 막대(08-07·08-08)가 **카드 오른쪽
  경계 밖**으로 나감. 전 페이지 실측하니 5개 페이지 **88건**, 홈은 페이지 자체에 가로
  스크롤(+23px)까지 생겨 있었다.
- **환경**: `dashboard/` Next.js(:9199), 실제 Chrome 헤드리스 계측.
- **원인 — 기본값 두 개가 겹쳤다**:
  ```
  선언  gridTemplateColumns: "1fr 1fr"
  실측  600.9px  +  721.5px      ← 같아야 할 두 칸이 120.5px 차이
  ```
  ① **`1fr` 은 `minmax(auto, 1fr)`** 이다. *최댓값* 만 "남는 공간의 1/n" 이고 **최솟값이
     `auto`(내용이 요구하는 폭)** 라, 칸이 내용보다 작아지지 못한다.
  ② **flex 자식의 `min-width` 기본값도 `auto`** 다. `flex:1` 이어도 내용 폭 아래로 안 준다.
     막대 45.8px < 라벨 47px — 라벨이 각 칸의 바닥을 떠받치고, 그 합이 칸의 최소 폭이 되고,
     그 칸이 그리드를 밀어냈다.
  · 그래서 **창을 줄여도 차트가 안 줄어든다**: 그리드 총폭이 1340px 에 고정된 채 창만 좁아진다.
    1280px → +138px, 1152px → +266px 초과.
  ③ Recharts `ResponsiveContainer width="100%"` 는 폭 측정용 `width:0` div 를 만드는데,
     부모가 못 줄면 이전 폭을 유지해 **+1256px** 을 만든다(같은 수치가 3개 페이지에 반복).
  ④ 표는 `maxWidth + nowrap + ellipsis` 라 긴 경로·메시지가 줄바꿈되지 못해 최대 +673px.
    이건 넘침인 동시에 **가림**이었다(59칸의 값이 안 보이고 있었다).
- **★ 헛다리 — 내가 처음 쓴 측정 기준이 틀렸다** (⛔ 그대로 쓰지 말 것):
  `scrollWidth - clientWidth > 1` 은 **`overflow:hidden`+`ellipsis` 로 의도적으로 잘린
  글자까지 넘침으로 잡는다.** 화면엔 안 삐져나가는데 지표는 빨갛다. 그 지표를 보고
  이미 정상인 곳을 고치려 들었다. 사용자가 신고한 것은 "화면 밖으로 나감" 이므로 기준은
  **시각적 이탈** 이어야 한다 — `rect.right > main.right` + 페이지 가로스크롤,
  단 조상에 `overflow-x: hidden|auto|scroll` 이 있으면 제외(가둬져 있으니 눈에 안 보인다).
- **★ 수정 중에 내가 만든 회귀 3건 — 검증자가 잡았다**:
  1. `/errors` 막대행에 `overflowX:auto` 를 주자 **CSS 규칙상 `overflow-y` 도 auto 로 승격**
     돼 가로 스크롤바 6px 이 고정 `height:80` 을 갉아먹었다. 값 라벨이 18px 중 8px 만 남고,
     `scrollHeight == clientHeight` 라 **세로 스크롤조차 안 생겨 읽을 방법이 사라졌다**.
     → `height` → `minHeight` (스크롤바 자리를 높이가 흡수). 실측 clientH 74 → 84.
  2. 홈 그리드 최소폭을 *막대 개수에서 파생*(15개 × 7ch ≈ 840px)했더니 1680px 화면에서도
     2단이 성립하지 못했다 — 넘침은 고쳤는데 **2단 디자인이 폐지**됐다.
     막대는 이미 카드 안에서 스크롤되므로 열이 막대 전체를 담을 필요가 없다.
     → 열 최소폭은 "막대가 다 들어갈 폭" 이 아니라 "카드가 읽을 만한 폭"(`CHART_COL_MIN`).
  3. 표에서 `nowrap` 을 뗀 열이 폭을 독점해 **메시지 열이 36px 로 짜부라졌다**
     (1024px, 행 높이 523px, 글자가 1~2자씩 세로로 흐름).
     → 마지막 열 `width:100%`(남는 공간 흡수) + 모듈 열 `minWidth 14ch ~ maxWidth 22ch`.
     · `overflowWrap:break-word` 는 **min-content 를 낮추지 못해** 표가 안 줄어든다.
       줄이려면 `anywhere` 가 필요한데 그건 경로를 단어 중간에서 가른다 — 그래서 상·하한을 함께 준다.
- **해결 원칙**: 정보를 잘라서 0 을 만들지 않는다. 줄바꿈·축소 허용·*의도된* 가로 스크롤로
  해결하고, 부득이 자를 때는 `title` 로 전문을 남긴다. 실제로 이번 수정은 **보이는 정보가
  늘었다** — 기존 은닉 3곳(오류 메시지 말줄임·칩 작업명·Y축 라벨 3px 잘림)을 없앴다.
- **파일**: `dashboard/app/{page,errors,learning,performance,quality,posts}/page.tsx`
- **검증 (실제 브라우저, 4개 폭)**:
  ```
  1024 / 1280 / 1440 / 1680px  →  전 10개 라우트 이탈 0건 · 가로스크롤 0개
  표 행 높이 523px → 103px · /errors 값 라벨 84/84px 전부 보임
  홈 2단 복원 확인 (1024·1440·1680 모두 2트랙) · tsc --noEmit 통과
  ```
- **★ 왜 아무도 못 막았나 — 규정이 죽은 파일을 겨누고 있었다**:
  CLAUDE.md 의 대시보드 폰트·색상 규정은 검증 명령이 `grep ... app.py` 이고 단일 진실 소스가
  `JARVIS03_RADAR/tokens.py`·`components.py` 인데 **셋 다 존재하지 않는다**(Streamlit 대시보드와
  함께 삭제됨). 대상이 없으니 grep 은 늘 "위반 0" — **규정이 한 번도 집행된 적이 없다.**
  그 사이 살아있는 Next.js 대시보드에 14px 미만 글자가 73곳 쌓였다. 별건으로 정정 필요.
- **교훈**:
  ① **`1fr` 과 `flex:1` 은 "줄어든다" 는 뜻이 아니다.** 둘 다 최소가 `auto` 다.
     줄어들게 하려면 `minmax(0,1fr)` · `min-width:0` 을 *명시* 해야 한다.
  ② **측정 기준이 틀리면 고칠수록 나빠진다.** 지표를 만들 때 "사용자가 겪는 증상과 같은
     것을 재는가" 를 먼저 확인할 것. 나는 아니었고, 그 지표를 보고 멀쩡한 곳을 건드렸다.
  ③ **`overflow-x` 만 auto 로 둘 수 없다.** 하나를 auto 로 하면 다른 축도 visible 을 잃는다.
     고정 높이와 함께 쓰면 스크롤바가 내용을 먹는다.
  ④ **규정의 대상이 사라지면 규정은 조용히 죽는다.** 검증 명령이 가리키는 파일이 실존하는지
     주기적으로 확인해야 한다.
...
---
## [591] ✅ 해결 — 대시보드 글자 848개가 14px 미만이었다 · 스크롤 기본값이 **과거**를 보여줬다 (2026-08-09)

- **배경**: [589] 로 넘침을 고치다 **규정이 죽은 파일을 겨누고 있었다**는 것을 발견했다.
  CLAUDE.md 의 대시보드 폰트·색상 규정이 검증 대상으로 지정한
  `JARVIS03_RADAR/{app,tokens,components}.py` 는 **셋 다 존재하지 않는다**(Streamlit
  대시보드와 함께 커밋 `0be08d9` 에서 삭제). 대상이 없으니 grep 은 늘 "위반 0" —
  **규정이 한 번도 집행된 적이 없었다.**
- **실태 (실측)**: 소스 선언 **183건** 위반 / 화면 렌더 기준 **14px 미만 글자 848개**.
  최악은 홈의 파이프라인 보드로 **7.5px** 까지 내려가 있었다.
- **해결 — 세 개를 함께 해야 재발하지 않는다**:
  ① **규정의 대상을 살아있는 `dashboard/`(Next.js) 로 정정** (CLAUDE.md).
     색상 단일 진실 소스도 `dashboard/app/globals.css` 의 `--c-*` 로 바로잡았다.
  ② **183건 수정** — 위계에 맞춰(14 캡션 / 16 본문 / 18 강조 / 20~22 헤더 …) 올리고,
     인라인 hex 는 토큰으로. 파이프라인 보드는 글자만 키우면 깨져서 보드 자체를 키웠다.
  ③ **`precommit_check --category dashboard` 신설** — 커밋 훅·CI·GUARDIAN 감사 3곳에서
     자동으로 돈다. 대상 목록을 박지 않고 `git ls-files dashboard` 에서 파생하므로
     새 페이지도 자동으로 검사된다. **대상 폴더를 못 찾으면 통과가 아니라 위반**(fail-closed)
     — 종전 규정이 정확히 그 방식으로 조용히 죽었기 때문이다.
- **★ 수정 중 낸 사고 하나 — 주석이 화면에 렌더됐다**:
  JSX 루트 앞에 설명을 넣으면서 `{/* */}` 를 빠뜨려, `/* ★ 열 최소폭은 … */` 이
  **본문 텍스트로 화면에 출력**됐다(사용자 화면에서 발견). JSX 안의 주석은 반드시
  `{/* */}`, JSX *밖*(return 괄호 안 루트 앞)은 `/* */` — 둘을 바꿔 쓰면
  한쪽은 렌더되고 한쪽은 문법 오류다(같은 날 `/errors` 에서 후자도 겪었다).
- **★ 사용자 지적 — 스크롤 기본값이 과거를 보여줬다**:
  막대가 카드를 넘칠 때 가로 스크롤로 해결했는데, 브라우저 기본 `scrollLeft = 0` 은
  **가장 오래된 쪽**이다. "오늘/최신" 을 보러 온 화면이 열자마자 과거를 보여주고
  최신이 가려졌다(실측 `/errors` 7일 추이 `scrollLeft 0/304`).
  → `dashboard/lib/scroll.ts` 의 `useLatestVisible(dep)` 단일 헬퍼 신설, 시간축 막대
    **3곳**(홈 시간대별·홈 일별·오류 7일추이)에 배선. 데이터가 바뀌면 다시 맞춘다.
  · **표에는 걸지 않았다** — 행이 시간순이 아닌 레코드 목록을 오른쪽 끝으로 보내면
    엉뚱한 열을 보여준다. 같은 `overflowX:auto` 라도 성격이 다르다.
- **파일**: `CLAUDE.md` · `shared/precommit_check.py` · `dashboard/lib/scroll.ts`(신설) ·
  `dashboard/app/**/page.tsx` · `dashboard/lib/utils.ts` · `dashboard/app/{layout,globals}.*`
- **검증**:
  ```
  렌더 14px 미만  848개 → 0개 (10개 라우트 전수)
  스타일 규정     183건 → 0건 · 전체 precommit 21카테고리 0위반
  넘침 회귀       1024/1280/1440/1680px 전부 이탈 0 · 가로스크롤 0
  스크롤 기본값   3곳 모두 scrollLeft = max (최신이 보임)
  주석·코드 누수  10개 라우트 0건
  ```
- **교훈**:
  ① **규정의 대상이 사라지면 규정은 조용히 죽는다.** 검증 명령이 가리키는 파일이
     실존하는지 자체가 검사 대상이어야 한다 — 그래서 `dashboard` 카테고리는 대상 폴더
     부재를 *위반* 으로 본다.
  ② **글자를 키우면 자리를 더 먹는다.** 폰트 규정과 넘침은 서로 밀어내므로
     한쪽을 고치면 반드시 다른 쪽을 재측정해야 한다.
  ③ **"스크롤 되니까 다 보인다" 는 거짓이다.** 스크롤은 *도달 가능성* 이지 *가시성* 이
     아니다. 기본 위치가 어디인지가 사용자가 실제로 보는 것을 정한다.

---
## [592] ✅ 해결 — 캐시의 절반을 쓰는 경로만 **어느 모델인지 장부에 없었다** (2026-08-09)

- **배경**: 사용자 질문 — *"sdk_query 가 토큰을 많이 쓰던데 얘가 뭐야?"*
  답하려고 장부를 뒤지다 이 경로만 `model` 이 빈 것을 발견했다.
- **`sdk_query` 의 정체**: 모델 별칭이 아니라 **회계 꼬리표**다.
  `run_sdk_query()`(Claude Code SDK 세션)가 `invoke_text` 밖에서 불릴 때 붙는다.
  호출자는 셋 — ① `auto_repair.run_auto_repair`(토 03:00 심층감사)
  ② `_try_sdk_targeted_fix`(Tier-2 표적 수리) ③ `delegate_to_claude_code`(사용자 위임).
  실측 대조: `21:13` 13턴 호출 = `error_log` 의 `Tier-2 SDK 수정 + 재현검증 통과`.
- **왜 비싼가 (실측)**: 호출 12건 · 평균 18.7턴 · 총 42분.
  ```
  출력    169K  ← 전체의 2.4% (9위, 별것 아님)
  캐시읽기 26.9M ← 전체의 50.7% (1위, 2위 analyzer 9.1M)
  8턴 0.70M → 19턴 2.28M → 44턴 5.85M → 57턴 7.18M   (턴당 약 12만, 선형)
  ```
  단발 호출이 아니라 **도구를 든 에이전트 루프**라 매 턴 대화 전체를 다시 보낸다.
  줄이려면 호출 횟수가 아니라 **턴 수**를 줄여야 한다.
- **원인**: `claude_sdk_compat._record_sdk_usage` 가 `model=""` 을 **코드에 박고** 있었다.
  값이 없어서가 아니다 — `run_sdk_query` 는 145줄 위에서 이미 `shared.llm.model_id()` 로
  모델을 정하고 SDK 옵션(`options["model"]`)에는 제대로 실어 보내면서, **장부에만**
  빈 문자열을 적었다. 살아있는 변수를 안 쓰고 빈 값을 넣은 전형적인 계측 누락.
- **영향**: 전체 캐시 읽기의 절반을 쓰는 경로가 "어느 모델이 썼는지" 를 남기지 않아
  ① 모델 교체 전후 비교 불가 ② 대시보드 "용도별 내역" 의 모델 칸이 이 행만 빔
  ③ ADR 017(전역 단일 모델) 준수 여부를 이 경로만 확인할 수 없었다.
- **해결 (세 갈래)**:
  ① **흘려보내기** — `_record_sdk_usage(..., model=model)`. 호출부도 함께 갱신.
  ② **소급** — `token_usage.backfill_missing_model(dry_run=True)` 신설.
     그 시각 이전 마지막 `shared/llm.py` 커밋을 **AST 로 파싱**해
     `MODELS["writer"].model_id` 를 읽는다 — 추측이 아니라 *기록 조회*다.
     못 읽으면 그 행은 건드리지 않는다(빈 값 > 거짓 값).
     · **함수로 만든 이유**: 일회성 스크립트로 DB 를 고치면 무슨 규칙으로 채웠는지
       확인도, 재실행도 못 한다 — 오늘 [590] 소급 채점에서 겪은 그 문제다.
     · `dry_run=True` 가 기본. 실수로 DB 를 고치지 않는다.
  ③ **★ 실측과 추정을 섞지 않는다** — `model_origin` 컬럼 신설
     (`measured` / `backfill_git`). 소급치를 실측처럼 적으면 다음 사람이 그것을
     실측으로 믿는다. 이 저장소가 반복해서 값을 치른 '복사본을 진실로 믿는' 병이다.
- **파일**: `shared/claude_sdk_compat.py` · `shared/token_usage.py` · `tests/test_learning_signal.py`
- **검증**:
  ```
  소급 14건 전부 claude-sonnet-5 (ADR 017 단일 모델과 일치) · 빈 model 0건
  measured 1,451건 / backfill_git 14건 으로 분리 기록
  변이 3/3 검출 — model="" 복원 / 호출부 인자 제거 / dry_run 기본값 False
  ```
- **교훈**:
  ① **비어 있는 계측은 "값이 없다" 가 아니라 대개 "안 넘겼다" 다.** 같은 함수 안에
     살아있는 변수를 두고 빈 값을 박은 경우가 실제로 있었다.
  ② **소급은 지어내기가 아니라 조회여야 한다.** git 이력은 기록이므로 파생해도 된다.
     대신 *조회로 얻은 값* 임을 데이터에 남겨야 한다 — 문서가 아니라 컬럼에.
  ③ **가장 비싼 경로가 가장 관측이 부실했다.** 비용 순위와 계측 품질은 저절로
     비례하지 않는다. 큰 소비처부터 장부를 확인할 것.

---
## [593] ✅ 해결 — 세분화 코드는 고쳤는데 **과거 233건은 그대로**라 결함4가 재발 (2026-08-09)

- **증상**: GUARDIAN auditor `audit_severity_selfcheck` 가 `[결함4] source='radar' 233건 중
  223건(96%)이 'ConnectionError' 한 타입` 을 다시 보고. `severity.type_granularity_issues()`
  상시 경고.
- **환경**: 전날(2026-08-08 23:06) 커밋 `5686c16` 에서 `JARVIS03_RADAR/collectors/__init__.py`
  에 `radar_error_type(exc)`·`report_radar()` 를 신설해 RADAR 11개 파일의 `_g_report` import
  를 전부 교체 완료. 커밋 메시지 자체가 "소급 재분류 실측: 과거 264건 → 5종" 을 계산해
  적어 두었다.
- **원인**: 그 "소급 재분류" 는 **측정만 하고 DB 에 쓰지 않았다.** 실제 커밋 diff 에 UPDATE 문도,
  backfill 스크립트도 없다. `report_radar` 는 향후 실시간 예외만 세분화하므로, 마지막 radar
  오류(2026-08-06 16:04, 수정 이전)를 포함한 기존 263건은 여전히 옛 뭉뚱그린 타입 그대로 DB 에
  남아 있었다. `type_granularity_issues()` 는 최근 14일 창을 보는데, 그 창 안의 오류 233건이
  전부 수정 *이전* 기록이라 결함이 그대로 재현됐다 — 코드는 고쳐졌지만 새 오류가 안 들어오는 한
  창이 빌 때(최대 14일)까지 매일 같은 결함으로 재발할 상황이었다.
- **헛다리**: 없음 — DB 조회로 마지막 radar 오류 시각(2026-08-06)이 수정 커밋 시각(2026-08-08)보다
  이전임을 바로 확인해 "코드는 맞는데 과거 데이터가 안 갱신됐다"로 즉시 좁혀졌다.
- **해결**: `collectors/__init__.py` 의 파생 로직을 `_derive_type(base, msg)` 로 추출해
  `radar_error_type(exc)`(실시간)과 `radar_error_type_from_record(error_type, message)`(소급)
  가 같은 규칙을 공유하도록(①단일 진입점, 로직 복제 금지) 했다. `tools/backfill_radar_error_types.py`
  신설 — `error_log WHERE source='radar'` 전 행에 대해 저장된 message 로 재분류값을 계산해
  변경분만 UPDATE(멱등, 재실행 안전). 실행 결과 263건 재분류:
  `RadarAcSearchNaverNameResolution` 126 · `RadarOpenapiNaverNameResolution` 86 ·
  `RadarTrendsGoogleNameResolution` 28 · `RadarNewsNaverNameResolution` 23 — 커밋 메시지의
  사전 실측치와 정확히 일치. 이후 `severity.selfcheck()` 결함 0건 확인.
- **파일**: `JARVIS03_RADAR/collectors/__init__.py`, `tools/backfill_radar_error_types.py`
- **교훈**:
  ① **"소급 재분류 실측"을 커밋 메시지에 적어 두는 것은 소급 재분류가 아니다.** 계산 결과를
     기록만 하고 DB 에 반영하지 않으면 다음 감사가 반드시 같은 결함을 다시 낸다 — 이것도
     '복사본을 진실로 믿는' 병의 변형이다(계산 결과를 문서화한 것을 실제 적용으로 착각).
  ② **forward-fix 는 과거 데이터를 치료하지 않는다.** 실시간 경로를 고친 뒤에는 *그 고침
     이전에 쌓인 기록이 현재 감사 창 안에 있는지* 를 항상 확인해야 한다. 창이 있는 감사
     (`GRANULARITY_WINDOW_DAYS=14`)는 특히 "언젠가 자연히 빠진다"에 기대지 말 것 — 마지막
     오류 시각이 수정 시각보다 이전이면 창이 빌 때까지 매번 재발한다.
  ③ **소급 스크립트는 실시간 로직의 사본이 아니라 같은 함수를 불러써야 한다.** `_derive_type`
     하나를 두 진입점(`radar_error_type`/`radar_error_type_from_record`)이 공유해 실시간·소급
     결과가 항상 일치하도록 만들었다.

---
## [594] ✅ 해결 — 07:00 경제 브리핑 미발행: **무인 실행인데 사람을 120초 기다렸다** (2026-08-09)

- **증상**: 사용자 신고 — 08-09 07:00 경제 브리핑이 작성되지 않았다.
- **전원·네트워크 문제가 아니다** (먼저 배제):
  ```
  데몬      08-08 23:16 기동 후 밤새 연속 가동 (5분 잡이 07시 전후 끊김 없이 실행)
  j01_economic_post  07:00:00 실행 · 07:02:43 종료 · success=1
  06:00 트렌드 수집  정상 완료 · 06:30 쿠키 사전점검  success=1
  ```
- **직접 원인**: 네이버가 **CAPTCHA/기기 인증**을 요구했고,
  `refresh_naver_cookies` 가 *"화면에서 직접 풀어주세요"* 라며 **120초를 기다렸다.**
  새벽 7시 예약 실행에 화면 앞에 사람이 있을 리 없다. 120초를 버리고 실패했다.
  · 산수가 맞는다: 잡 소요 **163초** = Tier-1 sweep 2초 + CAPTCHA 대기 120초 + 부대비용.
  · 재현으로 확정했다(대화형에서 같은 경로를 돌리자 CAPTCHA 화면이 떴다).
- **★ 왜 원인 규명에 재현이 필요했나 — 이게 더 큰 결함이다**:
  1. **오류 원장 0건.** 게이트 실패가 로그·텔레그램으로만 나가고 `error_log` 에 안 남았다.
     자동수리·학습·감사 어디에도 들어가지 않았다(당일 error_log 166건 중 이 건 0).
  2. **사유가 뭉개졌다.** `ensure_naver_ready` 의 반환은 `permanent`/`deadline` 두 갈래뿐이라
     "왜 사람이 필요한가"(CAPTCHA·기기인증·계정·자격증명 없음)를 구분하지 못한다.
  3. **`print()` 출력이 어디에도 안 남는다.** 실패 상세가 로그 파일에 없어 되짚을 수 없었다.
  4. **잡은 `success=1`.** 발행을 건너뛰었는데 성공으로 기록됐다.
- **해결 (3원칙)**:
  ① **단일 진입점** — 새 통로를 만들지 않았다.
     · 대기 여부 판정은 `naver_cookie_refresher` 안 한 곳.
     · 실패 보고는 `scheduler._naver_cookie_ready` 한 곳 — **경제·테마 공통**이라 ③ 자동 충족.
     · 결손 자체는 이미 있는 `publish_ledger` 감사(23:20)가 잡는다 — 중복 장치 안 만든다.
  ② **동적 설계** — '사람이 있는가' 를 새 플래그로 만들지 않았다.
     `shared.llm.current_job_id()` 에서 파생한다(예약 잡 안 = 무인). 실측 확인:
     잡 밖 `''` → 120초 / 잡 안 `j01_economic_post` → **0초**.
     오류 타입도 사유에서 파생(`naver_login_error_type`) — 중앙 매핑표를 두지 않는다(ERRORS [547]).
     대기 상수는 `NAVER_CAPTCHA_WAIT_SEC` 노브 1곳.
  ③ **4조합** — `_naver_cookie_ready` 가 경제(1017행)·테마(1067행) 공통이라 자동.
- **부수 발견 — 실패를 알리는 코드가 실패했다**:
  `scheduler.log()` 가 `logs/` 없으면 `FileNotFoundError` 로 죽었다. 그 디렉터리는
  `.gitignore` 대상이라 **새 체크아웃·새 기계엔 없다.** 하필 이 함수가 *발행 실패를
  알리는 경로* 에서 불린다 — 알리려다 크래시하면 운영자는 아무것도 못 받는다.
  회귀 테스트를 격리 트리에서 돌리다 발견했다. → 디렉터리 자동 생성 + 기록 실패해도 본류 유지.
- **파일**: `JARVIS08_PUBLISH/credentials/naver_cookie_refresher.py` ·
  `JARVIS02_WRITER/scheduler.py` · `tests/test_learning_signal.py`
- **검증**: **변이 4/4 검출**
  ```
  A 무인에도 계속 대기(사고 당시 동작)  → 죽음
  B CAPTCHA 사유 기록 제거              → 죽음
  C 게이트 실패 박제 제거               → 죽음
  D 사유 없는 return False 1곳 되살림   → 죽음
  로컬 303 passed · precommit 21카테고리 0위반 · 격리 트리(.git 없음)에서도 통과
  ```
  · **허용치를 두지 않았다** — 처음엔 `bare <= 2` 로 뒀다가 변이 D 가 **통과**하는 것을 봤다.
    느슨한 허용치는 그만큼 회귀를 통과시킨다.
- **남는 것 (고치지 않음)**: 잡이 `success=1` 로 남는 것. 발행 건너뜀은 *정책적 스킵*이고,
  결손은 `publish_ledger` 감사가 별도로 잡는다. 여기서 예외를 올리면 misfire 재시도 등
  부작용이 생겨 이 저장소 설계(감사 분리)와 어긋난다.
- **교훈**:
  ① **무인 시스템이 사람을 기다리는 것은 설계 오류다.** 기다림은 발행 창만 먹고
     결과를 바꾸지 못한다. 즉시 포기하고 사람을 부르는 편이 *더 빠른 복구*다.
  ② **원인을 알려면 재현해야 하는 상태 자체가 결함이다.** 실패 사유가 로그·원장에
     남지 않으면 다음에도 같은 시간을 쓴다.
  ③ **실패를 알리는 경로는 가장 견고해야 한다.** 그 경로가 죽으면 침묵이 된다.

---
## [595] ✅ 예방 — 쿠키 파일이 사라져도 **아무도 몰랐다**: 부재 경보 + 소실 추적 (2026-08-09)

- **배경**: [593] 에서 07:00 미발행의 근인을 CAPTCHA 로 확정했는데, *그 CAPTCHA 에 노출된
  이유* 가 따로 있었다 — `naver_cookies.pkl` 이 **통째로 사라져 있었다.**
  파일이 없으면 `cookie_needs_refresh()` 가 항상 True → **매 발행이 전체 로그인** →
  그때마다 네이버의 CAPTCHA 판정에 노출된다.
  발행 이력이 그 시점과 맞아떨어진다:
  ```
  08-07 21:00 테마  1,781초  ✅        08-08 21:00 테마     28초  ❌
  08-08 07:00 경제  1,425초  ✅        08-09 07:00 경제    163초  ❌ (CAPTCHA)
  ```
  20분 넘게 돌며 발행하던 것이 08-08 21:00 부터 갑자기 짧게 끝나며 실패한다.
- **★ 진짜 문제는 "아무도 몰랐다" 는 것**:
  ① `verify_all_logins()` 는 **이미 부재를 알고 있었다** — `"쿠키 파일 없음 또는 빈 list"`.
     그런데 `job_pre_publish_check` 가 그것을 `log.warning` 한 줄로만 처리했다.
     텔레그램도, 오류 원장도 없다. **곧 발행이 깨진다는 예고를 아무도 못 들었다.**
  ② 사라진 *시각* 을 알 수 없어 원인 추적이 불가능했다. 저장소에 그 파일을 지우는 코드는
     **0곳**(grep 실측), 정리 잡 규칙에도 없다 — 그런데 없어졌다. 창을 못 좁히니 여기서 멈췄다.
- **해결 (3원칙)**:
  ① **단일 진입점 — 판정을 새로 만들지 않았다.** 이미 있는 `verify_all_logins()` 의 결과를
     *밖으로 내보내기만* 한다(`_alert_precheck`). 새 점검 로직 0줄.
     **새 잡도 만들지 않았다** — 관측은 쿠키를 *만지는 지점*(읽기 `get_naver_cookies` ·
     점검 시)에서 한다. 그 지점들은 이미 publish 도메인 안에 있다.
  ② **동적 설계** — 오류 타입은 이슈 문구에서 파생(`precheck_error_type`):
     `PrecheckNaverCookieMissing` / `PrecheckNaverCookieStale` / `PrecheckTistoryEnvMissing`.
     중앙 매핑표를 두지 않는다(ERRORS [547]). 소실 창의 "그 사이 무엇이 돌았나" 도
     `job_runs` 를 조회해 파생한다 — 별도 기록을 만들지 않는다.
  ③ **4조합** — 사전점검 잡은 발행 잡마다 자동 생성(`_build_cookie_precheck_jobs`)되므로
     경제·테마 양쪽에 자동으로 걸린다. 플랫폼 루프라 네이버·티스토리도 함께.
- **★ 관측이 문구보다 권위를 갖는다**: 초판은 *이슈 문구* 에 "쿠키 파일 없음" 이 있으면
  소실 창을 붙였는데, 문구는 다른 시점의 판단이라 **"파일 없음" 과 "현재 존재" 가 한 알림에
  같이 실렸다**(실측). 어긋나는 경보는 신뢰를 깎는다 → 지금 관측 결과로만 판단한다.
- **부수**: 관측 파일이 `credentials/` 안이라 그 폴더 규칙(0600)에 걸렸다.
  기존 보안 테스트가 잡았다 — 예외를 두지 않고 `.json`·`.lock`·`.bak` 셋 다 0600 으로 조였다.
- **파일**: `JARVIS08_PUBLISH/credentials/login_manager.py` · `tests/test_learning_signal.py` · `.gitignore`
- **검증**: **변이 5/5 검출**
  ```
  A 경보 호출 제거(사고 당시 = log.warning 만)  → 죽음
  B 오류원장 박제만 제거(텔레그램은 유지)        → 죽음
  C 타입을 RuntimeError 로 뭉갬                  → 죽음
  D 소실 감지 제거(vanished 항상 False)          → 죽음
  E 마지막 관측 시각 유실                        → 죽음
  로컬 307 passed · CI 체크아웃 모사 304 passed · precommit 21카테고리 0위반
  ```
  · **정상일 때 침묵하는지도 검사한다** — 늘 울리는 경보는 아무도 안 듣는다.
- **한계 (정직하게)**: 관측 해상도는 *쿠키를 만지는 빈도* 다. 발행 사이 14시간 동안
  아무도 안 만지면 그 구간이 창이 된다. 다만 창 안에 돈 잡 목록을 함께 주므로
  "언제·무엇과 함께 사라졌나" 를 좁힐 수 있다 — 지금은 그 정보가 **0** 이다.
- **교훈**:
  ① **판정이 있는데 아무도 안 듣는 것**이 판정이 없는 것보다 나쁘다. 있다고 착각하기 때문이다.
     `verify_all_logins()` 는 처음부터 알고 있었다.
  ② **경보는 "다음 한 걸음" 까지 말해야 한다.** "쿠키 없음" 만으로는 심각성을 모른다 —
     "이대로면 전체 로그인 → CAPTCHA 시 발행 건너뜀" 까지 말해야 사람이 움직인다.
  ③ **사라진 것을 다루려면 있었다는 기록이 먼저다.** 소실은 사후에 추적할 수 없다.

---

## [596] ✅ 해결 — 선언은 있는데 **집행이 없었다**: 하루에 같은 병 6건 (2026-08-09)

- **증상**: 오류 강화학습 사슬 전수 점검 중, *규정·상수·가드가 존재하는데 실제로는 아무것도
  막지 않는* 사례가 연달아 나왔다. 전부 "코드에 써 있으니 된다" 고 믿고 아무도 동작을 안 봤다.
- **환경**: 적대적 검증 3회(단일 에이전트 2 + 5관점 병렬 1) + 좁은 회귀 검증 1회.
  전부 *실행으로 재현* 을 요구했고, 그러자 정적 검사가 통과시키던 것이 무너졌다.
- **원인** — 여섯 사례가 전부 같은 형태다:
  | 선언 | 집행 실태 |
  |---|---|
  | `conftest.py` "텔레그램·외부 전송 차단" (`JARVIS_TEST_MODE=1`) | 그 값을 **읽는 코드가 저장소에 0곳**. `pytest` 마다 사용자 실제 챗으로 가짜 운영 경보 발송 |
  | `architecture.DENY_FIX_PATHS` "자동수정 절대 금지" | 소비자가 `_is_deny_path` 하나뿐이고 *오류 레코드의 module* 만 봄. 실제 쓰기 관문 `_safe_path` 는 목록을 **모름** → 샌드박스에서 `jarvis_daemon.py`·`login_manager.py` 실제 패치 성공 |
  | `MAX_LLM_ATTEMPTS=3` 상한 | `_orchestrate` 만 `bump_llm_attempts` 호출. `deep_audit_backlog` 은 안 불러 그 통로에선 **영영 안 걸림** |
  | `_orchestrate` 의 Tier-2 앞 3문(critical·보안파일·잠정실패) | backlog 루프엔 **하나도 없음**. 같은 3행이 한쪽은 LLM 0회, 한쪽은 전원 세션 오픈 |
  | `severity` kind 가드 | 봉투(`abort`·`stuck`)를 무조건 격리해 **유일한 신호까지** 삼킴(stuck 24건 중 13건이 단독 보고) |
  | 폰트·색상 규정(동시 세션 발견) | 규정이 겨눈 파일 3개가 **모두 삭제된 상태** → grep 이 늘 '위반 0' → 화면 글자 848개가 14px 미만 |
- **헛다리**:
  ① "결과 통계가 0이니 그 기능이 필요 없다" — `abort` 86건·`stuck` 24건의 자동수리 실적이
     0이라 Tier-2 에서 뺐는데, 0인 *이유* 가 "사슬이 끊겨 있어서" 였다. 실적으로 필요를
     판단하면 고장난 기능은 영영 못 고친다.
  ② "커밋 메시지에 실측을 적어두면 반영된 것" — [593] 과 같은 병(계산만 하고 DB 미반영).
  ③ "상한 숫자를 옮기면 상한을 옮긴 것" — 옮긴 건 숫자였고 **상한 앞의 판단** 은 한쪽에 남았다.
- **해결**: 집행을 *실제 통로* 로 내렸다.
  · 차단은 **던지는 함수 하나**에서(`notify._post_message`) — 상위 래퍼에 걸면 새 래퍼가 샌다.
    문서화된 예외 통로(`preflight`)도 함께 막았다. '예외' 는 차단 면제가 아니다.
  · 보안 목록은 **쓰기 관문이 주인에서 파생**(`error_fixer._safe_path` → `architecture`).
    파생 실패는 통과가 아니라 **차단**(fail-closed).
  · Tier-2 앞 판단을 `guardian_agent.tier2_blocked_reason()` 하나로 추출해 두 통로 공용.
  · 봉투 판정을 kind 가 아니라 **동봉 실이슈 수**(`companions`, harness 가 싣는 사실)로.
- **파일**: `shared/notify.py`, `JARVIS00_INFRA/preflight.py`, `JARVIS07_GUARDIAN/error_fixer.py`,
  `JARVIS07_GUARDIAN/guardian_agent.py`, `JARVIS07_GUARDIAN/severity.py`,
  `JARVIS00_INFRA/harness.py`, `tests/conftest.py`
- **교훈**:
  ① **선언과 집행은 다른 사건이다.** 상수·규정·주석은 "의도" 이고, 그것을 *읽는 코드* 가 있어야
     집행이다. 새 가드를 넣으면 그 자리에서 `grep -rn "<그 이름>"` 으로 **소비자가 몇 곳인지**
     세라. 0이거나 1이면 그 가드는 없는 것과 같다.
  ② **차단은 가장 아래(실제로 행위를 하는 함수)에 건다.** 위에 걸면 우회로가 생긴다.
  ③ **결과가 0인 기능은 "불필요" 가 아니라 "고장" 일 수 있다.** 0의 원인을 먼저 밝힐 것.

---

## [597] ✅ 해결 — 검사가 "코드가 어떻게 생겼나" 만 물어 **뮤테이션이 통과**했다 (2026-08-09)

- **증상**: 골든 테스트가 초록인데 그 테스트가 지키는 성질을 코드에서 **지워도 여전히 초록**.
  하루 동안 뮤테이션 생존이 5건 나왔고, 전부 검사 방식 문제였다.
- **원인** — 세 변종, 뿌리는 같다(실제 동작을 안 본다):
  | 변종 | 실제 사고 |
  |---|---|
  | **소스 문자열 검사** | `assert "_authoritative" in src` — 변수는 남기고 *사용* 만 지우면 통과 |
  | **테스트가 로직을 재구현** | 대표행 순위 함수를 테스트가 다시 짜서 검사 → 호출부가 그 순위를 안 써도 통과 |
  | **느슨한 허용치** | `assert len(senders) >= 2` — 새 통로를 놓쳐도 옛 통로 둘만 세면 통과 / 동시 세션도 `bare <= 2` 로 같은 일을 겪음 |
  | **조건부 조기 return** | `if not f.exists(): continue` — 대상이 없으면 *아무것도 검사 안 하고* 초록(CI 에선 `.env`·쿠키가 없어 항상 그랬다) |
- **헛다리**: "전체 스위트가 초록이니 안전하다" — 순서 의존으로 **단독 실행 FAIL / 전체 PASS**
  인 테스트가 있었다(앞선 테스트가 `shared.llm` 을 import 해 환경변수를 채워줬다).
- **해결**:
  · 단언을 **실행 결과**로: 실제 harness 액션을 돌려 시도 1 누적 abort 에서 실이슈가 GUARDIAN 에
    닿는지 확인 / 실제 DB UPDATE 후 값 비교 / 깨진 파일을 실제로 import / 스레드를 겹쳐 레이스 재현.
  · weight 검증은 **공식으로 직접**: `weight + alpha*(reward - neutral)` 를 계산해 *어느 행이
    weight 를 움직였는지* 를 값으로 판별(재구현 대신).
  · 대상이 없으면 **명시적 `skip`** — 통과와 미검사는 다른 사건이다.
  · 허용치는 집합 대조로 교체.
  · 환경 의존은 **환경을 비운 별도 프로세스**로 측정(부모 `os.environ` 상속 차단).
- **파일**: `tests/test_publish_golden.py`, `tests/conftest.py`
- **교훈**:
  ① **수용 기준은 "뮤테이션 생존 0" 이고, 허용치·재구현·문자열 검사는 그 기준을 조용히
     무력화한다.** 테스트를 쓴 뒤 반드시 그 성질을 코드에서 지워보고 빨개지는지 확인할 것.
  ② **테스트는 대상 로직을 재구현하면 안 된다.** 재구현은 "내가 짠 게 내 생각과 같다" 를
     증명할 뿐이다. 호출부를 실제로 불러 결과를 봐야 한다.
  ③ **테스트가 운영 자원에 쓰지 않게 하는 방법은 검사가 아니라 기본값 교체다.**
     `conftest` autouse 로 데이터 경로·상태 파일 경로를 임시로 돌리면 새 테스트도 자동 안전.

---

## [598] ✅ 해결 — 고치면서 **관측을 함께 지웠다**: 신호가 사라진 3건 (2026-08-09)

- **증상**: 결함을 고쳤는데 그 결함이 *일어났다는 사실* 이 어디에도 안 남게 됐다.
- **원인**:
  ① **트렌드 판 보존 가드** — 오후 회차의 빈 판이 아침 판을 덮지 않게 막았다. 그러자
     `_verify_trends`·`_pack_empty_reason` 이 파일만 보고 "정상" 이라 판단해 *수집 전멸이
     안 보였고*, 원인이 `TrendCollectEmpty` 대신 `TopicPackNoCandidate` 로 **오분류** 됐다.
     내가 근거로 댄 "수집기가 `Radar*` 타입으로 보고한다" 는 DB 실적 **0건** 이었다 —
     데몬 재시작 이후 radar 오류가 아직 없어서다. *미래형 근거를 현재형으로* 말했다.
  ② **`_run_with_harness` 의 새 raise** — 하네스 abort 가 `job_runs.success=1` 로 적히던 것을
     고쳤는데, 하네스가 **실패가 아니라고 명시 분류** 한 `deferred`(데몬 재시작 연기·동시 실행
     중복)까지 예외로 올려, 2026-08-07 에 일부러 없앤 오탐을 되살렸다(실측 25일 8건).
  ③ **`raise` 위치** — `job_deep_audit` 의 `raise` 가 3부보다 앞이라, 실패한 회차엔 격리 버킷
     주간 보고가 안 나가고 `busy` 플래그가 1시간 거짓으로 남았다. 격리분을 가장 봐야 할
     회차가 정확히 그 회차다.
- **헛다리**: "파일에 사실을 박아 두면 관측된다" — 그 도장의 유일한 소비자가 *팩이 빌 때만*
  불리는 함수였다. 판을 지켰으니 팩은 안 비고, 도장은 영영 안 읽힌다.
- **해결**: 신호를 되살리되 **비용 없이** — `kind="trends_empty"` 로 달아 기록은 남고 Tier-2
  세션은 안 열리게(코드로 못 고치는 외부 상태). `deferred` 는 하네스가 실은 **구조화 판정
  필드** 를 그대로 존중(판정을 호출자가 다시 만들지 않는다). 정리(`report_ignored_bucket`·
  `clear_busy`)를 `raise` 보다 앞으로.
- **파일**: `JARVIS03_RADAR/radar_main.py`, `JARVIS03_RADAR/jobs.py`,
  `JARVIS07_GUARDIAN/severity.py`, `JARVIS07_GUARDIAN/guardian_agent.py`
- **교훈**:
  ① **가드를 넣을 때 "이 사건을 앞으로 어떻게 아는가" 를 같이 정해라.** 막기만 하면 그 사건은
     통계에서 사라지고, 다음 사람은 문제가 없어진 줄 안다.
  ② **"~가 이미 보고한다" 는 근거는 DB 로 확인하고 말할 것.** 코드가 그렇게 생겼다는 것과
     실제 기록이 있다는 것은 다르다(실적 0건이었다).
  ③ **정리(cleanup)는 `raise` 앞에.** 실패 경로에서 꺼지는 관측은 가장 필요한 순간에 꺼진다.

---

## [599] ✅ 해결 — 검사와 기록이 **다른 원본**을 봐서 학습 고리가 끊겨 있었다 (2026-08-09)

- **증상**: 글 품질 학습이 도는 것처럼 보이는데 `insight_usage` 966건 중 `violated` 기록이
  **1건**. 지침을 넣고 "지켰는지" 판정까지 하는데 그 판정이 학습에 닿지 않았다.
- **원인** — 원본이 갈라진 지점이 셋:
  ① **검사↔기록**: 게이트는 랭킹 재조회(`_pinned_pick`, 60분 인메모리 고정)로 *무엇을 검사할지*
     를 정하고, 기록은 주입 배치를 봤다. 고정은 데몬 재시작·TTL 로 사라지므로 두 목록이 갈라지고,
     갈라지면 어겼다고 판정된 지침이 **기록되지 못한다**.
  ② **배치 식별**: `latest_batch(scope, platform)` 가 `reward IS NULL` 만 걸어 **며칠 묵은 배치**
     가 후보였다(실측 33개가 1일 이상, 최고령 7/21). 작성기가 `platform=''` 로 주입한 경우
     게이트는 `'naver'` 로 물어 **다른 배치**를 잡았다.
  ③ **보상 귀속**: `(insight, analysis)` 쌍당 *처음 만난 행* 이 weight 를 갱신하는데,
     `used_at ASC` 라 그 '처음' 은 **항상 가장 오래된 미귀속 행** — 위반 판정이 없는 옛 행이
     선점해 페널티가 사라졌다.
  ④ **단위**: 항목 획득률 `[0,1]` 과 총점 중앙값 `0.685` 를 따로 폴백시켜 서로 비교했다.
     A1~A5(매력도 5축) 5개가 그 상태였고, 그 항목을 겨눈 지침은 옳아도 매번 Δw≈−0.21 을 맞았다.
- **헛다리**: "규칙이 같으면 결과도 같다" — 규칙이 같아도 **부르는 시점** 이 다르면 갈라진다.
  이게 이번 사고의 핵심이고, 종전 불변식("같은 규칙을 쓴다")이 그래서 부족했다.
- **해결**: 세 소비자가 **같은 실물**(`insight_usage` 기록)을 읽는다 —
  `db.batch_directives(batch_id)` 신설, `active_directives`·`record_directive_violations` 둘 다
  거기서 읽음. `latest_batch` 는 액션 상한(`watchdog.BLOG_ACTION_DEADLINE_SEC`)에서 파생한
  시간 창 + `platform=''`(미상) 호환 매칭. 보상 대표행은 **판정을 가진 행 중 최신**.
  보상과 중립점은 **같은 자** 에서(한쪽만 있으면 둘 다 총점으로).
- **파일**: `shared/db.py`, `JARVIS07_GUARDIAN/quality_learner.py`, `JARVIS02_WRITER/prepublish_gate.py`
- **교훈**:
  ① **"같은 규칙" 이 아니라 "같은 실물" 을 봐야 한다.** 규칙 공유는 시점이 다르면 깨진다.
     무엇을 했는지의 *기록* 이 있으면 규칙을 다시 돌리지 말고 그 기록을 읽어라.
  ② **대표를 고를 때는 '먼저 온 것' 이 아니라 '판단을 가진 것'.** 정렬이 우연히 만든 우선순위가
     의미를 갖는 순간 학습이 왜곡된다.
  ③ **폴백은 짝으로 움직여야 한다.** 비교하는 두 값이 각자 폴백하면 단위가 섞인다.

---
## [600] ✅ 해결 — 내가 만든 회귀: CAPTCHA 를 **낱말로 판정**하고 있었다 (2026-08-09)

- **배경**: [593] 에서 "무인 실행이면 CAPTCHA 를 기다리지 않는다" 로 고쳤다. 그런데
  사용자가 로그인 시험을 요청해 돌려보니 **CAPTCHA 분기가 매번 걸렸다.**
- **원인 — 판정 조건이 항상 참이었다**:
  ```python
  if "captcha" in src.lower() or "보안" in src or "기기" in src:
  ```
  실측(캡차가 **없는** 평상시 `nid.naver.com/nidlogin.login`, 19,620자):
  ```
  captcha 7회 · 보안 2회 · 기기 0회        ← 캡차 요소는 0건
  ```
  즉 저 조건은 **캡차와 무관하게 늘 참**이다. 그 분기는 "캡차 감지" 가 아니라
  사실상 **"15초 더 기다리기"** 였다(앞의 1차 대기가 15초뿐이라 조금만 느려도 여기로 온다).
- **★ 그래서 [593] 의 수정이 회귀가 됐다**: 내가 그 대기를 *무인일 때 0* 으로 만들었으니,
  **캡차가 아닌 그냥 느린 로그인까지 15초 만에 포기**한다. 발행을 깨뜨릴 뻔했다.
  다행히 사용자가 "로그인 직접 해보자" 고 해서 배포 전에 드러났다.
- **해결 — 섞여 있던 두 가지를 분리한다**:
  · `LOGIN_REDIRECT_WAIT_SEC`(60) — **로그인이 끝나기를** 기다리는 시간. 사람과 무관하므로
    **무인이어도 준다.**
  · `CAPTCHA_WAIT_SEC`(120) — **사람이 캡차를 푸는** 시간. 무인이면 0 이 맞다.
  · 판정은 낱말이 아니라 **요소**로: `captcha_present(driver)` —
    `img[id*=captcha]`·`input[name*=captcha]`·`#captchaimg`·`iframe[src*=recaptcha]` 중
    **화면에 보이는 것**이 있는가. 못 찾으면 False — *'모른다' 를 '사람 필요' 로 단정하지 않는다.*
- **파일**: `JARVIS08_PUBLISH/credentials/naver_cookie_refresher.py`
- **검증**: 변이 2/2 검출 (낱말 판정 복귀 / 대기 예산 0 복귀)
- **교훈**: **분기 이름이 하는 일과 다를 수 있다.** "CAPTCHA 감지" 라 적혀 있어 나도 그렇게
  믿고 고쳤다. 조건을 실제 데이터로 검증했어야 했다 — 페이지 19,620자를 세어보는 데 1분 걸렸다.

---
## [601] ✅ 해결 — 티스토리만 **존재 확인**이었다: 만료돼도 초록불 (2026-08-09)

- **배경**: 사용자가 *"블로그 작성이 편집창까지 가는지 눈으로 보고 싶다"* 고 해서
  네이버·티스토리 각각 실제 브라우저로 편집창 진입을 시험했다.
  · 네이버 ✅ (제목·본문칸·발행버튼 확인)
  · 티스토리 ❌ — `manage/newpost/` 가 `/auth/login` 으로 튕겼다.
- **원인 (두 겹)**:
  ① **판정이 비대칭이었다.** 네이버는 `check_cookie_valid()` 로 requests 실검증(1~2초)을
     하는데, 티스토리는 `verify_all_logins()` 에서 **env 변수 '존재' 만** 봤다.
     그래서 만료된 쿠키도 `tistory: OK`. 20:30 사전점검이 구조적으로 못 잡는다.
     · 실증: `TS_COOKIE` 를 가짜 40자로 바꿔도 종전 판정은 ✅.
  ② **자동 갱신도 절반이었다.** `auto_refresh_if_needed` 가 `not get_tistory_cookie()`
     — 즉 **부재일 때만** 갱신했다. 08-08 21:00 실패 때 쿠키는 **있었고**(40자) 만료였다.
     그래서 아무 일도 하지 않았고 발행은 28초 만에 튕겨 끝났다(잡 이력 실측 `success=0`).
- **해결 (3원칙)**:
  ① **판정 규칙을 한 곳으로** — `is_login_redirect(url)` 신설.
     ERRORS [292] 가 확립한 진실(*"manage 진입이 로그인으로 리다이렉트되는가"*)을 소유한다.
     셀레니움 경로(`check_cookie_valid`)도 이 함수를 부르게 고쳐 **규칙 사본을 없앴다.**
  ② **브라우저 없는 판정** `cookie_valid_http()` — 네이버와 대칭(requests, ~1초).
     `verify_all_logins`·`auto_refresh_if_needed` 가 이걸 쓴다.
     · **`None`(판정 불가)을 `False`(만료)로 뭉개지 않는다.** 네트워크 순단마다 거짓 경보가
       되고, 더 나쁘게는 *불필요한 로그인* 을 유발해 CAPTCHA 위험을 부른다([595] 참조).
  ③ **4조합** — 이제 네이버·티스토리 둘 다 실검증. 경제·테마는 같은 사전점검 잡을 쓴다.
- **파일**: `JARVIS08_PUBLISH/credentials/{tistory_cookie_refresher,login_manager}.py`
- **검증**: **변이 4/4 검출**
  ```
  A 티스토리 유효성 검증 제거(env 존재만)   → 죽음
  B 만료 시 자동갱신 제거(부재만)           → 죽음
  C 판정불가(None)를 만료로 뭉갬            → 죽음
  D 규칙 사본 부활(셀레니움이 따로 판정)     → 죽음
  실계정 실측: 만료 감지 False → 자동 갱신 31초(CAPTCHA 없음) → 편집창 진입 ✅
  로컬 312 passed · CI 모사 309 passed · precommit 21카테고리 0위반
  ```
- **교훈**:
  ① **한쪽만 검증하는 게이트는 검증하지 않는 게이트다.** 네이버가 실검증을 하고 있어서
     "인증 점검은 있다" 고 믿었는데, 티스토리는 이름만 있었다.
  ② **"있다" 와 "쓸 수 있다" 는 다른 질문이다.** `TS_COOKIE` 는 있었다 — 다만 죽어 있었다.
  ③ **자동 복구는 감지와 짝이어야 한다.** 만료를 감지해도 갱신 조건이 '부재' 면 아무 일도
     일어나지 않는다. 감지·복구가 같은 판정을 써야 루프가 닫힌다.

---
## [602] ✅ 해결 — 대역이 **소비자와 다른 심볼**을 겨눠 단언이 공허했다 (2026-08-09)

- **증상**: `test_네트워크_문제를_만료로_적지_않는다` 가 초록인데 아무것도 검사하지 않았다.
- **원인**: 테스트가 `monkeypatch.setattr(nc, "check_cookie_valid", _boom)` 로 대역을 넣었는데,
  코드는 그 사이 **`cookie_valid_http`**(3-상태)로 바뀌어 있었다. 대역이 한 번도 안 불렸고
  `_boom` 은 터지지 않았다 — 단언은 늘 참.
- **★ 오늘만 세 번째 같은 형태**:
  ① `test_소급분은_실측과_구분되어_기록된다` — 로컬 DB 행을 세다 CI 에서 무의미 ([587])
  ② `test_네트워크_문제…` 초판 — 대역을 넣고 **그 대역의 소스를 뜯어봤다**
     (`inspect.getsource(nc.check_cookie_valid)` 를 patch *뒤* 에 호출 → 가짜 함수를 파싱)
  ③ 이번 — 대역이 소비자와 다른 심볼
  공통점: **테스트가 자기 설정을 검사하고 진짜 코드를 안 건드렸다.**
- **해결**:
  · 대역은 **소비자가 실제로 부르는 심볼**에 건다. 바뀌었으면 테스트도 따라간다.
  · 원본 소스가 필요하면 **patch 전에** 잡는다.
  · 계약(3-상태)을 AST 로 검사 — `None` 이 있고 `False` 가 없어야 한다.
- **파일**: `tests/test_learning_signal.py`
- **검증**: 변이 2/2 검출 (네이버 실효 판정 제거 / 네트워크 예외를 False 로 뭉갬).
  **변이 적용 여부를 먼저 단언**하도록 시험 스크립트를 바꿨다 — 동시 세션이
  "변이가 적용조차 안 됐는데 초록" 을 겪었다고 알려와 흡수했다.
- **교훈**: **초록은 '검사가 돌았다' 는 뜻이 아니다.** 대역·소스 조회·단언이 *같은 대상*
  을 가리키는지 매번 확인해야 한다. 가장 싼 확인은 변이다 — 되돌려서 빨개지는지 본다.

---
## [603] 📌 병행 작업 — 같은 자리를 동시에 고치면 한쪽이 지워진다 (2026-08-09)

- **사건**: 두 세션이 `login_manager.verify_all_logins` 의 **네이버 분기 같은 자리**를
  몇 분 차이로 고쳤다. 둘 다 미커밋이라 파일에 `if not nv_issues:` 가 두 번 생겼고,
  그것을 정리하던 쪽이 **상대의 주석 블록과 `if` 줄까지 지워** `try:` 가
  `if cookie_age > 10:` 안으로 들어갔다 — *쿠키가 10시간 넘을 때만* 실효 검증이 도는 상태.
  문법은 멀쩡해서 컴파일·테스트로는 안 드러났다.
- **정리 결과**: 3-상태(`cookie_valid_http`) 판이 더 정확해 그쪽을 남기고 2-상태 블록을 뺐다.
  잃었던 주석은 복원했다. 현재 `if not nv_issues:` 1곳 · 판정 import 1곳.
- **오늘 같은 사고 3회** (양방향):
  · `git add -A` 로 상대 미저장 변경 4개를 내 커밋에 흡수
  · 상대가 `tests/test_publish_golden.py` 스테이징하며 내 변경 흡수 → 테스트 반쪽만 커밋돼 CI RED
  · 이번 건
- **합의한 규칙**:
  ① `git add <파일 나열>` 만 쓴다. 커밋 직전 `git diff --cached --name-only` 로 확인.
  ② 훅의 '잔여 0' 에 걸리면 `--no-verify` + **사유와 남은 파일명을 메시지에 명시**.
  ③ **편집 시작·종료를 한 줄로 알린다** — "지금 `<파일>` 만집니다" / "`<파일>` 놓습니다".
     mtime 으로 '편집 소강' 을 추정하지 않는다. 추정은 확인이 아니다.
- **교훈**: 훅의 "워킹트리 0건" 규정은 *한 사람이 작업할 때* 를 전제로 만들어졌다.
  둘이 붙으면 **그 규정을 지키려는 행동이 곧 상대 작업 흡수**가 된다.
  규정을 바꾸는 대신, 규정이 전제하는 조건(단독 작업)을 통신으로 만들어야 한다.

---
## [604] ✅ 해결 — "단일 소유자" 라 했는데 **세 번째 사본**이 발행 경로에 남아 있었다 (2026-08-09)

- **배경**: [601] 에서 티스토리 로그인 판정 규칙을 `is_login_redirect()` 한 곳으로 모았다고
  적었다. PR 본문 근거를 만들려고 18개 에이전트로 **적대적 검증**을 돌렸더니 반박당했다.
- **사실**: 묶은 것은 `credentials/` 안 **두 곳**뿐이었다.
  `JARVIS08_PUBLISH/platforms/tistory_poster.py:216` 에 같은 규칙의 **세 번째 인라인 사본**이
  그대로 있었고, 그건 하필 **발행 경로**다.
  ```
  _logged_in = ("/auth/login" not in _mng_url and "accounts.kakao.com" not in _mng_url)
  ```
  → URL 패턴이 바뀌면 사전점검·자동갱신은 고쳐지고 **발행만 옛 규칙으로 남는다.**
  내가 주장한 "한 곳만 고치면 전부 따라온다" 는 성립하지 않았다.
- **해결**: 발행 경로도 `is_login_redirect()` 를 부른다. 규칙 리터럴은 이제 주인 1곳뿐(실측 grep).
- **★ 함께 정정한 거짓 서술 2건** — 둘 다 *확인하지 않고 단정한* 것이다:
  ① `test_티스토리_만료를…` 도크스트링의 *"네이버는 실검증을 하는데 티스토리만 안 했다"* —
     **그때는 네이버 분기도 HTTP 요청이 0회였다.** 비대칭이 아니라 양쪽 다 존재 확인이었다.
  ② PR 본문 제목의 *"티스토리만 존재확인이던 비대칭"* — 같은 오류.
- **파일**: `JARVIS08_PUBLISH/platforms/tistory_poster.py` · `tests/test_learning_signal.py`
- **교훈**:
  ① **"단일 진입점으로 만들었다" 는 주장은 grep 으로 증명해야 한다.** 두 곳을 묶고
     세 번째를 못 본 채 "단일" 이라 적었다. 오늘 세 번째 같은 실수다.
  ② **적대적 검증이 내 서술을 반박했다.** 92항목 중 7건 기각 — 그중 3건이 내 것이었다.
     자기 작업을 자기가 검토하면 이 종류는 안 걸린다.

---
## [605] ✅ 해결 — **시스템이 발행 직전에 자기 쿠키를 지우고 있었다** (2026-08-09 21:00)

- **증상**: 사용자 신고 — 아침 07:00 경제에 이어 **밤 21:00 테마도 미발행**. 43초 만에 종료.
- **★ 근본 원인 — 로그가 그대로 말한다**:
  ```
  21:00:02 🗑️ [테마글] 쿠키·캐시 초기화: **네이버 쿠키 파일**, TS_COOKIE 환경변수, Chrome:…
  21:00:42 🚨 [테마글] 네이버 쿠키 점검 실패 — 발행 건너뜀
  ```
  `scheduler._clear_all_cookies()` 가 **발행 직전에 `naver_cookies.pkl` 을 무조건 삭제**한다.
  경제(1023행)·테마(1071행) 양쪽 다. 그래서 매 발행이 *전체 로그인* 이 되고,
  네이버가 반복 로그인에 CAPTCHA 를 걸면 그대로 실패한다.
  **원래 의도는 좋았다** — *"매번 새 로그인으로 신선한 쿠키 보장"*. 그 전제가 뒤집혔을 뿐이다.
- **★ 내가 [595] 에서 틀렸다**: *"저장소에 그 파일을 지우는 코드는 0곳(grep 실측)"* 이라 적었다.
  거짓이었다. 나는 `naver_cookies.pkl`·`COOKIE_FILE` 문자열로 grep 했는데 이 코드는
  `BASE_DIR / "naver_cookies.pkl"` 로 **조립** 해서 걸리지 않았다.
  · 07:00 로그에 이 항목이 없었던 것도 오해를 키웠다 — 그때는 파일이 *이미 없어서*
    `if _naver_cookie.exists():` 가 거짓이었을 뿐이다. 21:00 엔 내가 08:29 에 복구해 둬서 찍혔다.
  · **경로를 조립하는 코드는 문자열 grep 에 안 걸린다.** 삭제·쓰기 지점을 찾을 땐
    `unlink`·`rmtree`·`write_` 같은 *동사* 로도 훑어야 한다.
- **해결 (3원칙)**:
  ① **단일 진입점** — `_clear_all_cookies` 한 곳만 고쳤다. 경제·테마 공통이라 ③ 자동.
  ② **동적 설계** — 지울지 말지를 *유효성에서 파생* 한다:
     · **유효** → 보존(로그인 자체를 건너뛴다 → CAPTCHA 를 만날 일이 없다)
     · **만료** → 삭제(어차피 새로 받아야 한다)
     · **판정 불가** → 보존. '모른다' 를 '만료' 로 단정하면 네트워크 순단이 곧 강제
       재로그인이 되어 CAPTCHA 를 부른다([600] 과 같은 규칙)
     · Chrome 캐시는 로그인 데이터가 아니므로 종전대로 항상 삭제
  ③ 티스토리 `TS_COOKIE` 도 같은 규칙으로 통일.
  · **보존한 것도 로그에 남긴다** — 종전엔 삭제만 찍혀서 *"왜 로그인을 또 하지"* 를
    되짚을 수 없었다. 21:00 원인 규명이 늦어진 이유다.
- **파일**: `JARVIS02_WRITER/scheduler.py` · `tests/test_learning_signal.py`
- **검증**:
  ```
  유효     → 쿠키 보존 ✅ · TS_COOKIE 보존   🔒 보존(로그인 건너뜀)
  판정불가 → 쿠키 보존 ✅ · TS_COOKIE 보존
  만료     → 삭제      · TS_COOKIE 삭제     🗑️ 초기화: …(만료)
  로컬 319 passed · precommit 21카테고리 0위반
  ```
  · **복구 실측**: 쿠키 재생성 시 `⏳ 로그인 전환 대기(60초) — 캡차 요소 없음` →
    `✅ 로그인 완료(지연)` — **CAPTCHA 없이 49초**. [600] 의 요소 기반 판정이
    "느린 로그인" 을 "캡차" 로 오해하지 않은 첫 실측이다. 낱말 판정이었다면
    무인 실행에서 그대로 실패했을 경우다.
- **교훈**:
  ① **어제의 최적화가 오늘의 사고가 된다.** "매번 새 로그인" 은 CAPTCHA 가 없던 시절엔
     옳았다. 전제가 바뀌었는데 코드는 그대로였다. *왜 이렇게 하는가* 를 주석에 남겨야
     전제가 바뀐 것을 알아챌 수 있다.
  ② **"지우는 코드가 없다" 는 grep 결과를 믿지 말 것.** 경로 조립은 문자열 검색을 피해간다.
  ③ **보존도 기록해야 한다.** 삭제만 로그에 남기면 "안 지웠다" 와 "애초에 없었다" 가
     구분되지 않는다.
