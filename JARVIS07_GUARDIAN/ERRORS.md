...
> 맞춘 뒤 절감 대책 협의. 사용자 지시 *"①은 글 품질을 재서 A/B 검증 선행 후 위험하지 않게
> 구현. 그리고 ②~④도 순차 진행"* 에 따라 실행.

- **증상**: `invoke_text` 호출 1건이 **31,468 토큰**. 그중 우리가 보낸 프롬프트는 **227** 뿐이고
  나머지 31,241 은 Claude Code CLI 자체의 에이전트 정체성 + 내장 도구 스키마 + **사용자 MCP
  커넥터(Notion·Google Drive) 스키마**였다. 이 호출들은 도구를 **하나도 쓰지 않는다**.
- **원인 (두 겹)**:
  ① `_run_sdk_sync` 가 `ClaudeCodeOptions` 에 `system_prompt` 도 `disallowed_tools` 도 주지
     않아, 그 자리를 CLI 기본값이 통째로 점유했다. `_ISOLATE_CWD`(2026-07-20)가 CLAUDE.md
     자동 로드 48,940 토큰을 걷어냈지만 **그 아래 깔린 CLI 프리픽스는 그대로** — 같은 병의 2차 발현.
  ② `full_prompt = f"{system}\n\n{prompt}"` 로 **system 을 사용자 메시지에 병합**. 매 호출
     불변인 헌법·루브릭이 프롬프트와 한 덩어리가 되어 한 글자만 달라도 캐시가 전량 무효화됐다
     (`writer` 캐시 재사용 **1.0x** — 쓰기 2.27M ≈ 읽기 2.17M. 캐시 쓰기는 1.25배라 순손실).
- **★ 도구를 꺼도 되는 근거 (추측 아님)**: `_run_sdk_sync` 는 응답에서 **`TextBlock` 만 수집**한다
  — 모델이 도구를 써도 **그 결과는 버려진다**. 그런데 도구 왕복(멀티턴)은 호출의 11% 인데
  토큰의 **36%**(12.0M/33.2M)를 먹었다. 쓰지도 않는 기능에 토큰 1/3 을 태우고 있었다.
- **해결 ①②**: `disallowed_tools=["*"]` + `system` 을 `system_prompt` 로 이동.
  · **와일드카드인 이유(② 동적 설계)**: 이름 목록 11종은 13,914 까지만 줄었다 — MCP 도구를
    놓쳤기 때문. `["*"]` 는 227 까지 내려가고 **목록 유지보수도 필요 없다**.
  · 적용 범위는 **텍스트 경로 하나**. 도구가 *필요한* `_invoke_sdk_vision`(이미지 Read)·
    `run_sdk_query`(auto_repair 자가수정)는 건드리지 않았다.
  · ReAct 보존: `router.py` 가 `system` 에 주입한 도구 스키마를 **그대로** 넘긴다.
    (호출자 system 을 *대체* 하면 tool_calls 가 예외 없이 평문으로 강등돼 승인 게이트가
     무증상 우회된다 — 그래서 대체가 아니라 *이동* 이다.)
- **★ 품질 A/B (사용자 지시로 선행)** — 실제 발행이 쓰는 `draft_writer._gen_section_call1` 로,
  프롬프트·모델·입력을 완전히 동일하게 두고 SDK 옵션만 바꿔 측정:

  | 조건 | CHART 지시 준수 | 평균 문단 | 문장 | 길이 |
  |---|---|---|---|---|
  | A(현행) | **3/8** | 6.3 | 7.7 | 892자 |
  | D(신안) | **8/8** | 11.0 | 14.3 | 1,323자 |

  · 1차(루브릭 총점)는 **스스로 기각** — 부분 섹션이라 루브릭이 대부분 0점이었고 표본 3개로는
    조건 내 편차(41.5~49.0)가 조건 간 차이를 덮었다. 판별력 없는 측정으로 "안전" 이라 말할 수 없다.
  · 2차는 *프롬프트가 숫자로 요구한 것*(CHART 최소 개수)을 지켰는지만 봤다 — 주관 개입 0.
  · 3차는 **같은 시간대 A↔D 번갈아** 실행해 스로틀 시간대 교란까지 제거했다.
  · 결과는 동등이 아니라 **개선**. 도구가 없으니 모델이 도구 경로로 새지 않고 글에 집중한 것으로 보인다.
- **해결 ③**: 관련성 게이트(`chart_data._relevance_filter`)를 **로컬 임베딩 하이브리드**로.
  "이 표가 이 주제를 다루는가" 는 코사인 유사도가 하는 일 그 자체다.
  · 실측 보정(주제 4종 × 표 19개): 관련 최소 0.618 / 무관 최대 0.555 — 갈리지만 여유가 +0.063 뿐.
    그래서 **순수 임계값이 아니라 하이브리드**: 확실한 구간만 임베딩이 결정하고 중간 밴드는
    종전대로 LLM 이 본다. 전부 확실하면 LLM 0회, 아니면 LLM 이 보는 목록만 줄어든다.
  · fail-safe: 임베딩이 어떤 이유로든 실패하면 **전부 '애매'** 로 돌려 종전 동작(전량 LLM)이 된다.
- **★ 기각한 대책 2건 (실측이 분석을 반박)**:
  · *"동일 오류 중복 재분석 44.8M(25.9%) — 1순위"* → 7일 LLM 시도 **총 28회**, 중복 3회.
    `attempts > MAX_LLM_ATTEMPTS(3) → wontfix` 캡이 **이미 작동 중**(2026-07-06 박제).
  · *"동일 프롬프트 5~7회 반복 → 메모이제이션"* → 그 관찰의 실체는 반복 프롬프트가 아니라
    **매 호출 붙던 고정 프리픽스**(`input_tokens=2,947`이 184회 = 봉투 무게가 같았을 뿐).
    ①이 그 봉투를 없앴다. 진짜 결정론 호출(`temperature=0`)은 저장소 전체에 **2곳뿐** 이고
    둘 다 이미 처리됨(`_SYNONYM_CACHE` / ③). 나머지는 `temperature>0` — 캐시하면 헌법이
    금지한 "고정 팔레트·고정 템플릿" 이 되어 **품질 저하**다.
- **★ 구현 중 자수한 내 버그 1건**: ③에서 `if not vecs` 로 numpy 배열의 진리값을 물었다.
  다원소 ndarray 의 진리값은 ValueError 이고, 내 fail-safe 가 그 예외를 삼켜 **임베딩이 한 건도
  판정하지 못한 채** 전량 LLM 으로 가고 있었다. 테스트에서 "전부 애매" 가 나와 발견.
- **결과 (재시작된 프로세스 실측)**: 호출당 31,468 → **201~294 토큰**.
- **부수 효과 (보안)**: 트랜스크립트에 데몬 LLM 세션이 `mcp__claude_ai_Notion__notion-search`·
  `mcp__claude_ai_Google_Drive__search_files` 를 **실제 호출한 기록**이 있었다. 오류 하나
  분석하다 사용자 Notion 을 검색한 것이다. 도구 차단이 이 **데이터 경계**도 함께 닫는다.
- **킬스위치 (무배포 되돌리기)**: `LLM_TEXT_NO_TOOLS=0` / `LLM_SPLIT_SYSTEM=0` /
  `CHART_GATE_EMBED=0` · `CHART_GATE_HI` · `CHART_GATE_LO`.
- **파일**: `shared/llm.py` · `JARVIS09_COLLECTOR/chart_data.py`.
- **교훈**:
  1. **"안 쓰는 기능" 과 "결과를 버리는 기능" 은 다르다.** 도구는 전자로 보였지만 실제로는
     후자였고, 버릴 왕복에 토큰의 36% 가 나갔다. 코드를 읽어 *소비 여부* 를 확인해야 보인다.
  2. **목록을 박으면 새 것을 놓친다.** 도구 이름 11종을 적었더니 MCP 도구가 빠져나갔다.
     와일드카드는 절감이 클 뿐 아니라 *유지보수가 필요 없다* — ② 동적 설계의 전형.
  3. **판별력 없는 측정으로 안전을 주장하지 말 것.** 1차 A/B 는 평균이 정확히 같았지만
     그건 "차이 없음" 이 아니라 "잴 수 없었음" 이었다. 스스로 기각하고 다시 설계했다.
  4. **분석 결과는 실측으로 재봐야 한다.** 9-에이전트 심층분석의 1순위·4순위가 모두 기각됐다.
     하나는 이미 고쳐진 문제였고, 하나는 관찰의 오독이었다.

---

## [504] ✅ 해결 — 토큰 현황판이 1.62배 틀린 값을 보여주고 있었다 (2026-07-26)

> ★ **맥락**: 사용자 *"토큰 한도 소진을 어떻게 줄일까 — 품질 저하는 안 됨"* 심층 분석 중,
> 사용자가 *"제대로 측정된 값이 홈탭에 실시간으로 올라가야 한다. 그렇게 되고 있나?"* 라고 물어
> 확인하다 발견. **답은 '아니오'였다.**

- **증상**: 대시보드 홈탭 토큰 현황판 수치가 실제와 다름. 방향은 맞았다 — `summary()` 는
  DB(실소비의 18.7%만 기록)가 아니라 **트랜스크립트를 읽고 있었다**. 그런데 그 스캔에 결함 2개.
- **원인 ① 서브에이전트 전량 누락**: `glob(_TRANSCRIPT_ROOT / "*" / "*.jsonl")` — **깊이 2 고정**.
  서브에이전트 트랜스크립트는 `프로젝트/세션/subagents/…/agent-*.jsonl`(깊이 6)이라 한 건도
  닿지 않았다. **바로 위 docstring 은 "서브에이전트까지 포함한 총량" 이라고 적혀 있었다** —
  문서가 코드보다 앞서간 드리프트. 실측 누락분 **476,882,886 토큰(24.4%)**.
- **원인 ② 같은 응답 중복 계수**: Claude Code 는 응답 1건을 content block 수만큼 JSONL 여러
  줄로 쓰고 **각 줄에 같은 `usage` 를 복사** 한다. 줄 단위 합산이 실측 **2.22배** 부풀렸다.
- **합성 효과**: 두 오차가 **반대 방향** 이라 부분 상쇄돼 **1.62배 과대**. "대충 맞아 보이는데
  실은 둘 다 틀린" 상태 — 가장 발견하기 어려운 형태다.
- **해결 (0번)**: 재귀 glob(깊이 미박제) + `message.id` dedup + **소비 주체 분류 신설**
  (`daemon`/`subagent`/`session`). 대시보드에 주체별 막대·카드 추가.
  - ★ 이 과정에서 **내 실수 2건을 실측으로 잡았다**:
    ⓐ 데몬 폴더를 `llm_cwd` 로 매칭 → **Claude Code 는 `_` 도 `-` 로 바꾼다**(`llm-cwd`).
      경로에서 파생하되 *변환 규칙까지* 재현해야 진짜 파생(`_proj_dir_name`).
    ⓑ 저장소 루트를 데몬 폴더 목록에 넣었더니 **사람 세션 전량이 daemon 으로 뒤집혔다**
      (둘이 같은 폴더명). 구분 불가하면 섞어 세는 것보다 **분류하지 않는 편이 정직하다**.
- **해결 (1번) 계측 구멍**: `run_sdk_query`(auto_repair 심층감사·발행실패 수정) 와
  `_invoke_sdk_vision`(design_learner) 이 `record_call` 을 **한 번도 부르지 않았다**.
  가장 무거운 호출들이 장부에 0으로 적혀 "무엇을 줄일까" 를 물어도 답이 안 나왔다. 둘 다 배선.
  · **워크플로 제안 1건은 기각**: "`AssistantMessage.usage` 누적" — SDK 확인 결과 그 클래스에
    `usage` 필드가 **없다**(`content`/`model`/`parent_tool_use_id`). 구현 불가.
- **해결 (B) 심층 감사 주 1회**: `j07_deep_audit` 매일 03:00 → **토요일 03:00**.
  근거: 이 잡이 `guardian` alias 소비의 *전량*(7일 3.41M, 단건 최대 439K·10턴).
  늦어지지 않는 것 = 신규 오류 자동수정(`j07_retry_pending` 10분 + 발행 전 LLM-0 sweep).
  늦어지는 것 = 광범위 코드 감사·backlog 재처리·격리버킷 보고.
- **해결 (C) topic_pack 헛빌드 제거**: 팩 빌드가 `job_collect_trends` 안에 있어 09·12·15시마다
  같은 파일을 다시 만들어 덮어썼는데 **그 판을 먹는 소비자가 없었다**(경제 07:00 은 06시 판을
  이미 먹음 / 테마 21:00 은 `theme_picker` 사용 / 소진 복구는 `pick_slot_candidate` 담당).
  `build_topic_pack_once()` 로 분리해 **아침 잡에서만** 호출. 트렌드 *수집* 은 4회 그대로.
  실측 소비 09+12+15 analyzer 118호출 / 5.27M(장부 16.3%).
- **★ 기각한 대책 (2번)**: 워크플로가 1순위로 올린 *"동일 오류 중복 재분석 44.8M(25.9%)"* 는
  **실측과 달랐다**. 7일 LLM 시도 총합은 **28회**, 같은 지문 중복분은 **약 3회**뿐.
  `guardian_agent.py:781` 에 `attempts > MAX_LLM_ATTEMPTS(3) → wontfix` 캡이 **이미 작동 중**
  (2026-07-06 박제). 고쳤다면 **없는 병에 중복 방어를 얹는** 셈이었다.
- **파일**: `shared/token_usage.py` · `shared/claude_sdk_compat.py` · `shared/llm.py` ·
  `dashboard/{app/page.tsx,lib/api.ts}` · `JARVIS04_SCHEDULER/job_registry.py` ·
  `JARVIS03_RADAR/jobs.py` · `CLAUDE.md` · `JARVIS03_RADAR/CLAUDE_RADAR.md` (+ 코드 주석의
  하드코딩 시각 5곳을 잡 ID 참조로 교체 — 드리프트 원천 차단).
- **교훈**:
  1. **눈금이 틀리면 어떤 절감도 증명할 수 없다.** 그래서 절감 대책보다 *계측 정확화* 가 먼저다.
  2. **오차가 상쇄되면 더 위험하다.** 24% 누락과 122% 과대가 만나 "그럴듯한 수치" 를 만들었다.
     한 방향으로 크게 틀리면 눈에 띄지만, 상쇄된 오차는 아무도 의심하지 않는다.
  3. **분석 결과도 그대로 믿지 말 것.** 9-에이전트 심층분석의 1순위 대책이 *이미 고쳐진 문제*
     였다. 실측으로 확인하지 않았으면 멀쩡한 캡 위에 중복 방어를 얹을 뻔했다.
     교차검증 단계를 둬도 **최종 판단은 내가 재봐야** 한다.
  4. **문서가 코드보다 앞서갈 수 있다.** "서브에이전트까지 포함" 이라 적힌 docstring 바로 아래
     코드가 깊이 2만 훑고 있었다 — 주석은 *의도* 이지 *동작* 이 아니다.

---

## [503] ⚠️ 사고 — 내가 `LIKE '%_ic%'` 로 error_log 3,619행을 지웠다 (2026-07-26)

- **증상**: 테스트 잔여 행을 지우려던 SQL 이 무관한 행까지 삭제. `error_log` 가 **4,200행 → 583행**.
- **원인**: **SQL `LIKE` 에서 `_` 는 단일 문자 와일드카드다.** `module LIKE '%_ic%'` 는
  "임의의 한 글자 + ic" 를 뜻해 `serv`**ic**`e`·`publ`**ic** 같은 무관한 문자열에 전부 걸린다.
  `%_fin%`·`%_rv%`·`%_txtest%` 도 같은 병. 정규식 습관으로 `_` 를 리터럴이라 착각했다.
- **헛다리 아님**: 자동수정 가드와 무관하다. *사람(나)이 친 임시 조회* 였고, 그래서 어떤
  가드도 개입하지 않았다 — 이 사고의 교훈은 가드가 아니라 **손버릇** 에 있다.
- **복구**:
  · `shared/backups/jarvis_2026-07-25.sqlite`(03:00 자동 백업)에서 **3,484행 복원** — 결번 0.
  · 백업 이후 생성분 135행 중 **101행 복구** — 커밋 파생 기록은 *git 이 원본* 이라
    `job_git_audit` 과 동일 호출로 창을 넓혀 재생성(+106, +100), 수동수정 기록 2건 재박제.
  · **미복구 34행** = 오늘 내 회귀 시뮬레이션이 만든 테스트 산물 8행(되살릴 이유 없음) +
    실제 기록 26행. 26행의 *내용* 은 ERRORS [489]~[502] 본문에 남아 있고, DB 행은
    그 색인이었다. **잘린 50자로 되살리면 열화 사본이 되므로 재생성하지 않는다** —
    없는 기록보다 *틀린 기록* 이 더 나쁘다.
- **교훈**:
  1. **DB 에 `DELETE` 를 칠 때는 같은 `WHERE` 로 `SELECT COUNT(*)` 를 먼저 본다.** 예상
     건수와 다르면 거기서 멈춘다. 이번엔 `SELECT` 와 `DELETE` 를 한 스크립트에 붙여 실행해
     확인할 틈이 없었다.
  2. `LIKE` 에 사용자 문자열을 넣을 땐 `_`·`%` 를 이스케이프하거나 `instr(col, ?) > 0` 을 쓴다.
  3. **자동 백업이 이 사고를 살렸다.** 03:00 `db_backup` 잡이 없었다면 3,484행이 영구 소실이었다.
     백업의 값어치는 사고가 나기 전까지 보이지 않는다.

---

## [502] ✅ 해결 — 자동수정이 파일에 쓰는 정책 경로가 둘인데 한쪽엔 가드가 0종이었다 (2026-07-26)

> ★ **맥락**: 사용자 요청 *"오류가 나서 한 개 파일을 백업했지만, 수정하다 보니 여러 파일을
> 수정하게 되면 관련된 여러 파일도 같이 백업하면서 정상적으로 수정하면 좋을 텐데"* →
> 착수 전 전수 조사에서 **더 급한 구멍** 이 나와 사용자 승인 하에 4단계로 진행.

- **증상 (조사로 발견 — 사용자 보고 아님)**: `pattern_fixer.apply_stored_patches()` 가
  매일 04:30 `auto_repair` 에서 Claude SDK 보다 **먼저** 학습 패치 47건을 재적용하는데,
  `_safe_path`·`_removal_issue`·`_import_check`·`verify_fix`·`_DENY_FILES` 가 **하나도 없었다**
  (grep 전수 0건). 구문 검사마저 *파일에 쓴 뒤* 라 깨진 코드가 디스크에 먼저 착지했고,
  성공 시 `bak.unlink()` 로 되돌릴 수단을 스스로 지웠으며, **재현 검증 없이 밴딧 양의 보상**
  까지 줬다 — 2026-07-25 에 "재현 안 해본 fixed 는 fixed 가 아니다" 로 고친 그 병이
  이 경로에만 살아 있었다. `_DENY_FILES` 로 지킨 `learned_patterns.json` 을 대상으로 하는
  학습 항목이 실제로 2건 있었고, 적용되지 않은 건 가드가 아니라 **운**(diff 컨텍스트 불일치)이었다.
- **원인**: 가드는 `apply_fix` 안에 *절차* 로만 존재해 재사용할 형태가 아니었다. 그래서 두 번째
  경로가 생길 때 아무도 가져다 쓰지 않았다. 정책이 *함수* 가 아니라 *흐름* 에 박혀 있으면
  옆문이 생기는 것을 막을 수 없다 (CLAUDE.md 실례 [474] 와 같은 병).
- **해결 (4단계, 사용자 승인 순서대로)**:
  1. **안전 적용 코어** 신설 — `precheck_patchset`(경로안전·구문·삭제가드 전량) →
     `apply_patchset`(백업 전량 → 쓰기 전량 → import 전량) → 실패 시 `rollback_patchset` 전량.
     `apply_stored_patches` 가 이 정문(`apply_files_safely`)만 쓰도록 이관 + 재현검증·음의신호 정합.
  2. **잔버그 2건** — `_safe_path` 의 금지폴더 판정이 *문자열 포함* 이라
     `blogs.py`(b-**logs**)·`dialogs.py`(dia-**logs**)가 영구 수정 불가였다(저장소의 다른 두 곳은
     이미 올바른 패턴이었다 — 여기만 유일한 substring). `.md`/`.sh` 는 삭제가드가 `.py` 한정이라
     **전면 삭제도 통과**했다 → 언어 무관 검사(빈 패치·과다축소)는 전 확장자에 적용.
  3. **다중 파일 원자 트랜잭션** — `patches: [{target_file, patch}]` 계약(기존 단일 키는 1원소로
     승격). ★ 핵심은 **오버레이**: `_referenced_elsewhere` 가 디스크를 읽어 "B 가 아직 참조" 로
     판정하는 바람에, A 에서 함수를 지우고 B 에서 호출부를 지우는 **정당한 리팩터가 영원히 거부**
     됐다(실증). 판정 기준을 "지금 디스크" → **"이 트랜잭션이 끝난 뒤의 저장소"** 로 바꿨다.
     학습도 `stored_patches` 로 N개 전부 박제 — 대표 1개만 저장하면 재적용이 반쪽만 되살린다.
  4. **import 검증 서브프로세스화** — 종전엔 `exec_module` 로 **패치 코드를 살아있는 데몬 안에서
     실제 실행**했고, `sys.modules` 캐시 때문에 "새 A + 옛 B" 를 검사했다(다중 파일에선 검증의
     핵심이 무력화). 재현 프로브의 기존 선례를 따라 깨끗한 인터프리터로 분리.
- **★ 적대적 리뷰가 잡은 내 회귀 2건 (자수)**: 구현 후 8-에이전트 적대 검증 + 직접 재현.
  · `_IMPORT_PROBE_SRC` 에 넣은 `sys.modules[mod_name] = m` 선등록이 **무수정 상태의 정상 파일
    24개(JARVIS09 전량)를 실패로 판정** → 전량 롤백 + *옳은 패치를 낸 arm 에 음의 보상*.
    무수정 저장소 202개 실측: 선등록 24실패 / 선등록 없이 spec+exec 15실패(dataclass 가
    `sys.modules[__module__]` 을 못 찾음) / **`importlib.import_module` 0실패** → 채택.
    검사는 **운영이 실제로 겪는 경로** 를 타야 한다(ERRORS [499] 교훈의 반대 방향 재발).
  · `write_text` 는 **먼저 truncate 한 뒤** 인코딩하므로, 인코딩 예외 시 원본이 이미 0바이트인데
    `written=False` 라 롤백에서 제외됐다 — `.bak` 을 옆에 두고 "되돌렸다" 고 기록했다(실증).
    → 임시파일 + `os.replace` 원자 교체로 **truncate 자체를 제거**. 롤백 실패도 삼키지 않고
    사유에 실어 올린다(파손을 성공으로 집계하던 것을 정정).
- **검증**: 트랜잭션 9종(정상 다중적용·전량롤백·구문오류 전무·import 실패 롤백·금지파일 혼입 거부·
  중복지정 거부·쓰기실패 원본보존·오버레이 통과·반사실 거부) 전부 통과. 무수정 전수 import 오탐 0.
  하위호환 10종 통과. 04:30 스윕 시뮬레이션 0건 적용·0건 거부(회귀 없음). precommit 58종 0건.
- **정직한 한계**: "파일에 닿는 단 하나의 문" 이 아니다. `auto_repair` 의 Claude Code SDK
  자율수정(`bypassPermissions`)은 **여전히 경계 밖** 이다 — 별도 신뢰 경계이며 이 코어로 덮이지 않는다.
  배너에 "단 하나" 라고 적는 것 자체가 *복사본을 진실로 믿는* 사고라 문구를 정정했다.
- **파일**: `JARVIS07_GUARDIAN/error_fixer.py` · `pattern_fixer.py`.
- **교훈**: **정책은 흐름이 아니라 함수로 존재해야 재사용된다.** `apply_fix` 안에 절차로만 있던
  가드는, 두 번째 경로가 생겼을 때 아무도 가져다 쓰지 않았다 — 옆문은 악의가 아니라 *가져다 쓸
  모양이 없어서* 생긴다. 그리고 **검사가 운영이 겪지 않는 상태를 만들어내면 그건 검사가 아니라
  또 하나의 버그다** — `sys.modules` 선등록이 정확히 그것이었다.

---

## [500] ✅ 해결 — 네트워크가 끊긴 2초 때문에 그날 테마글이 통째로 사라졌다 (2026-07-25)

> ★ **맥락**: ERRORS [499] 조사 중 "오늘 글이 1건뿐" 을 발견해 파생. 사용자에게 두 제안을
> 설명하고 **①(알림 아웃박스) → ②(경계 있는 재시도)** 순서로 진행 승인받음.

- **증상**: 2026-07-25 오늘 발행이 경제 네이버 **1건뿐**. `j01_theme_post_21` 은 21:05:31 에
  실행돼 `success=1` 로 끝났는데 글도, `theme_*.log` 도 없다. 잡은 **2초 만에** 종료됐다.
- **원인 (두 겹)**:
  ① 21:05 맥이 깨어나던 순간 네트워크가 죽어 있어 `job_pre_naver_check()` 가 실패 →
     설계대로 "쿠키 점검 실패 → 발행 건너뜀" 으로 종료. **코드 버그가 아니다.**
     그러나 쿠키 점검은 `bool` 하나만 돌려줘 *네트워크 단절(몇 분 뒤면 됨)* 과
     *CAPTCHA·계정(사람 필요)* 이 **구분되지 않는다**. 둘 다 "오늘 발행 없음" 이 된다.
  ② 그 사실을 알리는 텔레그램마저 같은 네트워크 때문에 실패했는데, `notify.send_tg` 는
     실패 시 **로그 한 줄 남기고 메시지를 버렸다**(재시도·보관 0). 그 창에서 4건이 소멸했고
     그중 하나가 `🚨 네이버 쿠키 점검 실패 — 테마글 발행 건너뜀` 이었다.
     → **아무도 모른 채 하루치 글이 없어졌다.**
- **헛다리**: "misfire 유예가 부족했다" 가 아니다. 유예 3600초는 정상 동작했고 잡은 실제로
  (5분 지각) 실행됐다. 빈 곳은 *잡 실행* 이 아니라 **전제조건이 일시적으로 실패했을 때 다시
  묻지 않는 것** 이었다.
- **해결 ① 알림 아웃박스** — `notify_outbox` 테이블 + `shared/notify.py`:
  · 실제 전송은 `_post_message` **하나**로 모으고(재귀 차단), 그 위에 보관·재전송을 얹음.
  · **일시적 실패만 보관**한다: 예외(네트워크·타임아웃)·429·5xx. API 가 응답한 거절
    (`chat not found`·파싱 실패)은 100번 더 보내도 같으므로 **보관하지 않는다**.
  · 복구 시 `_on_send_success()` 가 *성공한 전송에 곧바로 이어* 밀린 것을 흘려보낸다.
    주기 잡 `notify_outbox_flush`(5분)는 조용한 시간대를 위한 바닥.
  · 지연 전송본에는 `🕐 HH:MM 발생 (지연 전송)` 을 붙인다 — 3시간 전 일을 지금 일로
    오해하는 것이 유실보다 더 나쁘기 때문. 유효기간 `NOTIFY_OUTBOX_TTL_H`(기본 6h) 초과분은 폐기.
  · **버튼 메시지는 보관하지 않는다**(의도적): 승인 대기 상태는 데몬 *메모리* 라 되살려
    보내도 눌리지 않고, 지난 승인을 지금 승인한 것으로 오해할 위험이 있다. 대신
    "요청이 있었다" 는 사실만 글로 남긴다.
  · 프로세스 경계: 발행 subprocess 와 데몬이 동시에 흘려보낼 수 있어 `outbox_claim()`
    (attempts 조건부 UPDATE) 낙관적 선점으로 중복 전송을 막는다. 메모리 잠금만으로는 못 막는다.
- **해결 ② 경계 있는 재시도** — `login_manager.ensure_naver_ready(deadline=)`:
  · `network_up()` 으로 원인을 가른다 — 네트워크 down = 일시적(재시도), up = 영구적(사람 호출).
  · **재시도 창은 그 잡 자신의 misfire 유예시간에서 파생**(`job_registry.job_window_deadline`).
    07:00 잡 → 08:00, 21:00 잡 → 22:00. 여기서 "몇 분 더" 같은 숫자를 만들지 않는다 —
    만들면 그게 곧 시간외 발행의 씨앗이다(사용자 박제 "발행은 07시와 21시뿐").
    `deadline=None`(창을 모름)이면 **기다리지 않는다**.
  · 잡 ID 도 박지 않는다 — `shared.llm.current_job_id()` 로 문맥에서 조회(신설).
  · ③ 모든 조합: 종전 쿠키 점검 블록이 경제·테마 콜백에 **두 벌로 복사**돼 있던 것을
    `scheduler._naver_cookie_ready()` 하나로 합쳤다. `_is_network_up()` 도 네이버·티스토리
    refresher 양쪽 **사본 2벌**이었던 것을 `login_manager.network_up()` 단독으로 통합.
- **검증**: 아웃박스 — ①단절→보관 ②영구실패→미보관 ③버튼→사실만 보관 ④복구→재전송+지연표기,
  그리고 **실제 네트워크로** 단절 재현 후 복구 시 즉시 회수까지 확인.
  재시도 — ①창 파생(07→08:00 / 21→22:00) ②네트워크 정상+실패→즉시 permanent(0.0s)
  ③단절+창 안→재시도 후 회복 ④단절+창 밖→즉시 포기(0.0s) ⑤창 미상→대기 없음.
  precommit 58종 0건.
- **파일**: `shared/notify.py`, `shared/db.py`, `shared/llm.py`,
  `JARVIS08_PUBLISH/credentials/{login_manager,naver_cookie_refresher,tistory_cookie_refresher}.py`,
  `JARVIS08_PUBLISH/credentials/LOGIN_SUPREME_LAW.md`(제2-B조 신설),
  `JARVIS04_SCHEDULER/job_registry.py`, `JARVIS02_WRITER/scheduler.py`.
- **교훈**: **실패를 한 종류로 뭉뚱그리면 대응도 한 종류가 된다.** `bool` 하나로 "안 됐다" 만
  전하면, 몇 분 뒤면 될 일과 사람이 와야 할 일이 같은 취급을 받는다. 전제조건 검사는
  *성공/실패* 가 아니라 **"어떤 종류의 실패인가"** 를 돌려줘야 한다.
  그리고 **알림은 시스템의 눈이다.** 눈이 감긴 채 실패하면 실패는 없었던 일이 된다 —
  알림 경로의 신뢰성은 기능이 아니라 *관측 가능성의 전제조건* 이다.
...
- **발단**: 사용자 질문 — *"각 항목에 대해 자세히 설명해줘."* 설명하려고 출처를 따라가다
  **라벨 2종이 또 틀렸음** 을 발견. ([480][481] 에 이은 3번째 라벨 사고)
- **원인 A — 학습 점수를 백테스트라 불렀다**:
  화면의 `백테스트 48.9%` 는 `learned_weights.r2` 였다. 이건 **학습에 쓴 데이터로 자기
  자신을 채점한 값** 이다. 진짜 백테스트(안 써본 데이터 검증)는 `backtest_history.r2` = **33.0%**.
  학습 점수는 언제나 낙관적이므로 **실력을 15%p 과대평가** 하고 있었다.
  시험 문제를 미리 보고 푼 점수를 모의고사 점수라 부른 셈.
- **원인 B — '영향 없음' 과 '판단 불가' 가 똑같이 0% 로 보였다**:
  `velocity`·`competition` 막대가 0% 로 표시돼 *"영향이 없다고 학습했다"* 처럼 읽혔다.
  실제로는 `learn_log` 적재 코드가 **상수를 하드코딩** 하고 있었다:
  ```python
  freshness=freshness, velocity=0.0, competition=50.0,   # ← 실제 값이 아님
  ```
  고유값이 각각 **1종** 뿐이라 학습이 *원리적으로 불가능* 하다.
  더구나 `trends` 테이블에 velocity/competition 컬럼 자체가 없어 **데이터 출처가 아예 없다.**
  즉 두 입력은 모델에 이름만 있는 **유령 피처** 다.
- **해결**:
  · API: `backtest_score` → **`train_r2`(자기 채점) + `backtest_r2`(안 써본 데이터)** 분리.
    학습·백테스트는 같은 잡에서 연이어 돌므로 날짜로 매칭. `n_samples` 도 함께 노출.
  · 화면: `학습 정확도 48.9% · 백테스트 33.0% (안 써본 데이터 검증 — 이쪽이 실력)` 로 표기.
  · API `feature_variance` 신설(입력별 `COUNT(DISTINCT)`) → 고유값 1종이면 막대 대신
    **"데이터 없음 — 입력이 항상 같은 값이라 학습 불가"** 로 표시. 0% 와 구별한다.
- **파일**: `api_server.py` · `dashboard/app/learning/page.tsx` · `dashboard/lib/api.ts`
- **검증**: API 실응답(train_r2 0.489 / backtest_r2 0.330 / 표본 366,
  feature_variance = trend 5 · perf 26 · fresh 31 · **velocity 1 · competition 1**) ·
  `tsc --noEmit` 통과 · :9199 HTTP 200 + 번들에 새 라벨 반영.
- **미해결(보고만)**: `velocity`·`competition` 은 수집 경로가 없다. 구현하거나 FEATURES 에서
  빼야 하는데, 모델 입력 변경이라 사용자 판단 필요. 현재는 화면에 "데이터 없음" 으로 정직하게 노출.
- **교훈**: **train score 와 test score 를 같은 이름으로 부르지 말 것.** 학습 점수는 항상 좋게
  나오므로 그것을 성능이라 표시하면 시스템을 과신하게 된다.
  그리고 **0 은 "없음" 이 아니라 "0" 이다** — 측정 불가를 0으로 채우면 사용자는 결론으로 읽는다.

---

## [483] ★★ 조회수 학습 3단 붕괴 — 플랫폼이 안 주는 값을 정답으로 삼고 있었다 (2026-07-22)

- **발단**: [482] 로 URL 누락을 고쳤는데도 **URL 있는 99건 중 조회수>0 은 4건** 뿐이었다.
  URL 은 원인의 절반이었다.
- **원인 A — 플랫폼이 조회수를 안 준다 (코드로 못 고침)**:
  실측(직접 호출):
  ```
  [네이버]  패턴 8개 모두 매칭 실패 (응답 길이 2819자)   ← 정상 페이지는 수만 자. 비로그인엔 조회수 없음
  [티스토리] 조회수 미수집 (정책 한계 — ERRORS.md [20])   ← 코드에 이미 명시돼 있었음
  ```
- **원인 B — 그 값을 학습 정답으로 삼아 3단이 연쇄 붕괴**:
  · `learn_log.actual_views` 366행 중 **365행이 0**
  · 학습기: `len(unique(y)) < 2` → **조용히 포기**, `learned_weights` **영구 0행**
    (잡은 `success=1` 로 보고 → "성공했는데 결과가 없는" 상태가 몇 달 지속)
  · 백테스트: 정답값이 상수면 sklearn `r2_score` 가 1.0 → **가짜 100%** (변별력 0)
  · analyzer 는 학습 실패를 모른 채 **하드코딩 기본값**(w_trend=0.45…)으로 키워드 선정
- **원인 C — 순위 신호가 있었는데 SELECT 에서 빠져 있었다**:
  `learning.py` 주석에 *"학습 신호 2종: actual_views OR naver_rank"* 라고 적혀 있고
  적재도 2종을 받는데, **학습 y 는 `actual_views` 만** 썼다. 게다가
  `db.learn_log_fetch()` 의 SELECT 에 `naver_rank` 가 **아예 없어** 볼 수도 없었다.
- **해결 (사용자 판단 — (다)+(나))**:
  · **(나) 정답값 교체** — `learning.build_target(rows, min_signal)` **단일 진입점** 신설.
    조회수에 *0 아닌 표본이 min_samples 이상* 이면 조회수를, 아니면 `naver_rank` 를 쓴다.
    순위는 작을수록 좋으므로 `log1p(100 - rank)` 로 뒤집어 "높을수록 좋음" 통일.
    미노출은 100위(=0점). **조회수가 되살아나면 자동으로 되돌아간다** — 데이터를 보고 고른다.
    ★ "고유값 2종 이상" 조건으로 처음 짰다가 실패 — 366행 중 365행이 0, 1행이 1 이라
      통과해버렸다. *변별의 유무가 아니라 양* 을 봐야 했다.
  · `db.learn_log_fetch()` SELECT 에 `naver_rank` 추가 + WHERE 를 `OR naver_rank IS NOT NULL` 로.
  · **(다) 조용한 실패 제거** — 학습 보류 시 사유(`정답값 변별 없음`/`샘플 부족`)와 사용 신호를
    반환·출력. 백테스트는 **검증 구간 정답값이 상수면 저장하지 않는다**(가짜 만점 차단).
  · 백테스트도 `build_target` 을 쓰게 통일 — 학습과 평가가 다른 정답을 쓰면 안 된다.
- **결과 (실측)**:
  ```
  before  learned_weights 0행          · backtest r2=1.0 (가짜 만점)
  after   learned_weights #1 n=366     · r2=0.489    ← 최초 학습 성공
          backtest n=366 r2=0.330 mape=90.6%          ← 진짜 값
  ```
- **파일**: `JARVIS03_RADAR/learning.py` · `shared/db.py`
- **교훈**: **정답값이 없는 학습은 실패가 아니라 침묵한다.** 학습기는 조용히 포기하고
  잡은 성공으로 보고했으며 백테스트는 만점을 냈다 — 세 신호 모두 "정상" 이었다.
  *수집 가능한 것* 을 정답으로 삼을 것. 그리고 **"고유값이 2종 이상" 같은 최소 조건은
  변별을 보장하지 않는다** — 365:1 도 2종이다.

---

## [482] ★ 발행 URL 이 저장되지 않아 조회수 수집이 불가능했다 (2026-07-22)

- **발단**: "가중치 변화 — 최신 3회" 카드가 *"가중치 데이터가 없습니다"* 로 비어 있음.
  추적하니 `learned_weights` 0행 → 학습 실패 → 원천 `actual_views` 전부 0 → **조회수 미수집**
  → **URL 부재** 로 거슬러 올라갔다.
- **원인**: 발행자(`naver_poster`/`tistory_poster`)는 발행 직후 RSS 로 URL 을 확보해
  `_last_post_url` 에 담아둔다. `bus.on_post_published_detail` 도 `url` 인자를 받는다.
  그런데 **호출부 6곳이 아무도 그 값을 넘기지 않았다.**
  → `post_analysis.url` 이 **2026-05-18 이후 전부 빔**(221건 중 122건). 최근 발행 6건 전부 공백.
  URL 이 없으면 조회수 수집기는 긁을 대상 자체가 없다.
- **해결**: `JARVIS08_PUBLISH.platforms.last_post_url(platform)` **단일 진입점** 신설
  (발행 도메인이 URL 의 주인). 호출부 6곳(경제·테마 × 네이버·티스토리 + 레거시 2)에
  `url=_last_url("<platform>")` 연결. 호출자가 각자 `_last_post_url` 을 직접 읽지 않게 했다 —
  플랫폼이 늘면 호출부마다 분기가 생긴다.
- **파일**: `JARVIS08_PUBLISH/platforms/__init__.py` ·
  `JARVIS02_WRITER/{trend_theme_writer,trend_economic_writer}.py`
- **검증**: 발행자 URL → `last_post_url()` 조회 → `emit(url=)` → DB 저장까지 실제 확인.
  URL 미확보(RSS 실패) 시 빈 문자열로 안전 처리도 확인.
- **교훈**: **인자가 있는데 안 넘기는 것은 문법 오류가 아니라 조용한 데이터 손실이다.**
  기본값 `url: str = ""` 이 누락을 정상처럼 삼켰다. 하류(조회수·학습)가 죽고 나서야 드러났다.

---

## [481] ★ '지침 주입' 이 7배 부풀려져 있었다 — 다른 컬럼을 세고 있었음 (2026-07-22)

- **발단**: 사용자 질문 — *"각 항목을 자세히 설명해봐. 뭔 의미인지 모르겠어."*
  설명하려고 코드를 확인하다 **내가 붙인 라벨 2개가 틀렸음** 을 발견.
- **원인 A — '지침 주입 661' 이 주입이 아니었다**:
  [480] 에서 `SUM(occurrences)` 를 '지침 주입' 으로 표시했다. 그러나
  · `occurrences` = **"이 지침이 글 분석에서 *재발견* 된 횟수"** — 학습 루프 **①단계**
  · 실제 주입 = `insight_usage` 테이블 행 수 = **92** — **②단계**
  `quality_learner.stats()` 도 `total_usage` 를 `COUNT(*) FROM insight_usage` 로 세고 있었다.
  → **661 vs 92, 7배 부풀림.** ①단계 값을 ②단계인 것처럼 보여줬다.
- **원인 B — '평균 지침 가중치' 는 학습 성과 지표가 아니다**:
  `weight`(0.802)는 *"내가 이 지침을 얼마나 믿나"* 라는 **내부 상태** 이고,
  `reward_sum/reward_count`(0.557)가 *"실제로 글이 좋아졌나"* 라는 **결과** 다.
  학습이 되는지 보려면 후자여야 한다.
- **해결**: KPI 를 **학습 순환 4단계** 로 재구성 —
  `① 쌓인 지침 309` · `② 실제 주입 92` · `③ 성과 채점 8` · `④ 평균 보상 0.557`.
  아래에 순환 설명 + **병목 경고**(③이 ①의 1/10 미만이면 경고) 표시.
  `rediscovered`(재발견 누계)·`avg_weight` 는 참고값으로 API 에 남기되 KPI 에서 뺐다.
- **부수 조사 — ②③ 이 안 도는 게 아니었다 (자체 정정)**:
  조사 전 "②③④가 거의 안 돈다" 고 사용자에게 보고했으나 **과한 진단** 이었다.
  · 주입 기록은 `2026-07-20 22:07` 부터 — **이틀 전 시작**. 지침 309개는 그 전에 쌓인 것.
  · 최근 14일 발행 32건 중 주입 배치 23건 = **71%**, 최근 4일 발행 15건 중 미분석 **0건**.
  · 미채점 74건은 전부 `7/21 22:39` 이후 주입분 — 보상 귀속 잡이 **매일 23:45** 라
    아직 순번이 안 온 것(경계가 잡 실행시각과 정확히 일치). 구조 문제 아님.
- **미해결(의도적 보류) — 탐색 폭**: 보상받은 지침이 309개 중 **2개** 뿐인 이유는
  `get_ranked_learning_insights` 가 `effective_weight DESC LIMIT limit*3`(=24) 로
  **후보를 미리 자르고** UCB 에 넘기기 때문. UCB 탐색 보너스는 정상 작동하지만
  *후보 목록에 없으면 뽑힐 수 없다.* + `last_seen >= -21일` 필터 + 주당 0.7 감쇠가 겹쳐
  "뒤처진 지침은 계속 뒤처지는" 고리가 생긴다.
  다만 **저품질 지침 유입을 막는 안전장치일 수도** 있어, 주입 데이터가 이틀치뿐인 현재
  판단을 보류. 화면에 `보상받은 지침 N개` 가 보이므로 일주일 뒤 늘지 않으면 재검토.
- **파일**: `api_server.py` · `dashboard/app/learning/page.tsx` · `dashboard/lib/api.ts`
- **검증**: API 실응답(① 309 · ② 92 · ③ 8 · ④ 0.557) / `tsc --noEmit` 통과 /
  :9199 HTTP 200 + 번들에 '④ 평균 보상' 포함.
- **교훈**: **컬럼 이름이 비슷하다고 같은 뜻이 아니다.** `occurrences`(재발견)와
  `insight_usage`(주입)는 이름이 다른 만큼 뜻도 달랐는데, 확인 없이 라벨을 붙였다.
  *지표를 만들 땐 그 값을 올리는 코드를 먼저 찾아볼 것.*
  그리고 **"안 돌고 있다" 고 진단하기 전에 언제 시작됐는지 볼 것** — 갓 시작한 것과
  고장난 것은 숫자만으로 구별되지 않는다.

---

## [480] ★ 자기학습 KPI 라벨이 *어느 학습인지* 를 말하지 않았다 (2026-07-22)

- **발단**: 사용자 지적 — *"학습 패턴 → 이게 블로그 품질 학습패턴인지 오류 학습 패턴인지
  알 수가 없다. 패턴 적중도, LLM 없이 해결도 마찬가지."*
- **원인**: 이 시스템의 자기학습은 **서로 다른 두 시스템** 이다:
  · ① 오류 자가수리 학습 (GUARDIAN) — 런타임 오류를 스스로 고침
  · ② 글 품질 학습 (ADR 014, quality_learner) — 블로그 글을 더 잘 쓰게 함
  [479] 에서 KPI 를 ①의 지표로 바꿔놓고 **라벨에 "오류" 를 안 붙였다.**
  게다가 페이지 부제는 여전히 *"품질 강화학습 가중치·인사이트·백테스트 현황"* 이라
  **①의 숫자를 ②의 제목 아래 보여주는** 상태였다 — 혼동을 오히려 키웠다.
- **해결**:
  · KPI 라벨에 소속 명시: `오류 학습 패턴` · `오류 패턴 적중` · `오류 — LLM 없이 해결` ·
    `오류 자동해소율`.
  · 부제를 실제 내용으로 교정: *"서로 다른 두 학습 시스템 — ① 오류 자가수리(GUARDIAN)
    ② 글 품질(ADR 014)"*.
  · **② 글 품질 학습 섹션 신설** — 탭이 ①만 보여주면 '자기학습' 이라는 이름값을 못 한다.
    KPI 4종(품질 지침 309 · 지침 주입 661 · 보상 검증 8 · 평균 가중치 0.802) +
    추세 차트 2종(지침 누적 53일치 · 신규 지침 일별).
    API 에 `quality_now` / `quality_timeline` 추가(`learning_insights` 파생).
  · 섹션마다 제목 + 한 줄 설명을 붙여 *어느 시스템의 숫자인지* 화면에서 바로 읽히게 함.
- **파일**: `api_server.py` · `dashboard/app/learning/page.tsx` · `dashboard/lib/api.ts`
- **검증**: `tsc --noEmit` 통과 / API 실응답(오류학습 패턴44·적중50·timeline 60점,
  품질학습 지침309·주입661·보상8·timeline 53점) / :9199 HTTP 200 +
  번들에 '오류 자가수리 학습'·'글 품질 학습' 두 섹션 문자열 포함 확인.
- **교훈**: **숫자를 고치는 것과 그 숫자가 무엇인지 밝히는 것은 별개다.**
  [479] 에서 값은 정확하게 만들었지만 라벨이 소속을 숨겨, 보는 사람에겐 여전히
  해석 불가였다. *지표는 값·라벨·맥락 셋이 맞아야 지표다.*
  특히 **제목(부제)과 내용이 어긋나면 값이 정확해도 오독을 유도한다.**

---

## [479] ★ AI 자기학습 탭 KPI 4개가 전부 무의미 + 합성 지문이 학습 자산을 부풀림 (2026-07-22)

- **발단**: 사용자 질문 — *"이 4개가 대표로 가장 중요한 지표 맞냐?"*
  화면에 **학습 샘플 0 인데 백테스트 정확도 100%** 라는 모순이 보였다.
- **원인 A — 숫자가 틀렸거나 무의미**:
  · `학습 샘플 수 = 0` — `learned_weights` 테이블이 **0행**. RADAR 키워드 회귀 학습기가
    아예 안 돌고 있다. 그런데 KPI 4개 중 3개가 이 학습기 산출물이었다.
  · `백테스트 정확도 = 100.0%` — 전 회차 `r2=1.0, mse=0.0, mape=0.0`.
    백테스트 코드 자체는 정상(train/test 분할·Ridge·held-out)인데, **정답값이 상수**면
    sklearn `r2_score` 가 1.0 을 돌려준다. 즉 **데이터가 없어서 나온 가짜 만점** —
    좋아져도 나빠져도 영원히 100%, 변별력 0.
  · `학습 인사이트 = 20` — API 의 `LIMIT 20` **배열 길이를 실제 개수로 착각**.
    DB 에는 309개. (CLAUDE.md '복사본을 진실로 믿지 말 것' 의 전형)
- **원인 B — 합성 지문이 학습 자산을 부풀림**:
  `pattern_fixer.py` 가 SDK 수정 시 *부수 변경 파일마다* `AutoRepairFix::<경로>` 지문을
  학습 등록했다. 이건 "무엇을 고쳤다" 는 **작업 기록** 이지 "무엇이 실패했다" 는
  **오류 지문이 아니다** — 재발할 수 없으므로 영원히 재사용되지 않는다.
  실측: 52개 중 **8개(15%)** 가 이것이었고 **적중 상위 5개 중 4개** 를 차지.
  → "학습 패턴 N개 · LLM 절약 N회" 를 부풀리고 있었다.
  CLAUDE.md/ADR 005 가 "Tier 2 도 *실제 오류 지문* 으로 학습(AutoRepairFix 합성 지문 아님)"
  이라고 못박은 그대로다.
- **헛다리(자체 정정)**: 조사 초반 `learned_patterns.json` 의 **최상위 키 2개**
  (`version`/`patterns`)를 패턴 수로 오해해 "패턴 2종·적중 0회 — 학습 정체" 라고
  사용자에게 잘못 보고했다. 실제 구조는 `patterns` **배열 51개**. 즉시 정정.
  → JSON 을 셀 때 *구조를 먼저 확인* 할 것.
- **해결**:
  · **노이즈 게이트 4 신설** (`pattern_fixer._SYNTHETIC_TYPES`) — 합성 지문 등록 차단.
    기존 8개도 제거(`.bak` 백업). **패턴 52→44개 · 적중 69→50회**
    (부풀려졌던 적중 19회 = 28% 제거).
  · **KPI 4개 교체** — 안 도는 학습기 지표 → 이 시스템 자기학습의 본체(GUARDIAN 오류학습)로:
    `학습 패턴` · `패턴 적중` · `LLM 없이 해결` · `오류 자동해소율`.
    선정 기준은 **"나빠지면 나빠진 게 보이는가"** — 백테스트 100% 는 그러지 못했다.
  · **각 KPI 아래 시계열 차트** — 숫자 하나는 '지금 몇 개' 만 알려주고 *추세* 를 못 보여준다.
    원천은 `self_repair_runs`(100회차, 5/15~) + `error_log` 일별 집계.
    홈탭과 동일한 recharts 패턴 사용 — 새 라이브러리·새 Next.js API 도입 0
    (`dashboard/AGENTS.md` 가 "이 Next.js 는 학습 데이터와 다르다" 고 경고).
  · `insights_total` 별도 조회 필드 추가 — LIMIT 길이 착각 재발 차단.
- **파일**: `JARVIS07_GUARDIAN/pattern_fixer.py` · `api_server.py` ·
  `dashboard/app/learning/page.tsx` · `dashboard/lib/api.ts`
- **검증**: 합성 지문 등록 시도 → 차단(0) / 실제 오류(ImportError) → 정상 학습(1) /
  `tsc --noEmit` 통과 / API 실응답(insights_total 309 · patterns_now 44·50 ·
  timeline 60점 · resolve_rate 30점) / :9199 HTTP 200 + 번들에 새 라벨 포함 확인.
- **교훈**: **지표가 항상 만점이면 그건 좋은 게 아니라 고장난 것이다.**
  변별력 없는 지표는 없느니만 못하다 — 잘 되고 있다는 착각을 준다.
  그리고 **학습 자산은 개수가 아니라 *재사용 가능성* 으로 세야 한다.**
