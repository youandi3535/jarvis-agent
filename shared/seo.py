...
def seo_score(title: str, body: str, keyword: str) -> dict:
    """SEO 4원칙 충족도 평가.

    원칙:
        ① 제목 맨 앞 (10자 이내) 키워드 등장
        ② 본문 첫 2문장(약 100자) 안 키워드 등장
        ③ 본문 3분할 (서론·본론·결론) 모두 등장
        ④ 본문 10문장(약 500자)당 1회 ± 50% (스터핑 금지)

    반환:
        {
            "score": 0~100,
            "title_ok": bool, "intro_ok": bool, "distrib_ok": bool, "density_ok": bool,
            "title_pos": int, "kw_count": int, "ideal_count": int,
            "body_korean_len": int,
        }
    """
    plain = sanitize_body(body)
    kor_len = count_korean(plain)

    if not keyword:
        return {"score": 0, "title_ok": False, "intro_ok": False,
                "distrib_ok": False, "density_ok": False,
                "title_pos": -1, "kw_count": 0, "ideal_count": 0,
                "body_korean_len": kor_len, "verdict": "키워드 없음"}

    # 키워드 정규화 — 괄호/특수문자 제거 후 첫 단어로 매칭
    kw_clean = re.sub(r"[()\[\]·,]+", " ", keyword).strip()
    primary_kw = kw_clean.split()[0] if kw_clean.split() else keyword
    if len(primary_kw) < 2:
        primary_kw = keyword  # 너무 짧으면 원본

    # ① 제목 맨 앞
    title_clean = re.sub(r"\[[^\]]*\]\s*", "", title or "").strip()
    title_pos = title_clean.find(primary_kw)
    if title_pos == -1:
        title_pos = title_clean.find(keyword)
    title_ok = (0 <= title_pos <= 10)

    # ② 본문 첫 2문장(약 100자)
    intro = plain[:100]
    intro_ok = (primary_kw in intro) or (keyword in intro)

    # ③ 분포
    if plain and len(plain) >= 600:
        third = len(plain) // 3
        s1, s2, s3 = plain[:third], plain[third:2*third], plain[2*third:]
        in_s1 = (primary_kw in s1) or (keyword in s1)
        in_s2 = (primary_kw in s2) or (keyword in s2)
        in_s3 = (primary_kw in s3) or (keyword in s3)
        distrib_ok = in_s1 and in_s2 and in_s3
    else:
        distrib_ok = False

    # ④ 밀도 — 한글 본문 10문장(약 500자)당 1회 적정
    kw_count = len(re.findall(re.escape(primary_kw), plain))
    if kw_count == 0 and primary_kw != keyword:
        kw_count = len(re.findall(re.escape(keyword), plain))
    ideal = max(1, kor_len // 500)
    low  = max(1, ideal // 2)
    high = max(2, int(ideal * 1.5))
    density_ok = low <= kw_count <= high
    if kw_count > high:
        verdict = f"과다 ({kw_count}/{ideal})"
    elif kw_count < low:
        verdict = f"부족 ({kw_count}/{ideal})"
    else:
        verdict = f"적정 ({kw_count}/{ideal})"

    score = sum([title_ok, intro_ok, distrib_ok, density_ok]) * 25

    return {
        "score": score,
        "title_ok": title_ok, "intro_ok": intro_ok,
        "distrib_ok": distrib_ok, "density_ok": density_ok,
        "title_pos": title_pos, "kw_count": kw_count, "ideal_count": ideal,
        "body_korean_len": kor_len, "verdict": verdict,
